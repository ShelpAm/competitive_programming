...
[[maybe_unused]] constexpr auto mod{998244353};
[[maybe_unused]] constexpr auto big{0x3f3f3f3f};
...
namespace impl {
  ...
  template<typename T> concept is_string = std::same_as<std::string, std::remove_cvref_t<T>>;
...
static inline auto solve_case()
{
  ...
  cin >> n;
  vec2<string> v(n, n, "?");
  arr1<long long, 4> dx{1, 0, -1, 0}, dy{0, 1, 0, -1};
  size_t d{};
  size_t x{}, y{};
  for (size_t i{}; i != n * n; ++i) {
    v[y][x] = std::to_string(i + 1);
    auto nx{x + dx[d]}, ny{y + dy[d]};
    if (nx >= n || ny >= n || v[ny][nx] != "?") {
      d = (d + 1) % 4;
      nx = x + dx[d];
      ny = y + dy[d];
    }
