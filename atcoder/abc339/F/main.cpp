...
static inline auto solve_case()
{
  ...
  cin >> n;
  impl::vec1<vec1uz> s;
  map<vec1uz, size_t> cnt;
  vec1uz mods{167772161, 469762049, 754974721, 1045430273, 1051721729, 1053818881};
  for (size_t i{}; i != n; ++i) {
    string a;
    cin >> a;
    vec1uz v;
  ...
  for (auto const& a: s) {
    for (auto const& b: s) {
      vec1uz v;
      for (size_t i{}; i != mods.size(); ++i) {
        v.emplace_back(a[i] * b[i] % mods[i]);
      }
      if (cnt.contains(v)) {
        ans += cnt[v];
      }
