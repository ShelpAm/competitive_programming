...
static inline auto solve_case()
{
  ...
  vec1uz a(n);
  cin >> a;
  vec1uz last(n + 1), len(N + 1);
  set<size_t> points;
  for (auto const i: a) {
    size_t max_len_ele{i};
    auto begin{points.lower_bound(i > d ? i - d : 0)}, end{points.upper_bound(min(i + d, N))};
    for (auto it{begin}; it != end; ++it) {
      if (len[*it] > len[max_len_ele]) {
        max_len_ele = *it;
      }
    }
    len[i] = len[max_len_ele] + 1;
    points.emplace(i);
  }
  return max(len);
}
