...
static inline auto solve_case()
{
  ...
  auto dis{[n](long long const a, long long const b) { return b >= a ? b - a : b - a + n; }};
  vec1ll delta(n + 1); // delta[i] denotes delta cost after breaking the path next to i.
  auto add{[&delta](long long const src, long long const dest, long long const dis,
                    long long const re_dis) {
    delta[src] -= dis - re_dis;
    delta[dest] += dis - re_dis;
    if (src > dest) {
      delta[0] += -dis + re_dis;
    }
  }};
  for (auto i{0}; i != m - 1; ++i) {
    add(x[i], x[i + 1], dis(x[i], x[i + 1]), dis(x[i + 1], x[i]));
  }

  size_t ans{inf<size_t>};
  size_t len{};
  for (auto i{0}; i != m - 1; ++i) {
    len += dis(x[i], x[i + 1]);
  }
