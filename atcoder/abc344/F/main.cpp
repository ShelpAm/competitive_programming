...
auto solve_case()
{
    u64 n;
    cin >> n;
    vvu p(n, vu(n)), r(n, vu(n - 1)), d(n - 1, vu(n));
    cin >> p >> r >> d;

    std::vector<std::vector<map<puu, u64>>> f(n, std::vector<map<puu, u64>>(n, map<puu, u64>{}));
    f[0][0][{p[0][0], 0}] = 0;
    ...
    for (u64 i = 0; i != n; ++i) {
        for (u64 j = 0; j != n; ++j) {
            ...
            for (auto [di, dj]: std::vector<puu>{{-1, 0}, {0, -1}}) {
                if (i + di >= n || j + dj >= n) {
                    continue;
                }

                for (auto it = f[i + di][j + dj].begin(); it != f[i + di][j + dj].end(); ++it) {
                    bool rubbish = false;
                    for (auto jt = f[i + di][j + dj].begin(); jt != f[i + di][j + dj].end(); ++jt) {
                    ...
                    if (rubbish) {
                        it = std::prev(f[i + di][j + dj].erase(it));
                    }
                }

                auto const mov = (di != 0 ? d : r)[i + di][j + dj];
                for (auto const& [k, v]: f[i + di][j + dj]) {
                    auto const [maxp, rema] = k;
                    auto const neo_p = max(maxp, p[i][j]);
                    if (rema < mov) {
                        u64 earning_times = (mov - rema - 1) / maxp + 1;
                        if (!f[i][j].contains({neo_p, rema - mov + earning_times * maxp})) {
                            f[i][j][{neo_p, rema - mov + earning_times * maxp}] = inf<u64>;
                        }
                        check_min(f[i][j][{neo_p, rema - mov + earning_times * maxp}], v + 1 + earning_times);
                    }
                    else {
                        if (!f[i][j].contains({neo_p, rema - mov})) {
                            f[i][j][{neo_p, rema - mov}] = inf<u64>;
                        }
                        check_min(f[i][j][{neo_p, rema - mov}], v + 1);
                    }
                }
            }
        }
    ...
    // for (u64 i = 0; i != n; ++i) {
    //     for (u64 j = 0; j != n; ++j) {
    //         for (auto const& [k, v]: f[i][j]) {
    //             auto const [maxp, rema] = k;
    //             debug("i, j, maxp, remai, f", vu{i, j, maxp, rema, f[i][j][{maxp, rema}]});
    //         }
    //     }
    // }

    u64 ans = inf<u64>;
    for (auto const& [k, v]: f[n - 1][n - 1]) {
        check_min(ans, v);
    }
    return ans;
}
