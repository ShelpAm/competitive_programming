...
auto solve_case()
{
    ...
    auto already_has = [&](i64 h, i64 w, i64 p, i64 q) {
        auto x1 = p, y1 = q, x2 = p + x1, y2 = q + y1;
        auto w1 = h, h1 = w;
        for (auto const& [x3, y3, x4, y4]: rects) {
            auto w2 = abs(y3 - y4), h2 = abs(x3 - x4);
            if (abs(x3 + x4 - x1 - x2) <= 2 * (w1 + w2) || abs(y3 + y4 - y1 - y2) <= 2 * (h1 + h2)) {
                return true;
            }
        }
        return false;
    };
    auto dfs = [h, w, &rects, fully_covered, already_has, xy](auto self, vu const& list, u64 u) {
        if (fully_covered()) {
            return true;
        }
        if (u == list.size()) {
        ...
        for (i64 i = 0; i + x <= h; ++i) {
            for (i64 j = 0; j + y <= w; ++j) {
                if (!already_has(x, y, i, j)) {
                    rects.push_back({i, j, i + x - 1, j + y - 1});
                    if (self(self, list, u + 1)) {
                        return true;
                    }
                    rects.pop_back();
                }
    ...
    for (i64 i = 0; i != 1 << n; ++i) {
        ...
        for (i64 j = 0; j != n; ++j) {
            if (i & (1 << j)) {
                list.push_back(j);
                area += xy[j].first * xy[j].second;
            }
        }
        if (area == h * w && dfs(dfs, list, 0)) {
            return true;
        }
    }
    return false;
