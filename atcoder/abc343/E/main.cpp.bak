...
auto solve_case()
{
    ...
    auto calc = [&](vvvu& cnt) {
        ...
        return tripleu{t[1], t[2], t[3]};
    };

    bool ok = false;
    std::vector<tripleu> ans{{0, 0, 0}};
    auto dfs = [&](auto self, std::vector<tripleu>& points, vvvu& cnt) {
        if (ok) {
            return;
        }
        if (points.size() == 3) {
            if (auto const& [a, b, c] = calc(cnt); tripleu{a, b, c} == tripleu{v1, v2, v3}) {
                ok = true;
                ans = points;
            }
            return;
