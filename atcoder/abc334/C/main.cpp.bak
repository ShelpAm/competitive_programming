...
static inline auto solve_case()
{
  ...
  if (k % 2 == 0) {
    size_t res{};
    for (size_t i{}; i < k; i += 2) {
      res += a[i + 1] - a[i];
    }
    cout << res;
  }
  else {
    vec1<size_t> d_sum_first(0);
    vec1<size_t> d_sum_second(0);
    for (size_t i{}; i < k; i += 2) {
      d_sum_first.emplace_back(a[i + 1] - a[i] + d_sum_first.back());
      d_sum_second.emplace_back(a[i + 2] - a[i + 1] + d_sum_second.back());
    }
    size_t m{big};
    for (size_t i{}; i != k; ++i) {
      check_min(m, i % 2 == 0
                       ? d_sum_first[i / 2] + d_sum_second.back() - d_sum_second[i / 2]
                       : d_sum_first[(i + 1) / 2] + d_sum_second.back() - d_sum_second[i / 2]);
    }
    cout << m;
  }
}
