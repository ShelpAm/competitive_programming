...
auto solve_case()
{
    ...
    disjoint_set ds(m);
    for (u64 i = 1; i != m; ++i) {
        auto const& [p, q, c] = segs[i];
        for (u64 j = i - 1; j != -1; --j) {
            auto const& [l, r, w] = segs[j];
            if (p <= r && r <= q) {
                ds.unite(i, j);
            }
        }
    ...
    std::map<u64, std::vector<triplei>> mp;
    for (u64 i = 0; i != m; ++i) {
        mp[ds.find(i)].push_back(segs[i]);
    }

    i64 tmp = 0;
    for (auto& [root, segs]: mp) {
        auto m = segs.size();
        vi f(m + 2);
        segs.insert(segs.begin(), {-1, -1, 0});
        segs.push_back({inf<i64>, inf<i64>, 0});
        for (u64 i = 1; i != m + 2; ++i) {
            auto const& [p, q, c] = segs[i];
            i64 mx = 0;
            for (u64 j = i - 1; j != -1; --j) {
