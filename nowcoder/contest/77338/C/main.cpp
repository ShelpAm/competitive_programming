...
template<typename info_type, typename tag_type> class segment_tree {
  public:
    ...
    [[deprecated("unimplemented")]] void modify_range(u64 a, u64 b, u64 x) { modify_range_impl(1, 0, n_, a, b, x); }
...
template<typename info_type, typename tag_type> segment_tree<info_type, tag_type>::segment_tree(u64 const n)
    : n_(n), info_(4ULL << msb(n_), info_type{0, 0, 0, 0}), tags_(4ULL << msb(n_))
{}
template<typename info_type, typename tag_type> segment_tree<info_type, tag_type>::segment_tree(std::vector<i64> const& raw)
...
template<typename info_type, typename tag_type> void segment_tree<info_type, tag_type>::push(u64 p, u64 l, u64 r)
{
    if (r - l == 1) {
        return;
    }
...
template<typename info_type, typename tag_type> void segment_tree<info_type, tag_type>::apply(u64 p, tag_type const& tag)
{
    info_[p].apply(tag);
    tags_[p].apply(tag);
}
template<typename info_type, typename tag_type>
void segment_tree<info_type, tag_type>::modify_range_impl([[maybe_unused]] u64 p, u64 l, u64 r, u64 a, u64 b, [[maybe_unused]] i64 x)
{
...
template<typename info_type, typename tag_type>
void segment_tree<info_type, tag_type>::apply_range_impl(u64 p, u64 l, u64 r, u64 a, u64 b, tag_type const& tag)
{
    // ranges not intersecting
    if (b <= l || r <= a) {
        return;
...
struct info {
    ...
    {
        return info{
            .min = ::min(this->min, rhs.min), .max = ::max(this->max, rhs.max), .sum = this->sum + rhs.sum, .act = this->act + rhs.act};
    }
...
auto solve_case()
{
    ...
    segtree st(n + 1);
    for (u64 i = 1; i != n + 1; ++i) {
        st.apply_range(i, i, tag{a[i - 1]});
    }
    for (u64 i = 0; i != t; ++i) {
        u64 l, r;
        cin >> l >> r;
        auto mx = st.query_range(l, r).max;
        cout << mx << ' ' << (mx >= q ? "YES" : "NO") << '\n';
    }
}
