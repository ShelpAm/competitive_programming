...
auto solve_case()
{
  ...
  u64 ans{};
  map<u64, unordered_map<char, u64>> roots_color_cnt;
  for (u64 i{1}; i != n + 1; ++i) {
    ++roots_color_cnt[ds.find(i)][s[i]];
  }

  for (auto const& [root, color_cnt]: roots_color_cnt) {
    if (ds.size()[root] == 1) {
      continue;
    }
    if (color_cnt.size() == 1) {
      ans += 3 * (color_cnt.begin()->second - 1);
    }
    else {
      u64 mn{inf<u64>}, mx{};
      for (auto const [color, cnt]: color_cnt) {
        check_min(mn, cnt);
        check_max(mx, cnt);
      }
      ans += 3 * mn + mx;
