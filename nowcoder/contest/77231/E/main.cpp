...
auto solve_case()
{
    ...
    std::vector<pii> dirs{{-1, 0}, {0, -1}, {1, 0}, {0, 1}};
    vvvu f(n, vvu(m, vu(4, inf<u64>)));
    priority_queue<quadraticu, std::vector<quadraticu>, greater<>> q;
    for (u64 i = 0; i != 4; ++i) {
        f[sx][sy][i] = 0;
        q.push({0, sx, sy, i});
    }
    vvvb vis(n, vvb(m, vb(4)));
    while (!q.empty()) {
        auto [dis, i, j, dir] = q.top();
        q.pop();
        ...
        if (a[i][j] == '*') {
            for (i64 t = 0; t != 4; ++t) {
                if (abs(t - static_cast<i64>(dir)) == 2) {
                    continue;
                }
                auto neox = i + dirs[t].first, neoy = j + dirs[t].second;
                if (neox >= n || neoy >= m || a[neox][neoy] == '#') {
                    continue;
                }
                if (dis + 1 < f[neox][neoy][t]) {
                    f[neox][neoy][t] = dis + 1;
                    q.push({dis + 1, neox, neoy, t});
                }
            }
        }
        else {
            auto neox = i + dirs[dir].first, neoy = j + dirs[dir].second;
            if (neox >= n || neoy >= m || a[neox][neoy] == '#') {
                continue;
            }
            if (dis + 1 < f[neox][neoy][dir]) {
                f[neox][neoy][dir] = dis + 1;
                q.push({dis + 1, neox, neoy, dir});
            }
        }
    }
    u64 dis = inf<u64>;
    for (u64 i = 0; i != 4; ++i) {
        check_min(dis, f[tx][ty][i]);
    }
    if (dis == inf<u64>) {
        cout << -1 << '\n';
    }
