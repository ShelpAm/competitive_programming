...
std::vector<pii> const dirs{{-1, 0}, {0, -1}, {0, 1}, {1, 0}};
auto solve_case()
{
    u64 n, m;
    cin >> n >> m;
    ...
    auto all_is = [&](auto const& neo, char k) {
        for (u64 i = 0; i != n; ++i) {
            for (u64 j = 0; j != m; ++j) {
                if (neo[i][j] != k) {
                    return false;
    ...
    auto [bx, by] = blue;
    for (auto [dx, dy]: dirs) {
        if (bx + dx < n && by + dy < m && c[bx + dx][by + dy] == '#') {
            auto neo = c;
            vvb vis(n, vb(m));
            auto dfs = [&](auto self, u64 x, u64 y) -> void {
                for (auto [di, dj]: dirs) {
                    if (x + di < n && y + dj < m) {
                        neo[x + di][y + dj] = '#';
                    }
                ...
                for (auto [dx, dy]: dirs) {
                    ...
                    if (neox < n && neoy < m && c[neox][neoy] == '#' && !vis[neox][neoy]) {
                        vis[neox][neoy] = true;
                        self(self, neox, neoy);
                    }
                }
