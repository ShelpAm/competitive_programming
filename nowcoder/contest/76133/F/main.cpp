...
auto solve_case()
{
    ...
    ans = (ans + pow2_x - 1) % mod; // all is 1
    debug("all is 1", pow2_x - 1);
    for (u64 i = 0; i <= cnt[2]; ++i) {
        for (u64 j = 0; j <= cnt[3]; ++j) {
            if (i == 0 && j == 0) {
                continue;
            }
            ans = (ans
                   + pow2_x * c(cnt[2], i) % p * c(cnt[3], j) % p * ((i + 1) % p * (j + 1) % p) % p)
                  % mod;
            debug("i, j, res", vu{i, j,
                                  pow2_x * c(cnt[2], i) % p * c(cnt[3], j) % p
                                      * ((i + 1) % p * (j + 1) % p) % p});
        }
    }
    cout << ans;
