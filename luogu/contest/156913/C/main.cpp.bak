...
{
  constexpr impl::arr1<pii, 4> dirs{pii{-1, 0}, {0, -1}, {1, 0}, {0, 1}};
  u64 n, m;
  cin >> n >> m;
  impl::vec2<char> v(n, m);
  cin >> v;
  impl::vec1<puu> src, dest;
  for (u64 i{}; i != n; ++i) {
    for (u64 j{}; j != m; ++j) {
      if (v[i][j] == 'A') {
  ...
  auto no_collision{[&](auto const off) {
    auto const [ox, oy]{off};
    for (auto const& [x, y]: src) {
      if (x + ox >= n || y + oy >= m || v[x + ox][y + oy] == '#') {
        return false;
      }
    }
    return true;
  ...
  u64 step{inf<u64>};
  while (!q.empty()) {
    auto const [ox, oy, dis]{q.front()};
    q.pop();

    if (ox == dest_off.first && oy == dest_off.second) {
      step = dis;
      break;
    }

    for (auto const [dx, dy]: dirs) {
      if (pii const next{ox + dx, oy + dy};
          !vis.contains(next) && no_collision(next)) {
        vis.emplace(next);
        q.emplace(next.first, next.second, dis + 1);
      }
    }
  }
  if (step == inf<u64>) {
