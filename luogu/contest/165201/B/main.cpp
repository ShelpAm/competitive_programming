...
auto solve_case()
{
    ...
    vvc a(n, vc(m));
    cin >> a;
    std::vector<std::vector<puu>> const cards{
        {{0, 0}, {0, 2}, {1, 1}, {2, 0}, {2, 2}},
        {{0, 1}, {1, 0}, {1, 1}, {1, 2}, {2, 1}},
        {{0, 1}, {1, 0}, {1, 2}, {2, 1}},
        {{0, 0}, {0, 1}, {1, 0}, {1, 1}},
    };
    auto can_place = [&](auto const& card, u64 x, u64 y, char avail) {
        for (auto const [i, j]: card) {
            if (x + i >= n || y + j >= m || a[x + i][y + j] != avail) {
                return false;
            }
        }
        return true;
    };
    auto place = [&](auto const& card, u64 x, u64 y, char ch) {
        for (auto const [i, j]: card) {
            a[x + i][y + j] = ch;
        }
    };
        ...
        for (u64 x = 0; x != n; ++x) {
            for (u64 y = 0; y != m; ++y) {
                for (u64 i = 0; i != cards.size(); ++i) {
                    if (!used[i] && can_place(cards[i], x, y, '.')) {
                        used[i] = true;
                        place(cards[i], x, y, '#');
                        cur += cards[i].size();
                        self(self);
