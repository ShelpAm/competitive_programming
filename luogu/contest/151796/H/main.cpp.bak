...
bool equal(char const* s, char const* t, int len)
{
  for (int i = 0; i != len; ++i) {
    if (s[i] != t[i]) {
      return false;
...
int find(char* s, char* t, int p)
{
  ...
  for (int i = p; i + t_len < s_len; ++i) {
    char c = s[i + t_len];
    s[i + t_len] = '\0';
    if (equal(s + i, t, t_len)) {
      return i;
    }
    s[i + t_len] = c;
  }
...
static inline auto solve_case()
{
  ...
  size_t cnt{};
  auto p{find(s.data(), a.data(), 0)};
  while (p != -1) {
    cout << "p:" << p << '\n';
    auto q{find(s.data(), b.data(), p - 1)};
    while (q != -1) {
      cout << "q:" << q << '\n';
      ++cnt;
      q = find(s.data(), b.data(), q + b.size() - 1);
    }
    p = find(s.data(), a.data(), p + a.size() - 1);
  }
  return cnt;
}
