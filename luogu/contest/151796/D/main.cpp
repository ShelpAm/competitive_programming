...
static inline auto solve_case()
{
  ...
  size_t p;
  cin >> a >> p >> x >> y;
  auto f{[&](size_t const x, size_t const y) {
    return (a[0] * x * x * x + a[1] * y * y * y + a[2] * x * x * y + a[3] * x * y * y + a[4] * x * y
            + a[5] * x + a[6] * y)
           % p;
  }};
  size_t ans{};
  for (size_t i{x.front()}; i != x.back() + 1; ++i) {
    for (size_t j{y.front()}; j != y.back() + 1; ++j) {
      check_max(ans, f(i, j));
    }
  }
  return ans;
