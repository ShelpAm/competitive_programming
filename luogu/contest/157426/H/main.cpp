...
static auto solve_case()
{
  ...
  auto cycled{[&]() {
    ...
    return std::ranges::any_of(in, [](auto const& t) { return t != 0; });
  }
  ...
  for (auto const i: iota(u64{}, n)) {
    if (conn[i]) {
      costs.emplace_back(a[i]);
    }
    else {
      non.emplace_back(a[i]);
    }
  }
  sort(costs);
  sort(non);
  return min({costs[0] + costs[1],
              non.size() >= 1 ? (costs[0] + non[0]) * 2 : inf<u64>,
              non.size() >= 2 ? (non[0] + non[1]) * 2 : inf<u64>});
}
