...
void solve_case()
{
  ...
  std::vector<vec2<high_accuracy_number>> dp(n,
                                             vec2<high_accuracy_number>(m, m));
  // dp[i][j][k] row: i, [j, j+k]
  for (size_t i{}; i != n; ++i) {
    for (size_t j{}; j != m; ++j) {
      dp[i][j][j] = mat[i][j] * 2;
    }
  }
  for (size_t len{1}; len != m + 1; ++len) {
    for (size_t i{}; i != n; ++i) {
      for (size_t j{}; j + len != m; ++j) {
        dp[i][j][j + len]
            = high_accuracy_number(2)
              * std::ranges::max(dp[i][j + 1][j + len] + mat[i][j],
                                 dp[i][j][j + len - 1] + mat[i][j + len],
                                 std::less<>());
        // std::cout << "Processing (i, j, k): " << i << ' ' << j << ' ' << j +
        // len
        //           << '\n'
        //           << dp[i][j][j + len] << '\n';
      }
