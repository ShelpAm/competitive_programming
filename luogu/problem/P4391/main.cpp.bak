...
{
  std::array<bool, size_t(1e6) + 3> not_prime{};
  ...
  // Checks the minimum length of the element string.
  auto check{[&](size_t const len) { return s.substr(len, len) == s.substr(0, len); }};
  // size_t l{1}, r(n);
  size_t cnt{1};
  for (auto const p: primes) {
    while (n != 0 && n % p == 0 && check(n / p)) {
      n /= p;
      cnt *= p;
    }
  }
