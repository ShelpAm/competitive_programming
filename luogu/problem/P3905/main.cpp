...
static inline auto solve_case()
{
  size_t n, m;
  cin >> n >> m;
  auto const adj{read_graph(n, m, true)};
  size_t d;
  cin >> d;
  vec2<bool> broken(n, n);
  for (size_t i{}; i != d; ++i) {
    size_t u, v;
    cin >> u >> v;
    --u, --v;
  ...
  // vec2uz f(n, n, big<size_t>);
  // for (size_t u{}; u != n; ++u) {
  //   f[u][u] = 0;
  //   for (auto const& [d, v]: adj[u]) {
  //     f[u][v] = broken[u][v] ? d : 0;
  //   }
  // }
  // for (size_t k{}; k != n; ++k) {
  //   for (size_t i{}; i != n; ++i) {
  ...
  // vec1<size_t> dis(n, big<size_t>);
  // priority_queue<puz, vec1<puz>, greater<>> q;
  // q.emplace(0, a);
  // while (!q.empty()) {
  //   auto const [_, u]{q.top()};
  //   q.pop();
  //   dis[u] = _;
  //   if (u == b) {
  //     break;
  ...
  //   for (auto const& [d, v]: adj[u]) {
  //     if (dis[v] == big<size_t>) {
  //       q.emplace(dis[u] + (broken[u][v] ? d : 0), v);
  //     }
  //   }
  // }
  // return dis[b];
