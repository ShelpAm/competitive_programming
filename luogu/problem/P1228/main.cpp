...
size_t x, y;
bool found{};
void divide_and_conquer(size_t const k, puz const& p)
{
  auto const w{1UZ << (k - 2)};
  auto const [i, j]{p};
  if (found) {
    divide_and_conquer(k - 1, {i - w, j - w});
    divide_and_conquer(k - 1, {i - w, j + w});
    divide_and_conquer(k - 1, {i + w, j - w});
    divide_and_conquer(k - 1, {i + w, j + w});
  }
...
static inline auto solve_case()
{
  ...
  cin >> k;
  cin >> x >> y;
  divide_and_conquer(k, {1UZ << (k - 1), 1UZ << (k - 1)});
}
