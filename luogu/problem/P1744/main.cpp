...
{
  prime_fileter_result res(upper_bound + 1);
  auto&& primes{res.primes};
  auto&& not_prime{res.not_prime};
...
struct dijkstra_result {
  dijkstra_result(size_t const size): dis(size, inf<i64>), prev(size){};
  impl::vec1<double> dis;
  vi prev;
};
template<typename T> auto dijkstra(impl::vec2<T> const& adjacent, size_t const source)
{
  dijkstra_result res{adjacent.size()};
  auto&& dis{res.dis};
  auto&& prev{res.prev};
  dis[source] = 0;

  priority_queue<T, impl::vec1<T>, greater<>> q;
...
static inline auto solve_case()
{
  ...
  i64 m;
  cin >> m;
  impl::vec2<pair<double, i64>> adj(n, 0);
  for (i64 i{}; i != m; ++i) {
    i64 u, v;
    cin >> u >> v;
    --u, --v;
    auto d{std::sqrt((x[u] - x[v]) * (x[u] - x[v]) + (y[u] - y[v]) * (y[u] - y[v]))};
  ...
  i64 s, t;
  cin >> s >> t;
  auto [dis, _]{graph::dijkstra(adj, s - 1)};
  // print(dis);
  cout << std::setprecision(2) << std::fixed << dis[t - 1];
}
