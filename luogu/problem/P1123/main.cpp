...
auto solve_case()
{
    ...
    vvb vis(n, vb(m));
    auto ok = [&](u64 i, u64 j) {
        for (i64 oi = -1; oi <= 0; ++oi) {
            for (i64 oj = -1; oj <= 1; ++oj) {
                if (i + oi < n && j + oj < m && vis[i + oi][j + oj]) {
                    return false;
                }
            }
        }
    ...
    auto dfs = [&](auto self, u64 i, u64 j) -> u64 {
        if (i >= n) {
            return 0;
        }
        ...
        if (ok(i, j)) {
            vis[i][j] = true;
            would = a[i][j] + self(self, ni, nj);
            vis[i][j] = false;
        }
        dont = self(self, ni, nj);
        return max(dont, would);
    };
    return dfs(dfs, 0, 0);
}
