...
static inline auto solve_case()
{
  ...
  cin >> (a | drop(1));
  vec1<size_t> fa(n + 1);
  vec1<pii> height_of_depth(n + 1); // pii : first: max_height, second: its idx
  height_of_depth[0] = {big, 0};

  auto check{
      [&](size_t const mid, auto const my_value) { return my_value < height_of_depth[mid].first; }};

  auto max_sum_of_depth{0UZ};
  for (auto i{1UZ}; i != n + 1; ++i) {
    // l is the max depth we can find in the sequence.
    auto l{0UZ}, r{n};
    while (l < r) {
      auto mid{(l + r + 1) >> 1};
      if (check(mid, a[i])) {
    ...
    check_max(height_of_depth[l + 1], std::make_pair(a[i], i));
    fa[i] = height_of_depth[l].second;
    max_sum_of_depth += l + 1;
  }
  cout << max_sum_of_depth << endl;
  print(fa | drop(1));
}
