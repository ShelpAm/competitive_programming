...
auto solve_case()
{
  size_t n, h;
  std::cin >> n >> h;
  std::vector<size_t> dp(h + 1 + m, big);
  dp[0] = 0;
  for (size_t i{}; i != n; ++i) {
    size_t p, c;
    std::cin >> p >> c;
    for (size_t j{p}; j <= h + m; ++j) {
      dp[j] = std::ranges::min(dp[j], dp[j - p] + c);
    }
  }
  std::cout << std::ranges::min(dp | std::ranges::views::reverse | std::ranges::views::take(m + 1));
}
