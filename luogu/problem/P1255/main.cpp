...
public:
  ...
  void emplace(size_t const e) { push(e); }
private:
  ...
  arr1<size_t, 100000> data_{};
  size_t head_{};
  size_t rear_{};
};
// breadth-first search
size_t bfs(size_t const n)
{
  size_t ans{};
  stack<size_t> q;
  q.emplace(0);
...
size_t dfs(size_t const n)
{
  ...
  return ans[n] = dfs(n - 1) + dfs(n - 2);
}
size_t dp(size_t const n)
{
  vec1<size_t> f(n + 1);
  f[0] = f[1] = 1;
