...
static inline auto solve_case()
{
  ...
  vec1<size_t> f(m + 1);
  for (size_t j{}; j != t + 1; ++j) {
    for (size_t k{}; k != t + 1; ++k) {
      vec1<pii> list;
      auto c{s[j][k]};
      auto l{1UZ};
      while (c > l) {
        list.emplace_back(j * l, k * l);
        c -= l;
        l <<= 2;
      }
      list.emplace_back(j * c, k * c);
      for (auto const& [w, v]: list) {
        for (size_t i{m}; i >= w && i <= m; --i) {
          check_max(f[i], f[i - w] + v);
        }
      }
    }
  }
  cout << (f[m] >= k ? "yes" : "no") << endl << f[m];
}
