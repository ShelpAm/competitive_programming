...
using vec2_placeholder = std::vector<std::vector<value_type>>;
template <typename value_type, size_t size>
using arr1 = ::std::array<value_type, size>;
template <size_t i, size_t j, typename value_type>
using arr2 = std::array<std::array<value_type, j>, i>;
template <typename value_type> using vec1 = ::std::vector<value_type>;
template <typename value_type>
class vec2 : public vec2_placeholder<value_type> {
public:
...
template <typename T>
concept string = std::same_as<string, remove_cvref_t<T>>;
template <typename T>
concept string_view = std::same_as<string_view, remove_cvref_t<T>>;
template <typename T>
...
public:
  ...
  [[nodiscard]] std::vector<puu> const& edges_of(u64 u) const
  {
    return adjacent[u];
  }
...
constexpr auto lsb(i64 const i) { return i & (-i); }
// i mustn't be 0
...
//     }
//     void apply_range(u64 a, u64 b, tag_type const& tag) { apply_range_impl(1,
//     0, n_, a, b, tag); } info_type query_range(u64 a, u64 b) { return
//     query_range_impl(1, 0, n_, a, b); }
//   private:
...
//     void apply_range_impl(u64 p, u64 l, u64 r, u64 a, u64 b, tag_type const&
//     tag); info_type query_range_impl(u64 p, u64 l, u64 r, u64 a, u64 b);
//
//     [[deprecated]] void add_on_segment_impl(u64 a, u64 b, i64 d, u64 l, u64
//     r, u64 x)
//     {
//         if (a <= l && r <= b) {
//             info_[x].tag += d;
...
//     : n_(n), info_(4ULL << msb(n_), info_type{0, 0, 0, 0}), tags_(4ULL <<
//     msb(n_))
// {}
// template<typename info_type, typename tag_type>
...
//         // if we don't change the structure of the tree, we won't save info
//         of internal
...
//     build(build, 1, n_, 1);
// }
// template<typename info_type, typename tag_type> void segment_tree<info_type,
// tag_type>::pull(u64 p)
// {
...
// }
// template<typename info_type, typename tag_type>
// void segment_tree<info_type, tag_type>::modify_range_impl([[maybe_unused]]
// u64 p, u64 l, u64 r,
//                                                           u64 a, u64 b,
//                                                           [[maybe_unused]]
//                                                           i64 x)
...
//     push(l, r, p);
//     u64 const m = (l + r) / 2;
//     return query_range_impl(p * 2, l, m, a, b) + query_range_impl(p * 2 + 1,
//     m, r, a, b);
// }
//
// struct tag {
...
auto solve_case()
{
  u64 n, m, s;
  cin >> n >> m >> s;
  return dijkstra(read_graph(n, m, false, true, true), s - 1).distance;
}
