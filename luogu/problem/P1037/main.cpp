...
public:
  ...
  high_accuracy_number operator*(high_accuracy_number const& other) const;
  high_accuracy_number operator/(high_accuracy_number const& other) const;
  auto operator<=>(high_accuracy_number const&) const = default;

  [[nodiscard]] std::string const& number() const { return number_; }
  [[nodiscard]] std::string& number() { return number_; }
private:
  std::string number_;
};
...
{
  for (size_t i{}; i != -1UZ; ++i) {
    if (high_accuracy_number{a} * high_accuracy_number{i} > high_accuracy_number{b}) {
      return i - 1;
    }
  }
...
high_accuracy_number high_accuracy_divide(std::string const& a, std::string const& b)
{
  ...
  std::string s;
  high_accuracy_number sub{a.substr(0, b.size())};
  auto j{b.size()};
  for (;;) {
    while (true) {
      if (j == a.size()) {
    ...
    auto t{high_accuracy_divide(sub.number(), b, 0)};
...
static inline auto solve_case()
{
  ...
  vec1uz cnt(10, 1);
  for (auto const i: iota(0UZ, 10UZ)) {
    unordered_set<size_t> numbers{i};
    queue<size_t> q;
    q.emplace(i);
    while (!q.empty()) {
      for (auto const v: adj[q.front()]) {
  ...
  // TODO: switch to high accuracy number
  high_accuracy_number res{1};
  for (auto const c: n) {
    res = res * cnt[c - '0'];
  }
  return res;
