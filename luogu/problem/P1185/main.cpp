...
void build_tree(auto&& buffer, auto&& tree, size_t const root, pii const root_pos, size_t const h)
{
  auto [x, y]{root_pos};
  debug("root_pos", root_pos);
  debug("h", h);
  if (y >= buffer.size()) {
    return;
  }
  ...
  if (tree[root * 2]) {
    build_tree(buffer, tree, root * 2, {x - h, y + h}, (h + 1) >> 1);
  }
  if (tree[root * 2 + 1]) {
    build_tree(buffer, tree, root * 2 + 1, {x + h, y + h}, (h + 1) >> 1);
  }
}
static inline auto solve_case()
{
  size_t m, n;
  cin >> m >> n;
  auto const t{3 * (1UZ << (m - 2))};
  debug("t", t);
  vec1<string> buffer(m >= 2 ? t : 1, string(t << 1, ' '));
  vec1<bool> tree((1UZ << m) + 1, true);
  for (size_t i{}; i != n; ++i) {
    size_t y, x;
    cin >> y >> x;
    tree[(1UZ << (y - 1)) + x - 1] = false;
  }
  build_tree(buffer, tree, 1, {t, 0}, (t + 1) >> 1);
