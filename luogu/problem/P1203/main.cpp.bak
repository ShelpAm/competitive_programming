...
auto solve_case()
{
    ...
    string s;
    cin >> s;
    vvu left(n, vu(2)), right(n, vu(2));

    u64 p = -1;
    for (u64 i = 0; i != n; ++i) {
        if (s[i] != 'w' && s[i] != s[(i + 1) % n]) {
            p = i;
            break;
        }
    }
    ...
    for (u64 i = 1; i != n; ++i) {
        if (s[i] == 'w') {
            for (auto t: {0, 1}) {
                left[(p + i) % n][t] = left[(p + i - 1 + n) % n][t] + 1;
            }
        }
        else if (s[i] == 'r') {
            left[(p + i) % n][0] = left[(p + i - 1 + n) % n][0] + 1;
            left[(p + i) % n][1] = 0;
        }
        else if (s[i] == 'b') {
            left[(p + i) % n][0] = 0;
            left[(p + i) % n][1] = left[(p + i - 1) % n][1] + 1;
        }
    }
    for (i64 i = -1; i != -static_cast<i64>(n); --i) {
        if (s[i] == 'w') {
            for (auto t: {0, 1}) {
                left[(p + i + n) % n][t] = left[(p + i - 1 + n) % n][t] + 1;
            }
        }
        else if (s[i] == 'r') {
            left[(p + i + n) % n][0] = left[(p + i - 1 + n) % n][0] + 1;
            left[(p + i + n) % n][1] = 0;
        }
        else if (s[i] == 'b') {
            left[(p + i + n) % n][0] = 0;
            left[(p + i + n) % n][1] = left[(p + i - 1 + n) % n][1] + 1;
        }
    }

    u64 ans = 0;
    for (u64 i = 0; i != n; ++i) {
        check_max(ans, max(left[i]) + max(right[i]) + 1);
    }
    return ans;
}
