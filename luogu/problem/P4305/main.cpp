...
static inline auto solve_case()
{
  ...
  // Discretion
  vec1<long long> nums(n);
  cin >> nums;
  auto copy{nums};
  sort(nums);
  auto const len{std::unique(nums.begin(), nums.end()) - nums.begin()};
  vec1<bool> visited(len + 1);

  debug("len", len);
  auto pos{[&](auto const i) {
    return std::ranges::lower_bound(nums.begin(), nums.begin() + len, i) - nums.begin();
  }};
  debug("nums", nums);
