...
static inline auto solve_case()
{
  ...
  cin >> n >> m >> x;
  --x;
  impl::vec2<pii> adj(n, 0), adj_rev(n, 0);
  for (i64 i{}; i != m; ++i) {
    i64 u, v, w;
    cin >> u >> v >> w;
    --u, --v;
    adj[u].emplace_back(w, v);
    adj_rev[v].emplace_back(w, u);
  }
  auto [dis, prev1]{dijkstra(adj, x)};
  auto [dis_rev, prev2]{dijkstra(adj_rev, x)};
  i64 ans{};
  for (i64 i{}; i != n; ++i) {
    check_max(ans, dis[i] + dis_rev[i]);
  }
  return ans;
}
