...
long long n, m;
arr1<pii, 4> const dirs{pii{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
bool dfs(auto const& v, auto&& visited, pii const global_pos, pii const inline_pos)
{
  auto&& visit_info{visited[inline_pos.first][inline_pos.second]};
  if (visit_info[0]) {
    return visit_info[1] != global_pos.first || visit_info[2] != global_pos.second;
  }
  // 记录点
  visit_info = {true, global_pos.first, global_pos.second};
  for (auto [dy, dx]: dirs) {
    auto i{(inline_pos.first + dy + n) % n}, j{(inline_pos.second + dx + m) % m};
    // i，j是下一点的坐标；若没有路，则跳过
    if (v[i][j] != '#'
        && dfs(v, visited, {global_pos.first + dy, global_pos.second + dx}, {i, j})) {
      return true;
    }
  }
  return false;
...
static inline auto solve_case()
{
  while (cin >> n >> m) {
    ...
    vec2<arr1<long long, 3>> visited(n, m);
    if (dfs(v, visited, s, s)) {
      cout << "Yes\n";
    }
    else {
      cout << "No\n";
