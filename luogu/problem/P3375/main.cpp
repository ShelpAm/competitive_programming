...
class kmp {
  public:
    kmp(string_view pattern): pattern_(pattern), T(pattern.size())
    {
        assert(!pattern.empty());
        // std::vector<std::uint_fast64_t> last(CHAR_MAX, -1);
        T[0] = -1;
        i64 cnd = 0;
        for (u64 i = 1; i != pattern.size(); ++i) {
        ...
        // partial_match_table_[]
    }
    std::vector<std::uint_fast64_t> match(string_view text)
    {
        i64 text_len = static_cast<i64>(text.size());
        i64 pattern_len = static_cast<i64>(pattern_.size());
        std::vector<std::uint_fast64_t> matches;
        i64 i = 0, j = 0;
        while (i != text_len) {
    ...
    [[nodiscard]] std::vector<std::int_fast64_t> const& next() const { return T; }
  private:
    string pattern_;
    std::vector<std::int_fast64_t> T;
};
auto solve_case()
{
    string s, t;
    ...
    kmp kmp(t);
    for (auto e: kmp.match(s)) {
        cout << e + 1 << '\n';
    }
    cout << 0 << ' ';
    print(kmp.next() | drop(1));
}
