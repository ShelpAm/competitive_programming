...
size_t max_width{}, max_depth{};
void dfs(auto&& g, auto&& depth, auto&& width, size_t const curr, size_t const prev)
{
  // cout << "Curr: " << curr << ", " << "Prev: " << prev << "\n";
  check_max(max_width, ++width[depth[curr]]);
  for (auto&& next: g[curr]) {
    if (next.first != prev) {
      check_max(max_depth, depth[next.first] = depth[curr] + 1);
      dfs(g, depth, width, next.first, curr);
    }
    else {
      ++next.second;
    }
...
size_t distance(auto&& g, size_t const curr, size_t const prev, size_t const dest)
{
  if (curr == dest) {
    return 0;
  }
  ...
  for (auto const next: g[curr]) {
    if (next.first != prev) {
      check_min(res, distance(g, next.first, curr, dest) + next.second);
    }
  }
  return res;
}
static inline auto solve_case()
{
  ...
  vec1<size_t> depth(n + 1), width(n + 1);
  dfs(g, depth, width, 1, 0);
  debug("depth", depth);
  debug("width", width);
  size_t x, y;
  cin >> x >> y;
  cout << max_depth + 1 << endl << max_width << endl << distance(g, x, 0, y);
}
