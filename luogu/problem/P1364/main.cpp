...
void prework(auto const& g, auto const& w, auto&& depth, auto&& size, auto&& f, size_t const curr)
{
  for (auto const next: g[curr]) {
    depth[next] = depth[curr] + 1;
    prework(g, w, depth, size, f, next);
    size[curr] += size[next];
  }
  f[0] += depth[curr] * w[curr];
}
void dp(auto const& g, auto const& depth, auto const& size, auto&& f, size_t const curr, auto&& ans)
{
  for (auto const next: g[curr]) {
    f[next] = f[curr] + size[0] - 2 * size[next];
    check_min(ans, f[next]);
    dp(g, depth, size, f, next, ans);
  }
}
static inline auto solve_case()
{
  ...
  auto size{w};
  prework(g, w, depth, size, f, 0);
  debug("depth", depth);
  debug("size", size);
  debug("f[0]", f[0]);
  auto ans{f[0]};
  dp(g, depth, size, f, 0, ans);
  debug("f", f);
  cout << ans;
}
