...
static inline auto solve_case()
{
  size_t n, w;
  cin >> n >> w;
  std::map<size_t, vec1<size_t>> items_for_money;
  for (size_t i{}; i != n; ++i) {
    size_t v, p;
    cin >> v >> p;
    items_for_money[v].emplace_back(p);
  }
  for (auto&& [_, e]: items_for_money) {
    sort(e);
  }
  size_t m{};
  for (auto i{items_for_money.begin()}; i != items_for_money.end(); ++i) {
    vec1<decltype(items_for_money)::const_iterator> its;
    auto j{i};
    while (j->first - i->first <= 3) {
      its.emplace_back(j);
      ++j;
    }
    vec1<size_t> items;
    for (auto const& it: its) {
      std::ranges::merge(items, it->second, items.begin());
    }
  }
}
