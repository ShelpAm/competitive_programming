...
auto floyd(vvi& f)
{
  auto const n{static_cast<i64>(f.size())};
...
static inline auto solve_case()
{
  i64 n, q;
  cin >> n >> q;
  vvi d(n, vi(n)), l(n, vi(n));
  cin >> d >> l;

  auto check{[&](i64 const mi) {
    auto neo_d{d};
    vi decrement(n);
    for (i64 i{}; i != n; ++i) {
      decrement[i] = mi / n + (mi % n > i ? 1 : 0);
    }
    for (i64 i{}; i != n; ++i) {
      for (i64 j{i + 1}; j != n; ++j) {
        neo_d[i][j] = neo_d[j][i]
            = max(l[i][j], d[i][j] - decrement[i] - decrement[j]);
      }
    }
    floyd(neo_d);
    i64 p{};
