...
bool has_cycle(auto&& g, size_t const curr, size_t const visited)
{
  // debug("curr", curr);
  ...
  for (auto const next: g[curr]) {
    ...
    res |= has_cycle(g, next, visited | 1 << curr);
    if (res) {
      return true;
    }
  }
...
size_t max_len(auto&& g, size_t const curr)
{
  size_t res{};
  if (g[curr].empty()) {
    return 1;
  ...
  for (auto const next: g[curr]) {
    check_max(res, max_len(g, next));
  }
  return res + 1;
}
string len_path(auto&& g, size_t const len, size_t const curr)
{
  if (len == 1) {
    return string{static_cast<char>('A' + curr)};
  }
  for (auto const next: g[curr]) {
    auto trial{len_path(g, len - 1, next)};
    debug("Trial_len", len - 1);
    debug("Trial", trial);
    if (trial.size() == len - 1) {
      return static_cast<char>('A' + curr) + trial;
    }
  }
  return "";
}
// dfs
size_t find(auto&& pa, size_t const x)
{
...
static inline auto solve_case()
{
  ...
  // size_t n, m;
  // cin >> n >> m;
  // // a is b's father means a is greater than b.
  // unordered_map<char, char> fa;
  // unordered_map<char, unordered_set<char>> sons;
  // for (size_t i{}; i != m; ++i) {
  //   char a, b;
  //   cin >> a >> b;
  //   fa.emplace(a, b);
