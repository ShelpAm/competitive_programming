...
auto solve_case()
{
  size_t n;
  cin >> n;
  vec1<std::tuple<size_t, size_t, size_t>> v(n);
  for (auto&& [h, a, c]: v) {
    cin >> h >> a >> c;
  }
  sort(v, [](auto&& lhs, auto&& rhs) { return std::get<1>(lhs) < std::get<1>(rhs); });

  array<bool, static_cast<size_t>(4e5) + 1> dp{};
  dp[0] = true;
  for (auto const& [h, a, c]: v) {
    for (size_t i{a}; i >= h; --i) {
      for (size_t j{}; j * h <= i && j <= c; ++j) {
        dp[i] |= dp[i - j * h];
      }
    }
  }
  for (size_t i{dp.size() - 1}; i != -1UZ; --i) {
    if (dp[i]) {
      cout << i;
      return;
    }
