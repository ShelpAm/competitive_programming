...
auto solve_case()
{
  ...
  sort(a);
  sort(b);
  auto const x{std::unique(a.begin(), a.end()) - a.begin()},
      y{std::unique(b.begin(), b.end()) - b.begin()};

  return n * n - (n - x) * (n - y);

  // unordered_set<u64> row, column;
  // u64 ans{};
  // for (u64 i{}; i != k; ++i) {
  //   u64 u, v;
  //   cin >> u >> v;
