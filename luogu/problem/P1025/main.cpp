...
size_t divide(size_t const n, size_t const k, size_t const last)
{
  static impl::vec1<vec2uz> ans(201, vec2uz(7, 101));
  if (ans[n][k][last] != 0) {
    return ans[n][k][last];
  }
  if (k == 1) {
    return ans[n][k][last] = 1;
  }
  for (auto const part: iota(last, n / k + 1)) {
    ans[n][k][last] += divide(n - part, k - 1, part);
  }
  return ans[n][k][last];
}
