...
auto solve_case()
{
  ...
  auto const t1{sqrt(2 * (h - k) / g)};
  auto const t2{sqrt(2 * h / g)};
  auto const right{v * t1};
  auto const left{v * t2 + l};

  debug("left", left);
  debug("right", right);

  u64 cnt{};
  for (auto const i: iota(i64{}, n)) {
    if (right - delta <= l + s1 - i && l + s1 - i <= left + delta) {
      ++cnt;
    }
  }
  return cnt;
