...
arr2uz<4, 4> vis{};
constexpr auto last{15};
bool ok(size_t const x, size_t const y, size_t const e)
{
  for (size_t i{}; i != 4; ++i) {
    if (vis[x][i] == e) {
      return false;
  ...
  size_t block_x{x / 2}, block_y{y / 2};
  for (size_t i{}; i != 2; ++i) {
    for (size_t j{}; j != 2; ++j) {
      if (vis[block_x * 2 + i][block_y * 2 + j] == e) {
        return false;
      }
    }
  }
...
size_t dfs(size_t const depth)
{
  if (depth == last) {
    return 1;
  }
  size_t res{};
  auto const neo_x{(depth + 1) / 4}, neo_y{(depth + 1) % 4};
  for (auto const i: iota(1UZ, 5UZ)) {
    if (ok(neo_x, neo_y, i)) {
      vis[neo_x][neo_y] = i;
      res += dfs(depth + 1);
      vis[neo_x][neo_y] = 0;
