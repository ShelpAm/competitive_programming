...
auto solve_case()
{
  ...
  std::vector<std::vector<string>> a(n, std::vector<string>(n));
  cin >> a;
  unordered_map<char, u64> mp;
  for (u64 i{1}; i != n; ++i) {
    mp[a.front()[i].front()] = i - 1;
  }

  vu p(10);
  std::iota(p.begin(), p.end(), 0);

  auto eval{[&](string_view s, u64 const base, auto const& p) {
    u64 ans{};
    for (auto const ch: s) {
      ans = ans * base + p[mp[ch]];
    }
    return ans;
  }};

  do {
    u64 possible_min_base{};
    for (auto const& [ch, idx]: mp) {
      check_max(possible_min_base, p[idx] + 1);
    }

    for (auto const base: iota(possible_min_base, u64{11})) {
      auto all_tests_pass{[&]() {
        for (u64 i{1}; i != n; ++i) {
          for (u64 j{i}; j != n; ++j) {
            if (eval(a[i][j], base, p)
                != eval(a.front()[i], base, p) + eval(a[j].front(), base, p)) {
              return false;
            }
          }
        }
