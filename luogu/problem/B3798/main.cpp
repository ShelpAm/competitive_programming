...
struct student_t {
  bool operator<(student_t const& other) const
  {
    if (sum > other.sum) {
      return true;
    }
...
{
  return os << "name: " << student.name << " sum: " << student.sum;
}
auto calc(auto&& marks)
{
  double const average_mark{std::accumulate(marks.begin(), marks.end(), 0LL)
                            / static_cast<double>(marks.size())};
  double res{};
  for (auto const mark: marks) {
    res += std::pow(mark - average_mark, 2);
  }
  return res;
}
size_t insert(auto&& coll, auto const& ele)
{
  if (coll.empty()) {
    coll.push_back(ele);
    return 0;
  ...
  coll.insert(coll.begin() + l, ele);
  return l;
}
auto insertion_sorted(auto const& coll)
{
  std::remove_cvref_t<decltype(coll)> res;
  for (auto const& ele: coll) {
    insert(res, ele);
    // std::cout << "ele(" << ele << ") is inserted to " << pos << '\n';
    // std::cout << "And now, the list is:\n";
    // print(res);
  }
  return res;
}
...
void solve_case()
{
  ...
  std::vector<long long> marks(m);
  for (auto&& student: students) {
    std::cin >> student.name;
    input(marks);
    student.sum = calc(marks);
  }

  // A non-standard approach
  // std::ranges::sort(students, std::less<>());

  // auto results{insertion_sorted(students)};

  // The third way - heap sort
  heap_sort(students);
