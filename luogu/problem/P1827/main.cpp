...
void subtree(string_view pre, string_view in)
{
  if (pre.size() == 0) {
    return;
  }
  auto root{pre[0]};
  auto delim_pos{in.find(root)};
  subtree(pre.substr(1, delim_pos), in.substr(0, delim_pos));
  subtree(pre.substr(delim_pos + 1), in.substr(delim_pos + 1));
  cout << root;
}
