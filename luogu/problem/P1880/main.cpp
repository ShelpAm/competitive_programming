...
static inline auto solve_case()
{
  ...
  vec2uz fmin(2 * n + 1, 2 * n + 1, big);
  vec2uz fmax(2 * n + 1, 2 * n + 1);
  for (size_t len{1}; len <= n; ++len) {
    for (size_t l{1}, r; (r = l + len - 1) != 2 * n + 1; ++l) {
      if (len == 1) {
        fmin[l][r] = fmax[l][r] = 0;
      }
      for (size_t k{l}; k != r; ++k) {
        check_min(fmin[l][r], fmin[l][k] + fmin[k + 1][r] + s[r] - s[l - 1]);
        check_max(fmax[l][r], fmax[l][k] + fmax[k + 1][r] + s[r] - s[l - 1]);
      }
    }
  }
  size_t min{big}, max{};
  for (size_t i{1}; i != n + 2; ++i) {
    check_min(min, fmin[i][i + n - 1]);
    check_max(max, fmax[i][i + n - 1]);
  }
  cout << min << endl << max;
}
