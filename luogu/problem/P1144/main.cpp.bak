...
constexpr auto mm{100003};
static inline auto solve_case()
{
  size_t n, m;
  cin >> n >> m;
  vec1<unordered_map<size_t, size_t>> adj(n);
  for (size_t i{}; i != m; ++i) {
    size_t u, v;
    cin >> u >> v;
    --u, --v;
  ...
  vec1<size_t> cnt(n), dis(n, big<size_t>);
  cnt[0] = 1;
  dis[0] = 0;
  queue<size_t> q;
  q.emplace(0);
  while (!q.empty()) {
    ...
    q.pop();
    for (auto const [v, num]: adj[u]) {
      if (auto neo_dis{dis[u] + 1}; neo_dis <= dis[v]) {
        if (neo_dis < dis[v]) {
          dis[v] = neo_dis;
          cnt[v] = 0;
          q.emplace(v);
        }
        cnt[v] = (cnt[v] + cnt[u] * num) % mm;
      }
    }
  }
  for (size_t i{}; i != n; ++i) {
