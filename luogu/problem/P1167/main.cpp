...
auto&& operator>>(auto&& is, std::tm& t)
{
  char c;
  return is >> t.tm_year >> c >> t.tm_mon >> c >> t.tm_mday >> c >> t.tm_hour >> c >> t.tm_min;
}
static inline long solve_case()
{
  ...
  std::tm begin{}, end{};
  cin >> begin >> end;
  auto dura{system_clock::from_time_t(mktime(&end)) - system_clock::from_time_t(mktime(&begin))};
  auto delta_minutes{duration_cast<minutes>(dura).count()};
  // if (end.tm_year >= 3200 && begin.tm_year <= 3200) {
  //   delta_minutes -= 24 * 60;
  // }
  debug("Pre", delta_minutes);
  auto cnt{0};
  for (auto const i: a) {
    if (delta_minutes < i) {
      return cnt;
    ...
    delta_minutes -= i;
    ++cnt;
    debug("In", delta_minutes);
  }
  debug("Post", delta_minutes);
  // Impossible.
  return cnt;
}
