...
auto solve_case()
{
  ...
  s[3] = {2, 1};
  for (u64 i{4}; i != n - 1; ++i) {
    k[i] = {k[i - 1].first + k[i - 2].first, k[i - 1].second + k[i - 2].second};
    s[i] = {k[i].first + s[i - 1].first, k[i].second + s[i - 1].second};
  }
  auto const t{(m - a * s.back().first) / s.back().second};

  if (x == n) {
    return m;
  }
  return s[x - 1].first * a + s[x - 1].second * t;
}
