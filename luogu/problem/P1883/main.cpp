...
template<typename T>
[[maybe_unused]] constexpr T inf{numeric_limits<T>::max() / (1ULL << 16)};
namespace impl {
...
{
  constexpr auto const precision{1e-9};
  u64 n;
  cin >> n;
  impl::vec2<double> mat(n, 3);
  cin >> mat;
  auto f{[&](auto const x) {
    double mx{-inf<double>};
    for (auto const& coefficients: mat) {
      double res{};
      for (u64 i{}; i != coefficients.size(); ++i) {
        res += coefficients[i] * pow(x, coefficients.size() - 1 - i);
      }
      check_max(mx, res);
    }
    return mx;
  ...
  while (abs(l - r) > precision) {
    auto const left_third{l + (r - l) / 3}, right_third{r - (r - l) / 3};
  ...
  cout << std::fixed << std::setprecision(4) << f(l) << '\n';
}
