...
static inline auto solve_case()
{
  ...
  for (i64 i{}; i != n; ++i) {
    cin >> t[i];
    enables[t[i]].emplace_back(i);
  }

  auto adj{read_graph(n, m, true, true, false)};
  ...
  for (i64 i{}; i != q; ++i) {
    ...
    auto end{enables.upper_bound(time)};
    for (auto it{enables.begin()}; it != end; ++it) {
      for (auto const k: it->second) {
        f[k][k] = 0;
        for (auto const [w, v]: adj[k]) {
          check_min(f[k][v], w);
          check_min(f[v][k], w);
        }
        for (i64 i{}; i != n; ++i) {
    ...
    enables.erase(enables.begin(), end);

    if (t[y] > time || t[x] > time) {
      cout << "-1\n";
    }
    else {
      cout << (f[x][y] == inf<i64> ? -1 : f[x][y]) << '\n';
    }
  }
}
