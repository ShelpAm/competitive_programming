...
{
  constexpr long long diff{86400};
  size_t n;
  cin >> n;
  arr1<size_t, 100001> num_of{};
  size_t num_of_kinds{};
  queue<pair<long long, vec1<size_t>>> q;
  for (size_t i{}; i != n; ++i) {
    long long t;
    size_t k;
    cin >> t >> k;
    vec1<size_t> colors(k);
    cin >> colors;
    ...
    q.emplace(t, colors);
    while (q.front().first <= t - diff) {
      for (auto const color: q.front().second) {
        if (--num_of[color] == 0) {
          --num_of_kinds;
        }
      }
