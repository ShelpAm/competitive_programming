...
auto solve_case()
{
  ...
  vvvu f(2 * n - 1, vvu(n, vu(n)));
  f[0][0][0] = a[0][0];
  for (u64 i{1}; i != 2 * n - 1; ++i) {
    for (u64 x1{}; x1 != n; ++x1) {
      auto const y1{i - x1};
      if (x1 >= n || y1 >= n) {
        continue;
      ...
      for (u64 x2{}; x2 != n; ++x2) {
        ...
        if (y1 >= 1 && y2 >= 1) {
          check_max(f[i][x1][x2], f[i - 1][x1][x2]);
        }
        if (x1 >= 1 && y2 >= 1) {
          check_max(f[i][x1][x2], f[i - 1][x1 - 1][x2]);
        }
        if (x2 >= 1 && y1 >= 1) {
          check_max(f[i][x1][x2], f[i - 1][x1][x2 - 1]);
        }
        if (x1 >= 1 && x2 >= 1) {
          check_max(f[i][x1][x2], f[i - 1][x1 - 1][x2 - 1]);
        }
        f[i][x1][x2] += a[x1][y1] + (x1 != x2 ? a[x2][y2] : 0);
      }
    }
  }
