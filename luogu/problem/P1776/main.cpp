...
auto solve_case()
{
    ...
    for (i64 i = 0; i != n; ++i) {
        ...
        for (i64 r = 0; r != w; ++r) {
            deque<i64> q;
            for (i64 j = 0; j * w + r <= W; ++j) {
                i64 const cur = f[fl ^ 1][j * w + r] - j * v;
                while (!q.empty() && q.back() < cur) {
                    q.pop_back();
                }
                q.push_back(cur);
                // std::println("cur: {}, q.front(): ", cur, q.front());
                check_max(f[fl][j * w + r], q.front() + j * v);
                if (j >= m && q.front() == f[fl ^ 1][(j - m) * w + r] - (j - m) * v) {
                    q.pop_front();
                }
            }
            // debug("f", f[fl]);
