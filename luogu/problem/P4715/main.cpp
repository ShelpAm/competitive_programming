...
static inline constexpr void debug([[maybe_unused]] std::string_view const sv,
                                   [[maybe_unused]] auto&& t)
{
#ifdef DEBUG
  std::cout << sv << ": ";
...
static inline auto solve_case()
{
  ...
  vec1<size_t> a(1 << n);
  cin >> a;
  auto l{std::ranges::max_element(a | take(1 << (n - 1)))};
  auto r{std::ranges::max_element(a | drop(1 << (n - 1)))};
  debug("l", *l);
  debug("r", *r);
  cout << 1 + (*l < *r ? l : r) - a.begin();
}
