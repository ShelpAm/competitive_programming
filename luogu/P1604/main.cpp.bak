...
// number_container_t Class with dynamic base parameter
class dynamic_number_container_t {
  friend auto& operator>>(auto& istream,
                          dynamic_number_container_t& number_container)
  {
    std::string tmp;
    istream >> tmp;
    number_container = tmp;
  ...
  friend auto& operator<<(auto& ostream,
                          dynamic_number_container_t const& number_container)
  {
    return ostream << number_container.to_string();
  }
public:
  dynamic_number_container_t() = default;
  dynamic_number_container_t(std::integral auto const num)
      : dynamic_number_container_t(std::to_string(num))
  {}
  dynamic_number_container_t(std::string const& s) { *this = s; }
  dynamic_number_container_t& operator=(std::string s)
  {
    if (s.empty()) {
      return *this;
    }
  ...
  auto operator<=>(dynamic_number_container_t const& rhs)
  {
    auto size_order{size() <=> rhs.size()};
    if (size_order == std::strong_ordering::equal) {
      for (size_t i{size() - 1}; i != -1UZ; --i) {
  ...
  dynamic_number_container_t& operator+=(dynamic_number_container_t const& rhs)
  {
    // Todo: remove this
    if (rhs.sign_ == -1 || sign_ == -1) {
      throw "Negetive number isn't supported.";
  ...
  [[deprecated]] dynamic_number_container_t&
  operator/=(dynamic_number_container_t const& rhs)
  {
    if (*this < rhs) {
      return *this = "0";
    }
  ...
  size_t operator[](size_t const index) const
  {
    return (*const_cast<dynamic_number_container_t*>(this))[index];
  }
  [[nodiscard]] size_t size() const { return digits_.size(); }
  [[nodiscard]] std::vector<char> to_vector() const
  {
  ...
  void base(long long const base) { base_ = base; }
private:
  // [[nodiscard]] char sign_char() const { return sign == 1 ? '\0' : '-'; }
...
dynamic_number_container_t operator+(dynamic_number_container_t const& lhs,
                                     dynamic_number_container_t const& rhs)
{
  return dynamic_number_container_t{lhs} += rhs;
}
...
dynamic_number_container_t operator-(dynamic_number_container_t const& lhs)
{
  return dynamic_number_container_t(-1) * dynamic_number_container_t{lhs};
}
dynamic_number_container_t operator/(dynamic_number_container_t const& lhs,
                                     dynamic_number_container_t const& rhs)
{
...
void solve_case()
{
  size_t base{};
  std::cin >> base;
  dynamic_number_container_t a;
  dynamic_number_container_t b;
  std::cin >> a >> b;
  a.base(base);
  b.base(base);
  std::cout << a + b;
