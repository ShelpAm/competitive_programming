...
void solve_case()
{
  ...
  double const m{static_cast<double>(n + 1) / 2};
  double const a{static_cast<double>(k + n - n % k) / 2};
  double const b{(m * n - a * (n / k)) / (n - n / k)};
  std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(1) << a
            << ' ' << b;
}
int main()
{
