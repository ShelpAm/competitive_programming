...
auto solve_case()
{
    ...
    auto day_test = [](u64 month, u64 day) {
        static set _31{1, 3, 5, 7, 8, 10, 12};
        if (day < 1 || day > 31) {
            return false;
        }
        if (_31.contains(month)) {
            return day <= 31;
        }
        else if (month == 2) {
            return day <= 27;
        ...
        for (auto _0pos: _0) {
            ...
            for (auto __2pos: _2) {
                ...
                for (auto _3pos: _3) {
                    ...
                    for (u64 i = _3pos + 1; i != a.size(); ++i) {
                        for (u64 j = i + 1; j != a.size(); ++j) {
                            for (u64 k = j + 1; k != a.size(); ++k) {
                                for (u64 l = k + 1; l != a.size(); ++l) {
                                    auto const month = a[i] * 10 + a[j];
                                    auto const day = a[k] * 10 + a[l];
                                    if (month <= 12 && month >= 1
                                        && day_test(month, day)) {
                                        debug("month, day", puu{month, day});
                                        debug("i j k l",
                                              std::array{i, j, k, l});
                                        ++ans;
                                    }
