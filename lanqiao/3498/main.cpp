...
auto solve_case()
{
    ...
    long double lo = 0, hi = 0.5;
    while (lo + eps < hi) {
        auto const mid = (lo + hi) / 2;
        auto const f = mid * mid * std::log2(mid) + (1 - mid) * (1 - mid) * std::log2(1 - mid);
        debug("mid", mid);
        debug("f", f);
        if (f >= h) {
            lo = mid;
    ...
    cout << lo << '\n';
    cout << static_cast<u64>(lo * n) << '\n';

    debug("H(S)", n * lo * (-lo * std::log2(lo)) + (-(1 - lo) * std::log2(1 - lo) * (1 - lo) * n));
}
