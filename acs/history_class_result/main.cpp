...
constexpr size_t max_length_of_name{13};
constexpr size_t num_person{10};
auto solve_case()
{
  std::fstream fs("/home/shelpam/result.csv");
  string buf;
  getline(fs, buf);
  while (std::getline(fs, buf)) {
    std::stringstream ss(buf);
    size_t ranking, group_id;
    char comma;
    ss >> ranking >> comma >> group_id;
    vec1<string> names(num_person);
    for (auto&& i: names) {
      i.resize(max_length_of_name);
      ss.getline(i.data(), ',');
    }
    vec1<string> ids(num_person);
    for (auto&& i: ids) {
      i.resize(max_length_of_name);
      ss.getline(i.data(), ',');
    }
    cout << ranking << comma << group_id;
    for (size_t i{}; i != num_person; ++i) {
      cout << comma << names[i] << '(' << ids[i] << ')';
    }
    ss.get();
  }
}
