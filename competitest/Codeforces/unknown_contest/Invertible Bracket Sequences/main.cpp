// Problem: Invertible Bracket Sequences
// Contest: unknown_contest
// Judge: Codeforces
// URL: https://m1.codeforces.com/contest/1976/problem/D
// Memory Limit: 256
// Time Limit: 2000
// Start: Thu 30 May 2024 10:55:06 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  unordered_map<char, char> cvt{{'(', ')'}, {')', '('}};
  auto check{[&](auto const l, auto const r, string& res) {
    auto t{s};
    for (int i{l}; i <= r; ++i) {
      t[i] = cvt[t[i]];
    }
  ...
  for (int i{}; i != n; ++i) {
    for (int j{i + 1}; j != n; ++j) {
      string res;
      if (check(i, j, res)) {
        cout << i << ' ' << j << " inverted part: " << res << '\n';
      }
    }
  }
  /*cout << "original: " << s << '\n';*/
  int c{};
  vector<vector<int>> good(2);
  for (int i{}; i != n - 1; ++i) {
    if (c > 0 && s[i] != s[i + 1]) {
      good[i % 2].push_back(i);
    }
    if (s[i] == '(') {
      ++c;
    }
