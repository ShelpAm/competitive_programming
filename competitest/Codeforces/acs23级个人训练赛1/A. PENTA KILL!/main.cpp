...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/553673/problem/A*/
/*Start: Sat 28 Sep 2024 01:04:14 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  int n;
  std::cin >> n;
  std::unordered_map<std::string, std::vector<std::string>> kills;
  for (int i{}; i != n; ++i) {
    std::string a, b;
    std::cin >> a >> b;
    kills[a].push_back(b);
    std::unordered_set<std::string> o;
    for (auto const e : kills[a] | std::views::reverse | std::views::take(5)) {
      o.insert(e);
    }
    if (o.size() == 5) {
      std::cout << "PENTA KILL!";
