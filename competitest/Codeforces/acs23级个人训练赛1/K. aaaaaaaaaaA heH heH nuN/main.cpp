...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/553673/problem/K*/
/*Start: Sat 28 Sep 2024 01:54:53 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::string app;
  for (auto const e : bits | std::views::reverse | std::views::drop(1)) {
    app.push_back('a');
    if (e == 1) {
      app.push_back('h');
    }
  }
  auto const minus{std::ranges::count(app, 'h')};
  for (int i = app.size(); i != 0; --i) {
    if (app[i - 1] == 'a') {
      app.insert(i - 1, std::string(minus + 1, 'h'));
      break;
    }
  }
  std::cout << sb + app << '\n';
