...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/553673/problem/C*/
/*Start: Sat 28 Sep 2024 03:00:26 PM CST*/
/*Author: ShelpAm*/
...
#define int i64
  ...
  auto query{[&](auto x) {
    std::deque<i64> q{0};
    std::vector<i64> f(n / x + 1, -inf<i64>);
    f[0] = 0;
    for (int i{1}; i * x <= n; ++i) {
      chmax(f[i], q.front() + a[i * x]);
      while (!q.empty() && q.back() < f[i]) {
        q.pop_back();
      }
      q.push_back(f[i]);
      if (i - p / x >= 0 && q.front() == f[i - p / x]) {
        q.pop_front();
      }
    }
    debug("f", f);
    auto const i{
        binary_search([&](auto i) { return i * x + p > n; }, n / x, -1)};
    return *std::ranges::max_element(f.begin() + i, f.end());
  }};

  std::vector<i64> ans(n + 1, -inf<i64>);
  for (int i{}; i != q; ++i) {
    int x;
    std::cin >> x;
    if (x > p) {
