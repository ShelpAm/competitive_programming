/*Problem: H. Visit the Park*/
/*Contest: JXNU Summer Train 14*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/543319/problem/H*/
/*Start: Fri 16 Aug 2024 03:28:21 PM CST*/
/*Author: ShelpAm*/
...
namespace {
[[maybe_unused]] constexpr std::int_fast64_t mod998244853{998'244'853LL};
...
private:
  std::vector<std::unordered_multimap<std::int_fast64_t, int>> adjacent;
};
...
void solve_case() noexcept
{
  auto divide{[](i64 const a, i64 const b) {
    return a * pow(b, mod998244853 - 2, mod998244853) % mod998244853;
  }};
  int n, m, k;
  std::cin >> n >> m >> k;
  auto const g{graph::read(n, m, false, true)};
  std::vector<std::unordered_map<int, int>> h(n);
  std::vector<std::unordered_map<int, int>> cnt(n);
  for (int u{}; u != n; ++u) {
    for (auto const &[w, v] : g.edges_of(u)) {
      h[u][v] += w;
      ++cnt[u][v];
  ...
  u64 ans{};
  for (int i{}; i != k - 1; ++i) {
    if (h[a[i] - 1].contains(a[i + 1] - 1)) {
      ans = (ans * 10 + h[a[i] - 1][a[i + 1] - 1]) % mod998244853;
    }
    else {
