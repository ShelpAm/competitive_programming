/*Problem: D. Pick Up*/
/*Contest: The 2019 ICPC China Shaanxi Provincial Programming Contest*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/104460/problem/D*/
/*Start: Thu 01 Aug 2024 08:27:33 PM CST*/
/*Author: ShelpAm*/
...
{
  i64 a, b;
  i64 xa, xb, xc, ya, yb, yc;
  std::cin >> a >> b >> xa >> ya >> xb >> yb >> xc >> yc;
  auto t{[](i64 a, i64 b, i64 v) {
    return static_cast<double>(std::abs(a - b)) / v;
  }};
  std::cout << std::min(t(xa, xc, a) + t(ya, yc, a),
                        1. * (std::abs(xa - xb), std::abs(ya - yb)) / (a + b) +
                            t(1. * (xa * b + xb * a) / (a + b), xc, b) +
                            t(1. * (ya * b + yb * a) / (a + b), yc, b))
            << '\n';
}
} // namespace
