...
void solve_case() noexcept
{
  ...
  for (int i{}; i != n; ++i) {
    cur += a[i];
    if (std::abs(cur) + suf[i + 1] == k && cur < 0) {
      ans += pow(2ULL, (n - 1 - i) + cnt_non_neg, mod998244353);
      ans %= mod998244353;
    }
    if (cur >= 0) {
      ++cnt_non_neg;
