...
// Start: Thu 27 Feb 2025 10:40:36 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto perform{[&s](int x) -> int { // true, steps; false, end_point
        for (int cnt{}; auto const e : s) {
            if (e == 'L') {
                --x;
            }
            else {
    ...
    debug("first steps", r);
    if (r <= k) { // At least reach once.
        k -= r;
    }

    r = perform(0);
    if (r == 0) { // Never reach again
        std::cout << 1 << '\n';
        return;
    }

    // Can reach repeatedly
    std::cout << 1 + (k / r) << '\n';
}
} // namespace
