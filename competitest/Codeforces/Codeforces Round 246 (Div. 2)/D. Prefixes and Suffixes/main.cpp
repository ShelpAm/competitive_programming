...
// Contest: Codeforces Round 246 (Div. 2)
// Judge: Codeforces
// URL: https://codeforces.com/problemset/problem/432/D
// Memory Limit: 256
// Time Limit: 1000
// Start: Tue 28 May 2024 02:22:18 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  cin >> s;
  int const n = s.size();
  auto const z{::string::z_function(s)};
  set<int> found{n};
  map<int, int> cnt{{n, 0}};
  for (int i{}; i != n; ++i) {
    if (z[i] + i == n) {
  ...
  for (auto const [len, num] : cnt) {
    if (found.contains(len)) {
      ans.push_back({len, num + 1});
    }
  }
  cout << ans.size() << '\n';
