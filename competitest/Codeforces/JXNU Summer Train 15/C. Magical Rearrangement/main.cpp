...
/*Contest: JXNU Summer Train 15*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/543826/problem/C*/
/*Start: Sun 18 Aug 2024 03:44:40 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  if (std::ranges::any_of(cnt, [&](auto const e) { return e > (n + 1) / 2; })) {
    std::cout << -1 << '\n';
    return;
  }

  auto remains{n};
  auto get{[&](int const last) -> int {
    if (remains % 2 == 1) {
      if (auto must{std::ranges::find(cnt, (remains + 1) / 2)}; must != cnt.end()) {
        if (last == must - cnt.begin()) {
          return -1;
        }
        return must - cnt.begin();
      }
      for (int i{}; i!=10; ++i) {
        if (cnt[i] && last != i) {
          return i;
