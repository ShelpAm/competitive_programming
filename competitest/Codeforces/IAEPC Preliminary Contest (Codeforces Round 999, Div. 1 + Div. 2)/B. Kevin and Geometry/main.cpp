...
// Start: Thu 13 Feb 2025 11:34:20 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    a.erase(std::ranges::find(a, longest));
    a.erase(std::ranges::find(a, longest));
    std::ranges::sort(a);

    for (auto const r : a | std::views::slide(2)) {
        if (r[0] + 2 * longest > r[1]) {
            std::cout << r[0] << ' ' << r[1] << ' ' << longest << ' ' << longest
                      << '\n';
            return;
        }
    }
