...
// Start: Fri 14 Feb 2025 12:11:07 AM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<std::pair<int, int>> mat; // bit-and value, cost
    for (unsigned mask{}; mask != 1 << m; ++mask) {
        unsigned t{max<unsigned>};
        for (int j{}; j != m; ++j) {
            if (mask & 1 << j) {
                t &= b[j];
            }
        }
        mat.push_back({t, std::popcount(mask)});
    }

    std::priority_queue<std::pair<int, int>> q; // delta, index
    std::vector value(n, std::vector<int>(m + 1));
    std::vector<int> step(n);
    for (int idx{}; auto &&[e, v] : std::views::zip(a, value)) {
        for (auto const &[t, i] : mat) {
            chmax(v[i], e - (e & t));
        }
        q.push({v[1], idx});
        ++idx;
    ...
    auto sum{sum_of(a)};
    for (int i{}; i != k; ++i) {
        auto const [d, idx]{q.top()}; // best delta
        q.pop();

        sum -= d;
        if (++step[idx] < m) {
            q.push({value[idx][step[idx] + 1] - value[idx][step[idx]], idx});
        }
    }
