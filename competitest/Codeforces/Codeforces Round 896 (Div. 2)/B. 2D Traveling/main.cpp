...
// Contest: Codeforces Round 896 (Div. 2)
// Judge: Codeforces
// URL: https://codeforces.com/contest/1869/problem/B
// Memory Limit: 256
// Time Limit: 1000
// Start: Mon 22 Apr 2024 09:03:05 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  auto const direct{abs(pos[a - 1].first - pos[b - 1].first)
                    + abs(pos[a - 1].second - pos[b - 1].second)};
  i64 d1{inf<i64>}, d2{inf<i64>};
  auto [s, t]{pos[a - 1]};
  auto [f, g]{pos[b - 1]};
  for (int i{}; i != k; ++i) {
    auto [x, y]{pos[i]};
    d1 = min(d1, abs(s - x) + abs(t - y));
    d2 = min(d2, abs(f - x) + abs(g - y));
  }
