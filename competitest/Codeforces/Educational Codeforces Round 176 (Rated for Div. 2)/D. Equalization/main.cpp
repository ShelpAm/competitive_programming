...
// Start: Thu 20 Mar 2025 09:12:07 PM CST
// Author: ShelpAm
...
constexpr auto k = 64;
std::vector f(k, std::vector(k, inf<u64>));
std::vector g(k, std::vector(k, inf<u64>));
int main()
{
    f[0][0] = 0;
    for (int x{1}; x != 64; ++x) {
        for (int i = f.size() - 1; i != -1; --i) {
            for (int j = f[i].size() - 1; j != -1; --j) {
                if (i >= x) {
                    chmin(f[i][j], f[i - x][j] + (1ULL << x));
                }
                if (j >= x) {
                    chmin(f[i][j], f[i][j - x] + (1ULL << x));
                }
            }
        }
    }
...
void solve_case()
{
    ...
    auto a = std::bitset<64>(x).to_string();
    auto b = std::bitset<64>(y).to_string();
    ...
    // completely a and b to 0
    // Or:
    //  Assume longest prefix length is m,
    //  then the answer is f[|a| - m + i][|b| - m + i] (0 <= i <= m).
    auto m = [&] {
        int l{};
        for (auto [p, q] : std::views::zip(a, b)) {
            if (p != q) {
                break;
            }
            ++l;
    ...
    auto ans = max<u64>;
    for (int i{}; i != m + 1; ++i) {
        debug("checking", std::pair{a.size() - m + i, b.size() - m + i});
        chmin(ans, f[a.size() - m + i][b.size() - m + i]);
    }
    for (int i = a.size(); i != 64; ++i) {
        for (int j = b.size(); j != 64; ++j) {
            chmin(ans, f[i][j]);
        }
    }
    std::cout << ans << '\n';
