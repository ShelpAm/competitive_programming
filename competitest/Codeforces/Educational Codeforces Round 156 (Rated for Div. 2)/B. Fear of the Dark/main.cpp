...
// Memory Limit: 256
// Time Limit: 2000
// Start: Tue 04 Jun 2024 07:19:48 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  cin >> p >> a >> b;
  auto const d{[](auto const &l, auto const &r) {
    auto const [x, y]{l};
    auto const [p, q]{r};
    return sqrt((x - p) * (x - p) + (y - q) * (y - q));
  }};
  long double ans{};
  check_max(ans, min(d(p, a), d(p, b)));
  check_max(ans, min(d(a, o), d(b, o)));
  if ((d(a, o) > ans || d(a, p) > ans) && (d(b, o) > ans || d(b, p) > ans)) {
    check_max(ans, d(a, b) / 2);
  }
  cout << ans << '\n';
}
