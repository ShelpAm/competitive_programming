...
/*Start: Thu 15 Aug 2024 05:33:33 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  std::vector<int> a(n);
  std::cin >> a;
  auto const b{calc_b(a)};
  std::vector<int> reverses{0};
  ...
  if (reverses.size() > 3) {
    ...
    for (auto const e : reverses) {
      for (auto d : {-2, -1, 0, 1, 2}) {
        d += e;
        if (d < 0 && d >= a.size()) {
          continue;
        }
        auto c{a};
        c.erase(c.begin() + d);
        if (auto const b{calc_b(c)}; std::is_sorted(b.begin(), b.end())) {
          std::cout << "YES\n";
          return;
        }
      }
