...
// Start: Sat 09 Aug 2025 02:46:41 PM CST
// Author: ShelpAm
...
class Graph {
  ...
  private:
    std::vector<std::vector<std::pair<std::int_fast64_t, int>>> adjacent_;
};
...
template <std::int_least64_t mod> class Mod_int {
  public:
    ...
    constexpr Mod_int(std::int_least64_t n) : value_{n % mod} {}
...
void solve_case()
{
    ...
    Mod_int<mod1e9p7> ans{2};
    int leaves{};
    std::vector<int> light(n);
    std::vector<int> heavy(n);
    for (int i{}; i != n; ++i) {
        if (g.edges_of(i).size() == 1) {
            ++leaves;
            ++light[g.edges_of(i).front().second];
        }
        else {
            for (auto [_, v] : g.edges_of(i)) {
                ++heavy[v];
            }
        }
    }
    if (std::ranges::find_if(heavy, [](auto e) { return e >= 3; }) !=
        heavy.end()) {
        std::cout << 0 << '\n';
        return;
    }
