...
// Start: Sun 20 Jul 2025 12:36:19 AM CST
// Author: ShelpAm
...
class Graph {
  public:
    ...
    explicit Graph(std::size_t max_num_of_vertices)
        : adjacent_(max_num_of_vertices)
    {
    }

    void add_edge(std::size_t u, std::size_t v, std::int_fast64_t w)
    {
        adjacent_[u].emplace_back(w, v);
    }

    [[nodiscard]] auto const &edges_of(std::size_t u) const
    {
        return adjacent_[u];
    }

    [[nodiscard]] auto &edges_of(std::size_t u)
    {
        return adjacent_[u];
    }
  ...
  private:
    std::vector<std::list<std::pair<std::int_fast64_t, int>>> adjacent_;
};
                                   ...
                                   std::vector<int> const &ok) noexcept
{
    ...
    std::vector<int> weight(g.size()); // Maximum size of all subtrees
    ...
    auto dfs = [&](auto dfs, int u, int p) -> void {
        ...
        for (auto [w, v] : g.edges_of(u)) {
            if (v != p && ok[v]) {
                dfs(dfs, v, u);
                size[u] += size[v];
                chmax(weight[u], size[v]);
            }
        }
        chmax(weight[u], sz - size[u]);
        if (weight[u] <= sz / 2) {
...
void solve_case()
{
    ...
    int n, m;
    std::cin >> n >> m;
    auto g = graph::Graph::read(n, m, false, false);

    std::vector<int> T(n);
    for (int i{}; i != n; ++i) {
        T[i] = g.edges_of(i).size();
    }

    // Earliest time to arrive i, and minimum waiting time
    std::vector<std::vector<std::pair<int, int>>> f(n);
    for (int i{}; i != n; ++i) {
        f[i].resize(g.edges_of(i).size(), std::pair{inf<int>, inf<int>});
    }
    f[0][0] = {0, 0};
    ...
    while (!q.empty()) {
        ...
        int i{};
        for (auto it = g.edges_of(u).begin(); it != g.edges_of(u).end();) {
            // for (auto [i, edge] : std::views::enumerate(g.edges_of(u))) {
            auto [_, v] = *it;
            auto [cur_t, cur_wait] = f[u][lvl];
            auto wait = (i - cur_t) % T[u];
            if (wait < 0)
                wait += T[u];
            auto arrive_t = cur_t + wait + 1;
            auto next_lvl = arrive_t % T[v];
            if (chmin(f[v][next_lvl], std::pair{arrive_t, cur_wait + wait})) {
                q.push({v, next_lvl});
                ++it;
            }
            else {
                it = g.edges_of(u).erase(it);
            }
            ++i;
        }
    }
    auto [t, wait] = std::ranges::min(f[n - 1]);
    std::cout << t << ' ' << wait << '\n';
}
} // namespace
