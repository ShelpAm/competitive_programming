...
// Memory Limit: 256
// Time Limit: 5000
// Start: Fri 03 May 2024 12:12:25 AM CST
// Author: ShelpAm
...
auto solve_case() {
  ...
  while (q--) {
    int l, r;
    cin >> l >> r;
    if ((a[l - 1] ^ a[r]) == 0) { // Can be splited to 2 partitions
      cout << "YES\n";
    } else { // Can be splited to at least 3 partitions
      auto const pl{*ranges::lower_bound(pos[a[r]], l - 1)};
      auto const pr{*prev(ranges::upper_bound(pos[a[l - 1]], r))};
      cout << (pl < pr ? "YES" : "NO") << '\n';
    }
  }
  cout << '\n';
