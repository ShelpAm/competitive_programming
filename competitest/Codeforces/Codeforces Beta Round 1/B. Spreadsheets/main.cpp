...
// Memory Limit: 64
// Time Limit: 10000
// Start: Mon 27 May 2024 09:30:52 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  string s;
  cin >> s;
  auto ctoi{[](char const ch) { return ch >= 'A' ? ch - 'A' + 10 : ch - '0'; }};
  auto itoc{[](int const x) { return x >= 10 ? 'A' + x - 10 : '0' + x; }};
  auto stoi{[ctoi](string_view s) {
    int x{};
    for (auto const ch : s) {
      x = x * 27 + ctoi(ch);
    }
  ...
  auto itos{[itoc](string_view s) {
    int x{std::stoi(string{s})};
    string t;
    while (x != 0) {
      debug("x%27", x % 27);
      t += itoc(x % 27);
  ...
  if (s.size() == 4) {
    auto a{s.substr(0, 2)}, b{s.substr(2)};
    cout << 'R' << stoi(b) << 'C' << stoi(a) << '\n';
  }
  else if (s.size() == 6) {
    auto a{s.substr(1, 2)}, b{s.substr(4, 2)};
    cout << itos(b) << itos(a) << '\n';
  }
  else {
    assert(false);
