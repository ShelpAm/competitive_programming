...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/541349/problem/B*/
...
void solve_case() noexcept
{
  ...
  std::vector<std::unordered_map<int, i64>> f(2);
  int fl{};
  f[fl][m] = 0;
  for (int i{}; i != n; ++i) {
    fl ^= 1;
    f[fl] = f[fl ^ 1];
    for (auto const &[k, v] : f[fl ^ 1]) {
      chmax(f[fl][k / d[i]], v + 1LL * p[i] * k);
    }
  }

  i64 ans{};
  for (auto const &[_, v] : f[fl]) {
    chmax(ans, v);
  }
  std::cout << ans << '\n';
}
