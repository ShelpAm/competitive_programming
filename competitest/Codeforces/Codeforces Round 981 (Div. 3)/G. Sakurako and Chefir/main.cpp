...
void solve_case()
{
  ...
  int n;
  std::cin >> n;
  auto const g{Graph::read(n, n - 1, true, false)};

  auto const k{msb(static_cast<u64>(n))};
  std::vector<int> l(n); // max depth rooted by 1 in sub-tree.
  std::vector<int> h(n); // max depth rooted by i in sub-tree.
  std::vector<int> depth(n);
  std::vector<std::vector<std::set<int>>> f(
      k + 1, std::vector<std::set<int>>(n, {-inf<int>}));
  std::vector<std::vector<int>> pa(k + 1, std::vector<int>(n));
  ...
  auto dfs{[&](auto dfs, int u, int d) -> void {
    ...
    for (int i{1}; i != f.size(); ++i) {
      f[i][u].insert() std::max(f[i - 1][u], f[i - 1][pa[i - 1][u]]);
    }
  }};
  dfs(dfs, 0, 0);
  ...
  for (int i{}; i != q; ++i) {
    ...
    auto t{v};
    int ans{};
    for (auto i{pa.size() - 1}; i != -1; --i) {
      if (k >= (1 << i)) {
        chmax(ans, depth[v] + f[i][t]);
        k -= 1 << i;
        t = pa[i][t];
      }
    }
    chmax(ans, depth[v] + f[0][t]);
    std::cout << ans << ' ';
  }
  std::cout << '\n';
}
