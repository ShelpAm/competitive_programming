...
// Contest: Codeforces Round 455 (Div. 2)
// Judge: Codeforces
// URL: https://codeforces.com/contest/909/problem/C
// Memory Limit: 256
// Time Limit: 2000
// Start: Thu 30 May 2024 01:16:02 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  vector<vector<i64>> s(2, vector<i64>(n + 1));
  int fl{1};
  char op;
  cin >> op;
  s[fl][0] = f[fl][0] = 1;
  for (int i{2}; i != n + 1; ++i) {
    ...
    s[fl][n] = f[fl][n];
    for (int j{n - 1}; j != -1; --j) {
      s[fl][j] = (s[fl][j + 1] + f[fl][j]) % mod;
    }
    cin >> op;
  }
  cout << sum_of(f[fl]) % mod;
}
