...
/*Start: Sat 27 Jul 2024 12:31:50 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::cin >> s;
  s += 'L';
  auto next{[&](int const i, char const c) {
  ...
  int i{-1};
  while (i != s.size() - 1) {
    if (auto const nex{next(i, 'L')}; nex <= i + m) {
      i = nex;
    }
    else {
      for (int j{std::min(n, i + m)}; j >= i; --j) {
        if (s[j] != 'C') {
          i = j;
          break;
        }
      }
      if (auto const next_l{next(i, 'L')};
          k >= next_l - i &&
          std::ranges::all_of(s.begin() + i + 1, s.begin() + next_l,
                              [](auto const e) { return e != 'C'; })) {
        k -= next_l - i;
        i = next_l;
      }
      else {
