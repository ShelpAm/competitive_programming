...
// Memory Limit: 256
// Time Limit: 2000
// Start: Fri 31 May 2024 11:51:32 AM CST
// Author: ShelpAm
...
[[maybe_unused]] constexpr std::int_fast64_t ls(std::int_fast64_t const i)
{
  return i & (-i);
}
// i mustn't be 0
[[maybe_unused]] constexpr std::size_t ms(std::uint_fast64_t const i)
{
  assert(i != 0);
...
auto solve_case()
{
  ...
  int n, m;
  cin >> n >> m;
  vector<vector<int>> c(2, vector<int>(n + m + 1));
  cin >> c;
  vector<int> rev(n + m + 1);
  vector<int> occ(n + m + 1);
  u64 s{};
  vector<int> left{n, m};
  for (int i{}; i != n + m; ++i) {
    auto selection{c[0][i] > c[1][i] ? 0 : 1};
    if (left[selection] == 0) {
      selection ^= 1;
      rev[i] = 1;
    }
  ...
  vector<vector<int>> right_want(2, vector<int>(n + m + 1));
  vector<int> r(2, n + m);
  for (int i = n + m; i != -1; --i) {
    for (auto const t : {0, 1}) {
      right_want[t][i] = r[t];
    }
    if (rev[i]) {
      r[occ[i] ^ 1] = i;
  ...
  for (int i{}; i != n + m; ++i) {
    if (auto const first{right_want[occ[i]][i]}; first != n + m) {
      cout << s - c[occ[i]][i] + c[occ[first] ^ 1][first] -
                  c[occ[first]][first] + c[occ[i] ^ 1].back()
           << ' ';
    }
    else {
      cout << s - c[occ[i]][i] + c[occ[i]].back() << ' ';
    }
  }
  cout << s << ' ';
  cout << '\n';
