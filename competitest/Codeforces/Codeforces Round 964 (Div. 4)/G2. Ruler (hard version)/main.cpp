...
// Start: Thu 02 Jan 2025 04:13:55 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    // If x >= l, then it reports l + 1.
    int l{2}, r{999};
    while (r - l > 2) {
        auto const ll{l + ((l + r) / 3)};
        auto const rr{r - ((l + r) / 3)};
        auto const area{ask(ll, rr)};
        if (area == ll * rr) {
            l = rr + 1;
        }
        else if (area == ll * (rr + 1)) {
            l = ll + 1;
            r = rr;
        }
        else if (area == (ll + 1) * (rr + 1)) {
            r = ll;
        }
    }
    for (int i{l}; i <= r; ++i) {
