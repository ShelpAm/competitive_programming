...
// Start: Tue 29 Apr 2025 11:13:54 AM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto next = [&pos, n](int p, char c) {
        auto it = std::ranges::upper_bound(pos[c - 'a'], p);
        return it == pos[c - 'a'].end() ? n : *it;
    };

    std::vector<int> left(n + 1);
    for (int i{n - 1}; i != -1; --i) {
        int max{};
        for (auto c : std::views::iota('a', 'a' + k)) {
            chmax(max, next(i, c));
        }
        left[i] = left[max] + 1;
    }
