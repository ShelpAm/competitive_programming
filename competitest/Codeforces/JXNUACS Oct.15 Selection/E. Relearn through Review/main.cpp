...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/557809/problem/E*/
/*Start: Tue 22 Oct 2024 01:41:59 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  auto solve{[n, k](auto const &a) {
    auto suff{a};
    for (int i{n - 1}; i != 0; --i) {
      suff[i] = std::gcd(suff[i], suff[i + 1]);
    }

    auto pref{a};
    for (int i{2}; i != n + 1; ++i) {
      pref[i] = std::gcd(pref[i], pref[i - 1]);
    }
    ...
    for (auto const i : diff) {
      ...
      for (int j{i + 1}; j != n + 1; ++j) {
        tmp = std::gcd(tmp, a[j] + k);
        auto fuck{tmp};
        if (i != 0) {
          fuck = std::gcd(fuck, pref[i]);
        }
        if (j != n) {
          fuck = std::gcd(fuck, suff[j + 1]);
        }
        chmax(ans, fuck);
      }
    }
  ...
  i64 fuck{a[1]};
  for (auto const e : a | std::views::drop(1) | std::views::take(n)) {
    fuck = std::gcd(fuck, e);
  }
  chmax(ans, fuck);
