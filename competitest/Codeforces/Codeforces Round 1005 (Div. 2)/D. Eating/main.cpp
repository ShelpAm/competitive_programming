...
// Start: Sun 16 Feb 2025 11:25:42 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    for (int i{}; i != q; ++i) {
        ...
        int must_ok{};
        int reachable{};
        i64 bound{n + 1}; // the bound you cannot reach
        for (int b{30}; b != -1; --b) {
            auto const &pre{pref[b]};
            auto get_bits{[&] { return (x >> b & 1) + pre[must_ok]; }};
            if (get_bits() % 2 == 1) {
                auto next_1{
                    std::ranges::lower_bound(pref[b], pre[must_ok] + 1) -
                    pref[b].begin()};
                must_ok = std::min(next_1 - 1, bound - 1);
                if (must_ok == n) {
                    all = true;
                }
                next_1 =
                    std::ranges::lower_bound(pref[b], pre[must_ok + 1] + 1) -
                    pref[b].begin();
                chmin(bound, next_1);
            }
            else {
            ...
            debug("After b " + std::to_string(b) + ", l and bound",
                  std::pair{must_ok, bound});
        }
        chmin(must_ok, bound);
        if (all) {
            std::cout << n << ' ';
