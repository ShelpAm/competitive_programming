...
// Start: Sun 16 Feb 2025 10:41:48 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    a.push_back(-1);
    std::vector<std::pair<int, int>> ans; // [l, r)
    int l{};
    ...
    if (std::ranges::all_of(
            ans, [](auto const &r) { return r.first >= r.second; })) {
        std::cout << 0 << '\n';
        return;
    }

    auto const [p, q]{std::ranges::max(
        ans, {}, [](auto const &r) { return r.second - r.first; })};
    std::cout << p + 1 << ' ' << q << '\n';
}
} // namespace
