/*Problem: A. Shaking Trees*/
/*Contest: JXNU Summer Train 6*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/538860/problem/0*/
/*Start: Sun 28 Jul 2024 01:10:59 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  dfs(dfs, 0, 0);
  std::vector<int> cnt(102), pref(102);
  cnt[101] = 1;
  u64 u{1}, d{1};
  for (int i{}; i != n; ++i) {
    u = u * (i + 1) % mod1e9p7;
  ...
  for (int i{2}; i != cnt.size(); ++i) {
    ...
    if (cnt[i] == 1) {
      d *= pref[i - 1] - pref[last];
      d %= mod1e9p7;
      last = i - 1;
    }
  }
  std::cout << std::ranges::max(depth) << ' '
            << u * pow(d, mod1e9p7 - 2, mod1e9p7) % mod1e9p7;
}
} // namespace
