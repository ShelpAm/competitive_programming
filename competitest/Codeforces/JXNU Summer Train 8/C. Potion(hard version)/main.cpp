...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/539980/problem/C*/
/*Start: Fri 02 Aug 2024 03:47:03 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::cerr << x << ' ' << y << ' ' << a << ' ' << b << '\n';
  std::function<int(i64, i64)> f{[&s, &f, a, b](i64 u, i64 v) -> int {
    std::cerr << u << ' ' << v << "\n";
    if (!s.insert({u, v}).second) {
      return inf<int>;
    }
    if (u == 0 || v == 0) {
      return (u != 0) + (v != 0);
    ...
    int res{inf<int>};
    if (v % a == 0 && u >= b * v / a) {
      check_min(res, f(u - b * v / a, v + b * v / a));
    }
    if (u % a == 0 && v >= b * u / a) {
      check_min(res, f(u + b * u / a, v - b * u / a));
    }
    return res + 1;
  }};
  auto res{f(x, y)};
