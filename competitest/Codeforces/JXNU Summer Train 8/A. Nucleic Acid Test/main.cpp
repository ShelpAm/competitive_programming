...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/539980/problem/A*/
/*Start: Fri 02 Aug 2024 02:36:31 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  for (int i{}; i != n; ++i) {
    (base[i] ? bases : nonbases).push_back(i);
  }

  auto const dist{graph::floyd(g)};
  i64 const ub = 7e11 / (t + 1);
  auto const ans{binary_search(
      [&](auto const v) {
        auto const d{v * t}; // d < 4e11
        Dsu dsu(n);
        for (auto const i : bases) {
          for (auto const j : bases) {
            for (auto const k : nonbases) {
