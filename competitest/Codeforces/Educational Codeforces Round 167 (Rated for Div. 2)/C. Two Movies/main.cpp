...
/*Start: Thu 15 Aug 2024 01:43:03 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  std::vector<std::vector<int>> p(2, std::vector<int>(n));
  std::cin >> p;
  for (int i{}; i != n; ++i) {
    int x{p[0][i]}, y{p[1][i]};
    if (x > y) {
  ...
  debug("a, b", std::pair{a, b});
  debug("pos, neg", std::pair{positive, negative});
  auto const minus{std::min(std::abs(a - b), negative)};
  (a > b ? a : b) -= minus;
  negative -= minus;
  auto const plus{std::min(std::abs(a - b), positive)};
  (a > b ? b : a) += plus;
  positive -= plus;
  if (a != b) {
    ...
    std::cout << a + ((positive - negative) < 0 ? (positive - negative - 1) / 2
                                                : (positive - negative) / 2)
              << '\n';
  }
}
