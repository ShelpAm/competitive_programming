...
// Start: Tue 18 Feb 2025 10:47:08 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    for (int i{}; i != n; ++i) {
        for (int j{}; j != m; ++j) {
            ...
            if (!div.contains(a[i][j])) {
                div[a[i][j]] = 1;
            }
        }
    }

    auto const divs{div | std::views::values};
    std::cout << sum_of(divs) - std::ranges::max(divs) << '\n';
}
} // namespace
