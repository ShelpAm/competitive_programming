/*Problem: C. New Game*/
/*Contest: Educational Codeforces Round 170 (Rated for Div. 2)*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/contest/2025/problem/C*/
/*Start: Mon 14 Oct 2024 10:48:36 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  for (int last{-1}; auto const &[num, v] : cnt) {
    if (num > last + 1) {
      ...
      if (pref.size() - 1 < k) {
        ...
        for (int i{1}; i + k - 1 < pref.size(); ++i) {
          chmax(ans, pref[i + k - 1] - pref[i - 1]);
        }
      }
      pref.assign({0});
    }
