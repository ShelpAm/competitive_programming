/*Problem: F. Haunted House*/
/*Contest: JXNU_TRAIN_1*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/552292/problem/F*/
/*Start: Tue 24 Sep 2024 03:04:53 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::queue<std::pair<int, int>> q; // type, 1 - bad guies, 0 - self;  vertex
  std::deque<bool> occupied(n);      // 0 - unvisited,  1 - occupied
  for (int i{}; i != g; ++i) {
    int x;
    std::cin >> x;
    q.push({1, --x});
  ...
  int ans{};
  while (!q.empty()) {
    auto const [t, u]{q.front()};
    q.pop();

    if (t == 0 && is_exit[u] && !occupied[u]) {
      ++ans;
    }
    occupied[u] = true;
