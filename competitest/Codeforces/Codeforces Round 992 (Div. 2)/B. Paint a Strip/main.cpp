...
/*Start: Sun 08 Dec 2024 11:06:19 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
    ...
    std::vector<int> producing{0, 1};
    while (producing.back() < 1e5) {
        producing.push_back((producing.back() + 1) * 2);
    }

    while (t--) {
        int n;
        std::cin >> n;
        std::cout << std::ranges::lower_bound(producing, n) - producing.begin()
                  << '\n';
    }
}
} // namespace
