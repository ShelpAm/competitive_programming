...
// Start: Fri 14 Feb 2025 02:03:37 AM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::ranges::for_each(a, dec);
    std::ranges::for_each(b, dec);

    Dsu dsub(n);
    for (auto const &[u, v] : b) {
        dsub.unite(u, v);
    }

    int ans{};
    auto const [first, last]{std::ranges::remove_if(a, [&dsub](auto const &p) {
        return dsub.find(p.first) != dsub.find(p.second);
    })};
    ans += last - first;
    a.erase(first, last);

    Dsu dsua(n);
    for (auto const &[u, v] : a) {
        dsua.unite(u, v);
    }
    for (auto const &[u, v] : b) {
        if (!dsua.united(u, v)) {
