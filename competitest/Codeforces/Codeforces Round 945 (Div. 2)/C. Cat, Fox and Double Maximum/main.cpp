...
// Memory Limit: 256
// Time Limit: 2000
// Start: Fri 24 May 2024 09:46:47 AM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  cin >> a;
  vector<int> ans(n);
  auto k{min_element(a.begin() + 1, a.end() - 1) - a.begin()};
  set<int> avail;
  for (int i{1}; i != n + 1; ++i) {
    avail.insert(i);
  }
  for (int i{1}; i < n - 1; i += 2) {
    ...
    auto const selection{avail.upper_bound(n + 1 - a[i])};
    // debug("n + 1 - a[i]", n + 1 - a[i]);
    // debug("selection", *selection);
    ans[i] = *selection;
    avail.erase(selection);
  }
  for (int i{}; i != n; ++i) {
    if (ans[i] == 0) {
      auto const it{prev(avail.upper_bound(n + 1 - a[i]))};
      ans[i] = *it;
      avail.erase(it);
    }
  }
  ...
  // int cnt{};
  // for (int i{1}; i < n - 1; ++i) {
  //   if (a[i - 1] + ans[i - 1] < a[i] + ans[i] &&
  //       a[i] + ans[i] > a[i + 1] + ans[i + 1]) {
  //     ++cnt;
  //   }
  // }
  // debug("cnt", cnt);
