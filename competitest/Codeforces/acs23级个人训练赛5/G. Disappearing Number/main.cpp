/*Problem: G. Disappearing Number*/
/*Contest: acs23级个人训练赛5*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/558271/problem/G*/
/*Start: Tue 15 Oct 2024 01:47:31 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  auto solve{[&](auto solve, std::string_view d, int x) -> i64 {
    if (d.empty()) {
      return 0;
    }
    auto const s{pow(9ULL, d.size() - 1, inf<i64>)};
    return (d[0] - (x < d[0])) * s + solve(solve, d.substr(1), x);
  }};
  std::cout << solve(solve, n, x) + 1 << '\n';
}
} // namespace
