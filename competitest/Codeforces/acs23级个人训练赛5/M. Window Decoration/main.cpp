/*Problem: M. Window Decoration*/
/*Contest: acs23级个人训练赛5*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/558271/problem/M*/
/*Start: Tue 15 Oct 2024 02:11:18 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::ranges::sort(a);
  auto const r{std::ranges::unique(a)};
  a.erase(r.begin(), r.end());

  double area{static_cast<double>(a.size() * 2)};
  for (int i{}; i != a.size(); ++i) {
    for (int j{}; j != i; ++j) {
      if (auto d{std::abs(a[i].first - a[j].first) +
                 std::abs(a[i].second - a[j].second)};
          d == 1) {
        area -= 0.5;
      }
    }
