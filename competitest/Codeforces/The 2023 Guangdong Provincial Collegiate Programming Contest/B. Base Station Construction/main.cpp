// Problem: B. Base Station Construction
// Contest: The 2023 Guangdong Provincial Collegiate Programming Contest
// Judge: Codeforces
// URL: https://codeforces.com/gym/104369/problem/B
// Memory Limit: 1024
// Time Limit: 2000
// Start: Mon 20 May 2024 06:06:35 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  cin >> m;
  vector<int> L(n + 1);
  vector<vector<int>> left_bounds(n + 2);
  for (int i{}; i != m; ++i) {
    int l, r;
    cin >> l >> r;
    left_bounds[r + 1].push_back(l);
  }
  vector<u64> f(n + 2, inf<u64>);
  f[0] = 0;
  int cnt{};
  deque<u64> q{0};
  for (int i{1}, l{}; i != n + 2; ++i) {
    for (auto const e : left_bounds[i]) {
      if (l < e) {
        --L[e];
        ++cnt;
      }
