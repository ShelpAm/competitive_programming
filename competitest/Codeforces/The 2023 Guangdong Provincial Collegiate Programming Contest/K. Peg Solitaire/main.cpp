// Problem: K. Peg Solitaire
// Contest: The 2023 Guangdong Provincial Collegiate Programming Contest
// Judge: Codeforces
// URL: https://codeforces.com/gym/104369/problem/K
// Memory Limit: 1024
// Time Limit: 1000
// Start: Mon 20 May 2024 06:06:36 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  auto rm{[](vector<pair<int, int>>& a, pair<int, int> const& e) {
    a.erase(find(a.begin(), a.end(), e));
  }};
  auto contains{[](auto const& a, pair<int, int> const& e) {
    return find(a.begin(), a.end(), e) != a.end();
  }};
  int ans{inf<int>};
  function<void(vector<pair<int, int>> const&)> simulate{
      [&](vector<pair<int, int>> const& v) {
        auto valid{[&](auto const& next) {
          return next.first >= 0 && next.first < n && next.second >= 0 &&
                 next.second < m && !contains(v, next);
        }};
        check_min(ans, v.size());
        for (auto const [p, q] : v) {
          for (auto const [x, y] : v) {
            if (abs(x - p) == 1 && q == y) {
              if (pair next{2 * x - p, y}; valid(next)) {
                auto u{v};
                rm(u, {p, q});
                rm(u, {x, y});
                u.push_back(next);
            ...
            else if (abs(y - q) == 1 && p == x) {
              if (pair next{x, 2 * y - q}; valid(next)) {
                auto u{v};
                rm(u, {p, q});
                rm(u, {x, y});
                u.push_back(next);
