...
// Memory Limit: 256
// Time Limit: 2000
// Start: Wed 10 Jul 2024 11:18:57 AM CST
// Author: ShelpAm
...
void solve_case()
{
  ...
  auto go_begin_with{[&](int a) {
    std::vector<int> dist(n); // dist[i] represents distance from a to i
    dist[a] = -1;
    ...
    dfs(dfs, a, a);
    return (n - 1) * 2 - std::ranges::max(dist);
  }};
  auto intermediate{[&] {
    ...
    while (!q.empty()) {
      ...
      for (auto const &[_, v] : g.edges_of(u)) {
        ...
        else if (vis[0][v] && id == 1) {
          return std::pair{t + 1, v};
        }
      }
    }
    // Unreachable
  }};
  auto [initial, c]{intermediate()};
  std::cout << initial + go_begin_with(c) << '\n';
}
} // namespace
