...
// Memory Limit: 256
// Time Limit: 1000
// Start: Mon 27 May 2024 11:12:22 AM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  int ans{};
  int const beg = sqrt(max(d * d - 2 * m - 1, 0)),
            end = sqrt(d * d + 2 * m) + 1;
  vector<vector<int>> f(m + 1, vector<int>(end - beg + 1, -inf<int>));
  f[d][d - beg] = cnt[d];
  check_max(ans, f[d][d - beg]);
  for (int i{d + 1}; i != m + 1; ++i) {
    for (int l = beg; l <= end; ++l) {
      for (auto dist : {-1, 0, 1}) {
        dist += l;
        auto const from{i - l};
        if (from >= 0 && dist >= beg && dist <= end) {
          if (check_max(f[i][l - beg], f[from][dist - beg] + cnt[i])) {
            check_max(ans, f[i][l - beg]);
          }
        }
      }
    }
