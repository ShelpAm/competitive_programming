...
// Contest: European Championship 2024 - Online Mirror (Unrated, ICPC Rules,
// Teams Preferred) Judge: Codeforces URL:
// https://codeforces.com/problemset/problem/1949/B Memory Limit: 256 Time
// Limit: 2000 Start: Mon 06 May 2024 08:01:09 PM CST Author: ShelpAm
...
auto solve_case()
{
  ...
  for (int t{}; t != n; ++t) {
    int m{inf<int>};
    for (int i{}; i != n - t; ++i) {
      check_min(m, abs(a[i] - b[i + t]));
    }
    for (int i{n - t}; i != n; ++i) {
      check_min(m, abs(a[i] - b[i - (n - t)]));
    }
    // debug("t", t);
    // debug("m", m);
    check_max(ans, m);
