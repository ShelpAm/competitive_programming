...
auto toposort(Graph const &g, std::deque<bool> const &is_src, bool directed)
    -> Toposort_result
{
...
void solve_case()
{
    ...
    for (auto const &[_, u, v] :
         std::views::iota(0, 31) | std::views::transform([&](auto i) {
             return std::tuple{i, transform(a, i), transform(b, i)};
         })) {
        graph::Graph g(n + m);

        std::deque<bool> is_src(n + m);
        for (int i{}; i != n; ++i) {
            for (int j{}; j != m; ++j) {
                if (u[i][j] && !v[i][j]) {
                    is_src[i] = true;
