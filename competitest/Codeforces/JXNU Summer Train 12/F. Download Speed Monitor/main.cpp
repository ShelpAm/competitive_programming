...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/542262/problem/F*/
/*Start: Sun 11 Aug 2024 06:48:57 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  constexpr std::string_view kibps{"KiBps"};
  constexpr std::string_view mibps{"MiBps"};
  ...
  for (int i{}; i != n; ++i) {
    s += a[i];
    if (i >= k - 1) {
      double spd{static_cast<double>(s) / k};
      std::string_view unit{kibps};
      if (spd >= 1024) {
        spd /= 1024;
        unit = mibps;
      }
