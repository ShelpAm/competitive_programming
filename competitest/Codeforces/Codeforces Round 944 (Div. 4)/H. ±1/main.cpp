...
// Memory Limit: 256
// Time Limit: 2000
// Start: Fri 24 May 2024 12:38:26 PM CST
// Author: ShelpAm
...
{
  using namespace std;
  constexpr auto max_n{500};
  int n;
  cin >> n;
  vector<vector<int>> a(3, vector<int>(n));
  vector<unordered_set<pair<bitset<max_n>, bitset<max_n>>>> f(n + 1);
  f[0].insert({bitset<max_n>{0}, bitset<max_n>{0}});
  for (int i{1}; i != n + 1; ++i) {
    for (auto [mask, used] : f[i - 1]) {
      for (int j{}; j != 1 << 3; ++j) {
        auto ok{[&] {
          for (int k{}; k != 3; ++k) {
            if (used.test(a[k][i]) && (j >> k & 1) != mask.test(a[k][i])) {
              return false;
            }
          }
          return true;
