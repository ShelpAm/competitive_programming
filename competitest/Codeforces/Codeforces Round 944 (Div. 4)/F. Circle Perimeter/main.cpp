...
// Memory Limit: 256
// Time Limit: 1000
// Start: Fri 24 May 2024 10:41:50 AM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  cin >> r;
  auto calc{[&](u64 x) {
    auto lo{static_cast<u64>(sqrt(r * r - x * x))},
        hi{static_cast<u64>(sqrt((r + 1) * (r + 1) - x * x) + 1)};
    while (r * r > x * x + lo * lo) {
      ++lo;
    }
    while ((r + 1) * (r + 1) <= x * x + hi * hi) {
      --hi;
    }
    return 2 * (hi - lo + 1) - (lo == 0 ? 1 : 0);
  }};
  u64 ans{};
  for (int i = 0; i != r + 1; ++i) {
    ans += 2 * calc(i);
