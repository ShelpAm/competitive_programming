...
/*Start: Fri 27 Sep 2024 09:42:39 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  auto segs{[&](auto k) {
    i64 r{x.end() - std::ranges::lower_bound(x, k)};
    i64 l{std::prev(std::ranges::upper_bound(x, k)) - x.begin() + 1};
    auto ans{r * l};
    if (std::ranges::binary_search(x, k)) {
      --ans;
    }
    return ans;
  }};
  ...
  for (int i{}; i != x.size() - 1; ++i) {
    ++ans[segs(x[i])];
    if (x[i + 1] != x[i] + 1) {
      ans[segs(x[i] + 1)] += x[i + 1] - x[i] - 1;
    }
  }
  ++ans[n - 1]; // the rightmost point
