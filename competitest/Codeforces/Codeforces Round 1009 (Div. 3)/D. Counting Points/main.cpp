...
// Start: Wed 12 Mar 2025 06:07:41 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto get_y{[](std::pair<i64, i64> cir, i64 x) {
        return binary_search(
            [&](auto y) {
                return y * y + (cir.first - x) * (cir.first - x) <=
                       cir.second * cir.second;
            },
            0, cir.second + 1);
    }};

    std::map<int, int> h;
    for (auto const &[x, r] : a) {
        for (int i{x - r}; i != x + r + 1; ++i) {
            chmax(h[i], get_y({x, r}, i));
        }
    }
