...
// Memory Limit: 256
// Time Limit: 3000
// Start: Tue 21 May 2024 04:40:09 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  for (int i{}; i != m; ++i) {
    ...
    cin >> c >> k;
    int& cnt{players[i % 2]};
    map<int, set<int>>*from{&row}, *other{&col};
    vector<map<int, set<int>>::iterator> its;
    if (c == 'U') {
      u += k;
      for (auto it{from->begin()}; it != from->lower_bound(u); ++it) {
        cnt += it->second.size();
        for (auto const e : it->second) {
          (*other)[e].erase(it->first);
        }
        its.push_back(it);
      }
    }
      ...
      d -= k;
      for (auto it{from->upper_bound(d)}; it != from->end(); ++it) {
        cnt += it->second.size();
        for (auto const e : it->second) {
          (*other)[e].erase(it->first);
        }
      ...
      swap(from, other);
      l += k;
      for (auto it{from->begin()}; it != from->lower_bound(l); ++it) {
        cnt += it->second.size();
        for (auto const e : it->second) {
          (*other)[e].erase(it->first);
        }
      ...
      swap(from, other);
      r -= k;
      for (auto it{from->upper_bound(r)}; it != from->end(); ++it) {
        cnt += it->second.size();
        for (auto const e : it->second) {
          (*other)[e].erase(it->first);
        }
