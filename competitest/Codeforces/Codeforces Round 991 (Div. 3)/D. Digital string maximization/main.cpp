...
/*Start: Sun 08 Dec 2024 02:53:42 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
    ...
    for (int i{}; i != s.size() - 1; ++i) {
        int off{};
        bool selected{};
        for (int j{1}; i + j != s.size() && j != 10; ++j) {
            if (auto const neo{s[i + j] - j};
                neo >= '0' && neo > s[i + off] - off) {
                off = j;
                selected = true;
            }
        }
        if (selected) {
            s[i + off] -= off;
            std::ranges::rotate(s.begin() + i, s.begin() + i + off,
                                s.begin() + i + off + 1);
        }
    }
