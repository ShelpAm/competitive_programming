...
/*Start: Fri 06 Dec 2024 01:47:28 AM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
    ...
    std::vector<std::vector<int>> sep(2);
    for (int i{}; i != n; ++i) {
        sep[i % 2].push_back(a[i]);
    }

    auto solve{[](auto const &a) -> int {
        return sum_of(a) % a.size() == 0 ? sum_of(a) / a.size() : -1;
    }};

    auto x = solve(sep[0]);
    auto y = solve(sep[1]);
    std::cout << (x == y && x != -1 ? "YES" : "NO") << '\n';
}
} // namespace
