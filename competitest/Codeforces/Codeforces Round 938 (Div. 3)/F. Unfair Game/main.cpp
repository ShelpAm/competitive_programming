...
// Memory Limit: 256
// Time Limit: 2000
// Start: Mon 29 Apr 2024 05:13:30 PM CST
// Author: ShelpAm
...
auto solve_case() {
  ...
  for (int i{}; i != N; ++i) {
    for (int j{}; j != N; ++j) {
      for (int k{}; k != N; ++k) {
        bool thiswin{(i + j) % 2 == 0 && (i + k) % 2 == 0};
        if (i >= 1) {
          check_max(f[i][j][k], f[i - 1][j][k] + thiswin);
        }
        if (j >= 1) {
          check_max(f[i][j][k], f[i][j - 1][k] + thiswin);
        }
        if (k >= 1) {
          check_max(f[i][j][k], f[i][j][k - 1] + thiswin);
        }
      }
    }
  }
