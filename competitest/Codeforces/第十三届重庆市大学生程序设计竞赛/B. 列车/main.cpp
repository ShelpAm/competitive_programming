...
// Problem: B. 列车
// Contest: 第十三届重庆市大学生程序设计竞赛
// Judge: Codeforces
// URL: https://codeforces.com/gym/105887/problem/B
// Start: Thu 15 May 2025 04:08:18 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::priority_queue<std::pair<int, i64>, std::vector<std::pair<int, i64>>,
                        std::greater<>>
        q;
    q.push({1, inf<i64>});
    for (auto [l, r, c] : a) {
        if (q.empty() || q.top().first > r) { // Useless bus.
            continue;
        }

        i64 on_board{};
        while (on_board < c && !q.empty()) {
            auto [R, C] = q.top();
            q.pop();
