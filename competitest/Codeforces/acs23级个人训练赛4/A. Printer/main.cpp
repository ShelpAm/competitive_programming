/*Problem: A. Printer*/
/*Contest: acs23级个人训练赛4*/
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/557560/problem/A*/
/*Start: Sat 12 Oct 2024 01:14:01 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::cout << binary_search(
                   [&](__int128_t s) {
                     __int128_t c{};
                     for (auto const &[t, l, w] : a) {
                       __int128_t const blocks{(s + w) / (t * l + w)};
                       __int128_t const left{(s + w) % (t * l + w)};
                       c += blocks * l;
                       c += std::max<__int128_t>((left - w) / t, 0LL);
                     }
                     debug("c", std::bitset<128>(c));
                     return c >= k;
                   },
                   1e18, -1)
            << '\n';
