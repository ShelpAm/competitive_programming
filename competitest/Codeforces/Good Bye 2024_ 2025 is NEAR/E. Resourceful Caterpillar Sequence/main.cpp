...
void solve_case()
{
    ...
    std::vector<int> n_gt_0(n);
    std::vector<int> n_gt_1(n);
    auto init{[&](this auto init, int u, int p) -> void {
        n_gt_0[u] = dist[u] > 0 ? 1 : 0;
        n_gt_1[u] = dist[u] > 1 ? 1 : 0;
        for (auto const &[_, v] : g.edges_of(u)) {
            if (v != p) {
                init(v, u);
                n_gt_0[u] += n_gt_0[v];
    ...
    auto dfs{[&](this auto dfs, int u, int p) -> void {
        for (auto const &[_, v] : g.edges_of(u)) {
            if (v != p) {
                ...
                if (dist[v] == 0) {
                    ans += n_gt_0[0] - n_gt_0[v];
                }
                else if (dist[u] == 1) {
                    ans += n_gt_1[0] - n_gt_1[v];
                }
                dfs(v, u);
            }
        }
