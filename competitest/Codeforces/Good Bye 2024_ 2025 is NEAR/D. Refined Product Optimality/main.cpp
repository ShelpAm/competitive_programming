...
// Start: Wed 12 Feb 2025 06:38:33 PM CST
// Author: ShelpAm
...
template <std::int_least64_t mod> class Z {
  public:
    constexpr Z(std::int_least64_t n) : n_{n} {}
    ...
    constexpr Z &operator/=(Z rhs)
    {
        n_ *= modular_inverse(rhs.n_, mod); // modular_inverse
        n_ %= mod;
        return *this;
    }

    constexpr operator std::int_least64_t()
    {
        return n_;
    }

  private:
    std::int_least64_t n_; // n_ is guarenteed to be normalized.
};
void solve_case()
{
    int n, q;
    ...
    std::ranges::for_each(a, [](auto &e) { std::ranges::sort(e); });

    Z<mod998244353> product{1};
    for (auto const &[x, y] : std::views::zip(a[0], a[1])) {
        (product *= std::min(x, y));
    }
    std::cout << product << ' ';
    ...
    for (int i{}; i != q; ++i) {
        ...
        auto const v{b[--o][--x]++};
        auto const pos{std::ranges::upper_bound(a[o], v) - a[o].begin() - 1};

        product /= std::min(a[o][pos], a[o ^ 1][pos]);
        product *= std::min(++a[o][pos], a[o ^ 1][pos]);
