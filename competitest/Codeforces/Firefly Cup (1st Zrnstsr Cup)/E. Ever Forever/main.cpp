...
// Problem: E. Ever Forever
// Contest: Firefly Cup (1st Zrnstsr Cup)
// Judge: Codeforces
// URL: https://codeforces.com/group/BJlsDCvlJO/contest/586547/problem/E
// Start: Fri 07 Feb 2025 03:10:17 PM CST
// Author: ShelpAm
...
void solve_case()
{
    constexpr auto m{static_cast<std::size_t>(1e6 + 1)};
    i64 ans{};
    int t{1};
    std::vector<std::unordered_map<char, int>> next(m);
    std::vector<int> size(m);
    std::vector<int> endings(m);
    auto insert{[&](std::string_view s) {
        int cur{};
        for (auto const e : s) {
            if (!next[cur].contains(e)) {
                next[cur][e] = t++;
            }
            debug("next[cur][e]", next[cur][e]);
            cur = next[cur][e];
            ++size[cur];
            ans += endings[cur];
        }
