...
// Judge: Codeforces
// URL: https://codeforces.com/group/BJlsDCvlJO/contest/586547/problem/I
// Start: Fri 07 Feb 2025 01:20:02 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    // auto common_prefix{[](auto &&s, auto &&t) {
    //     auto const m{std::min(s.size(), t.size())};
    //     int n{};
    //     for (int i{}; i != m; ++i) {
    //         if (s[i] != t[i]) {
    //             break;
    ...
    auto ans{(s.size() + 1) * (t.size() + 1) - 1};
    for (auto const e : std::views::iota('a', 'z' + 1)) {
        ans -= std::ranges::count(s, e) * std::ranges::count(t, e);
    }
