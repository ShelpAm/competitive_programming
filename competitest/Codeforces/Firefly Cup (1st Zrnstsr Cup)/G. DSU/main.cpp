...
// Judge: Codeforces
// URL: https://codeforces.com/group/BJlsDCvlJO/contest/586547/problem/G
// Start: Fri 07 Feb 2025 04:09:47 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto result{shelpam::graph::toposort(g, true)};
    if (!result.acyclic) {
        std::cout << -1 << '\n';
        return;
    };

    std::cout << result.order.size() << '\n';
    for (auto const e : result.order) {
        std::cout << e << ' ' << a[e] << '\n';
    }
}
} // namespace
