...
// Problem: D. Greedy Counting
// Contest: Firefly Cup (1st Zrnstsr Cup)
// Judge: Codeforces
// URL: https://codeforces.com/group/BJlsDCvlJO/contest/586547/problem/D
// Start: Fri 07 Feb 2025 01:44:53 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    for (int i{n - 1}; i != -1; --i) {
        ...
        // Only find tails
        auto dfs{[&](auto dfs, int u, int p, int len, i64 sum) -> void {
            visited[u] = 1;
            len += p - u;
            sum += len;
            debug("u, p, len, sum", std::tuple{u, p, len, sum});
            ans += sum;
    ...
    //         std::vector<int> b(a.begin() + _i, a.begin() + _j + 1);
    //         int i = 0;
    //         int ans = 1;
    //         while (true) {
    //             auto const j{std::ranges::find_if(
    //                              b.begin() + i + 1, b.end(),
    //                              [&b, i](auto bj) { return bj > b[i]; }) -
    //                          b.begin()};
    //             if (j == b.size()) {
    //                 break;
    //             }
