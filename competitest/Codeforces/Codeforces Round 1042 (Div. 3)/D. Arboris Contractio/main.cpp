...
// Start: Mon 11 Aug 2025 02:19:02 AM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    int n;
    std::cin >> n;
    auto g = graph::Graph::read(n, n - 1, false, false);

    std::vector<int> pa(n, -1);
    std::vector<int> sons(n);
    for (int u{}; u != n; ++u) {
        if (g.edges_of(u).size() == 1) {
            auto v = g.edges_of(u).front().second;
            pa[u] = v;
            ++sons[v];
        }
    }
    ...
    int init{};
    for (int i{1}; i != n; ++i) {
        init += g.edges_of(i).size() == 1 && pa[i] != 0;
    }
    debug("init", init);

    int ans{inf<int>};
    auto dfs = [&](auto self, int u, int p, int cur) -> void {
        chmin(ans, cur);
        for (auto [_, v] : g.edges_of(u)) {
            if (v != p) {
                self(self, v, u,
                     cur + sons[u] - (pa[v] == u) - sons[v] + (pa[u] == v));
            }
        }
    };
