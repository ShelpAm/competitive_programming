...
// Start: Sat 01 Mar 2025 07:39:00 PM CST
// Author: ShelpAm
...
void solve_case()
{
    auto is_square{[](auto x) {
        auto k = binary_search([x](auto k) { return k * k <= x; }, 0, x + 1);
        return k * k == x;
    }};
    ...
    std::ranges::iota(a, 0);
    std::vector<i64> s;
    std::partial_sum(a.begin(), a.end(), std::back_inserter(s));
