...
// Start: Mon 03 Feb 2025 12:04:57 AM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    int m1;
    std::cin >> m1;
    auto const g{Graph::read(n, m1, false, false)};
    int m2;
    std::cin >> m2;
    auto const h{Graph::read(n, m2, false, false)};

    --s1, --s2;

    debug("start", std::pair{s1, s2});

    Graph f(n * n);

    auto comp{[n](int u1, int u2) { return u1 * n + u2; }};
    auto decomp{[n](int u) { return std::pair{u / n, u % n}; }};

    for (int u1{}; u1 != n; ++u1) {
        for (int u2{}; u2 != n; ++u2) {
            for (auto const &[_, v1] : g.edges_of(u1)) {
                for (auto const &[_, v2] : h.edges_of(u2)) {
                    f.add_edge(comp(u1, u2), comp(v1, v2), std::abs(v1 - v2));
                }
            }
        }
    }
    ...
    i64 ans{max<i64>};
    auto const res{dijkstra(f, comp(s1, s2))};
    std::ranges::for_each(std::views::iota(0, n), [&](auto const i) {
        if (std::ranges::any_of(f.edges_of(comp(i, i)), [&](auto const &p) {
                auto [v1, v2]{decomp(p.second)};
                return v1 == v2 && i != v1;
            })) {
            chmin(ans, res.distance[comp(i, i)]);
        }
    });

    std::cout << (ans < infinity ? ans : -1) << '\n';
}
} // namespace
