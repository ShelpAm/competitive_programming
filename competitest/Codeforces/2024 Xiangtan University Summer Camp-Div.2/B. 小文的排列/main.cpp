...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/105323/problem/B*/
/*Start: Fri 30 Aug 2024 01:55:32 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  if (std::ranges::any_of(a, [m](auto e) { return e > m; })) {
    std::cout << "NO\n";
    return;
  }
  ...
  std::vector<int> patch(m - 1, 0);
  a.insert(a.begin(), patch.begin(), patch.end());
  a.insert(a.end(), patch.begin(), patch.end());
  int over{};
  for (int i{}; i != m; ++i) {
    if (++cnt[a[i]] == 2 && a[i] != 0) {
      ++over;
  ...
  for (int i{};; ++i) {
    ...
    if (++cnt[a[i + m]] == 2 && a[i + m] != 0) {
      ++over;
    }
  }
  for (int i{n + m - 1}; i != a.size(); ++i) {
    ok[i] = 1;
  }
  debug("a ", a);
  debug("ok", ok);
