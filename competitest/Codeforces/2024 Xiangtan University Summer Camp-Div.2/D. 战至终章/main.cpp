...
/*Judge: Codeforces*/
/*URL: https://codeforces.com/gym/105323/problem/D*/
/*Start: Fri 30 Aug 2024 01:36:42 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  std::vector<std::set<int>> prerequisite(n);
  ...
  while (!q.empty() && q.top().first <= p) {
    ...
    ans.push_back(u);
    for (auto v : next[u]) {
      prerequisite[v].erase(u);
      if (prerequisite[v].empty()) {
        q.push({a[v], v});
      }
    }
  }
