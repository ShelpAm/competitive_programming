...
// Judge: Codeforces
// URL: https://codeforces.com/gym/536487/problem/F
// Memory Limit: 256
// Time Limit: 2000
...
void solve_case()
{
  ...
  list<int> latest_messages;
  unordered_map<int, pair<list<int>::iterator, list<int>>> users;
  vector<pair<int, int>> kick_list;

  auto kick{[&](int time, int person) {
    latest_messages.erase(users[person].first);
    users.erase(person);
    kick_list.push_back({time, person});
  }};

  for (int i{}; i != m; ++i) {
    int t, p;
    cin >> t >> p;
    auto it{latest_messages.insert(latest_messages.end(), p)};
    if (users.contains(p)) {
      latest_messages.erase(exchange(users[p].first, it));
      users[p].second.push_back(t);
    }
    else {
      if (users.size() == n) {
        kick(t, latest_messages.front());
      }
      users.insert({p, {it, list<int>{t}}});
    }
    auto &talks_time{users[p].second};
    while (!talks_time.empty() && talks_time.front() < t - T) {
      talks_time.pop_front();
    }
    if (talks_time.size() == K) {
      kick(t, p);
    }
  }

  cout << kick_list.size() << ' ' << users.size() << '\n';
  for (auto const &[t, p] : kick_list) {
    cout << t << ' ' << p << '\n';
  }
  for (auto const &[p, _] : users) {
    cout << p << ' ';
  }
}
} // namespace
