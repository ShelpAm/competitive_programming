// Problem: D. Concrete Painting
// Contest: JXNU Summer Train 1
// Judge: Codeforces
// URL: https://codeforces.com/gym/536487/problem/D
// Memory Limit: 256
// Time Limit: 2000
...
#define int u64
  ...
  vector<int> ends{_ends.begin(), _ends.end()};
  int cur_k{start[ends.front()] - close[ends.front()]};
  for (int i{1}; i != ends.size(); ++i) {
    auto end{ends[i]};
    blocks.push_back({ends[i - 1], end, cur_k});
    cur_k -= close[end];
    cur_k += start[end];
  }

  u64 ans{};
  for (auto const &[l, r, k] : blocks) {
    ans += (pow(2ULL, k, mod998244353) - 1) * pow(2ULL, n - k, mod998244353) %
           mod998244353 * (r - l) % mod998244353;
    ans %= mod998244353;
  }
  cout << ans;
}
