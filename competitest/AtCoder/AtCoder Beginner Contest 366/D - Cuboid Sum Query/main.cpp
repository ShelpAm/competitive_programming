...
/*Start: Sat 10 Aug 2024 10:04:07 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  for (int i{}; i != n; ++i) {
    for (int j{}; j != n; ++j) {
      for (int k{}; k != n; ++k) {
        std::cin >> a[i + 1][j + 1][k + 1];
        a[i + 1][j + 1][k + 1] += a[i][j + 1][k + 1] + a[i + 1][j][k + 1] +
                                  a[i + 1][j + 1][k] - a[i + 1][j][k] -
                                  a[i][j + 1][k] - a[i][j][k + 1] + a[i][j][k];
        /*std::cerr << a[i + 1][j + 1][k + 1] << '\n';*/
      }
    }
  }
  int q;
  ...
  for (int i{}; i != q; ++i) {
    ...
    auto const ans{a[rx][ry][rz] - a[lx - 1][ry][rz] - a[rx][ly - 1][rz] -
                   a[rx][ry][lz - 1] + a[rx][ly - 1][lz - 1] +
                   a[lx - 1][ry][lz - 1] + a[lx - 1][ly - 1][rz] -
                   a[lx - 1][ly - 1][lz - 1]};
    std::cout << ans << '\n';
  }
}
