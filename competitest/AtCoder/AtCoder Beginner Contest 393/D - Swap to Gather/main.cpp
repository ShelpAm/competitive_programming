...
// Start: Sat 15 Feb 2025 08:14:22 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto const m{static_cast<i64>(a.size())};
    a.insert(a.begin(), 0);
    ...
    auto square{[](i64 n) { return n * (n + 1) / 2; }};
    i64 ans{inf<i64>};
    for (int i{1}; i != m + 1; ++i) {
        chmin(ans, pref[m] - pref[i] - pref[i - 1] + ((2 * i - m - 1) * a[i]) -
                       square(i - 1) - square(m - i));
    }
    std::cout << ans << '\n';
}
} // namespace
