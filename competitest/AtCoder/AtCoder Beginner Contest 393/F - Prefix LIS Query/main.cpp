...
// Contest: AtCoder Beginner Contest 393
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc393/tasks/abc393_f
// Start: Sat 15 Feb 2025 10:00:47 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<int> f(n, max<int>);
    for (int i{}; i != n; ++i) {
        auto const &[e, qs]{std::tuple{a[i], queries[i]}};
        *std::ranges::lower_bound(f, e) = e;
        for (auto const &[id, x] : qs) {
            ans[id] = std::ranges::upper_bound(f, x) - f.begin();
        }
    }
