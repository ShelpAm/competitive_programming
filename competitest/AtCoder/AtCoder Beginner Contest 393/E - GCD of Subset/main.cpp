...
// Start: Sat 15 Feb 2025 09:04:16 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    constexpr auto N{static_cast<int>(1e6)};
    ...
    for (int i{1}; i != N + 1; ++i) {
        ...
        if (auto const root{
                binary_search([k](auto x) { return x * x <= k; }, 0, k + 1)};
            root * root == k) {
            factor_o[root] -= v;
        }
    }

    std::array<int, N + 1> ans{};
    for (int i{1}; i != N + 1; ++i) {
        if (factor_o[i] < k) {
            continue;
        }
