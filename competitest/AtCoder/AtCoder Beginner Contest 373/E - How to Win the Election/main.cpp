...
/*Start: Sat 28 Sep 2024 08:26:54 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  for (int rank{}; auto const &[e, v] : a) {
    auto check{[&](__int128 x) {
      auto const greaterthan{b.end() - std::ranges::upper_bound(b, e + x)};
      if (greaterthan >= m) {
        return false;
      }
      bool const contains_self{rank >= n - m};
      auto const sumof_lessthan{contains_self
                                    ? s[n - greaterthan] - s[n - m - 1] - e
                                    : s[n - greaterthan] - s[n - m]};
      return (e + x + 1) * (m - greaterthan) > sumof_lessthan + k - s[n] - x;
    }};
    auto const x{binary_search(check, k - s[n], -1, false)};
    ans[v] = check(x) ? x : -1;
    ++rank;
  }
