...
void solve_case()
{
  ...
  std::vector<std::vector<i64>> items(w + 1);
  for (int i{}; i != n; ++i) {
    int u;
    i64 v;
    std::cin >> u >> v;
  ...
  for (int u{1}; u != w + 1; ++u) {
    ...
    for (int j{w}; j >= u; --j) {
      std::vector<int> used(items[u].size());
      std::priority_queue<std::pair<i64, int>> q;
      for (int idx{}; auto const v : items[u]) {
        q.push({v - 2 * used[idx] - 1, idx});
        ++idx;
      }
      i64 cur{};
      for (int k{1}; j - k * u >= 0; ++k) {
        auto const [delta, idx]{q.top()};
        q.pop();
        ++used[idx];
        cur += delta;
        chmax(f[j], f[j - k * u] + cur);
        q.push({items[u][idx] - 2 * used[idx] - 1, idx});
      }
    }
  }
