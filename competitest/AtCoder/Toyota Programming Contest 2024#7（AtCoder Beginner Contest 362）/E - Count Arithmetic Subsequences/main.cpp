...
// Contest: Toyota Programming Contest 2024#7（AtCoder Beginner Contest 362）
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc362/tasks/abc362_e
// Memory Limit: 1024
// Time Limit: 2000
// Start: Sat 13 Jul 2024 09:01:01 PM CST
// Author: ShelpAm
...
void solve_case()
{
  ...
  vvvvu f(n + 1, vvvu(n, vvu(n, vu(n + 1))));
  for (int k{}; k != n; ++k) {
    for (int i{}; i != n; ++i) {
      for (int j{i}; j != n; ++j) {
        f[k][i][j][0] = 1;
  ...
  for (int i{1}; i != n + 1; ++i) {
    for (int j{}; j != n; ++j) {
      for (int k{j}; k != n; ++k) {
        for (int l{1}; l != n + 1; ++l) {
          auto begin{ranges::lower_bound(a, a[i - 1] - (a[k] - a[j])) -
                     a.begin()};
          auto end{ranges::upper_bound(a, a[i - 1] - (a[k] - a[j])) -
                   a.begin()};
          if (i == 1 && j == 0 && k == 0 && l == 1) {
            debug("begin end", vector{begin, end});
          }
          for (auto it{begin}; it != end; ++it) {
            (f[i][j][k][l] += f[it + 1][j][k][l - 1]) %= mod998244353;
            if (i == 1 && j == 0 && k == 0 && l == 1) {
              debug("f[from]", f[it + 1][j][k][l - 1]);
            }
          }
          (ans[l] += f[i][j][k][l]) %= mod998244353;
        }
      }
    }
  }
