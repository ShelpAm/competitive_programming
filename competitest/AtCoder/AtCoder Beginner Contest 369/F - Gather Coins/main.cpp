...
/*Start: Sun 01 Sep 2024 12:58:55 AM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  std::map<int, int> f, y_to_i;
  f[1] = 0;
  y_to_i[1] = n;
  for (auto [x, y, i] : a) {
    auto it{std::prev(f.upper_bound(y))};
    auto neo{it->second + 1};
    if (chmax(f[y], neo)) {
      prev[i] = y_to_i[it->first];
      y_to_i[y] = i;
      it = f.upper_bound(y);
      while (it != f.end() && it->second <= neo) {
        it = f.erase(it);
      }
    }
  }
  ...
  debug("p", p);
  int x{1}, y{1};
  std::ranges::sort(a, [](auto l, auto r) { return std::get<2>(l) < std::get<2>(r); });
  std::vector<char> ans;
  for (auto i : p) {
