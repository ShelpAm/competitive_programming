...
// Start: Sun 10 Aug 2025 07:35:25 PM CST
// Author: ShelpAm
...
template <typename T> constexpr T pow(T base, auto exp, auto p)
{
...
void solve_case()
{
    ...
    using V2 = Vector2d<i64>;
    int n;
    std::cin >> n;
    std::vector<V2> a(n);
    std::cin >> a;

    auto hash = [](V2 l) { return (l.x() ^ 0x0fffefff) + l.y(); };

    std::unordered_map<V2, int, decltype(hash)> slopes;
    std::unordered_map<V2, int, decltype(hash)> center;
    i64 s{};
    for (int i{}; i != n; ++i) {
        auto p1 = a[i];
        for (int j{i + 1}; j != n; ++j) {
            ...
            ++center[p1 + p2];
            auto d = p1 - p2;
            auto gcd = std::gcd(d.x(), d.y());
            d /= gcd;
            if (d.y() < 0 || d.y() == 0 && d.x() == -1) {
                d *= -1;
            }
            ++slopes[d];
        }
    ...
    for (auto [_, c] : slopes) {
        s += c * (c - 1) / 2;
        debug("slopes _, c", std::tuple{_.x(), _.y(), c});
    }
    for (auto [_, c] : center) {
        s -= c * (c - 1) / 2;
        debug("center _, c", std::tuple{_.x(), _.y(), c});
    }
