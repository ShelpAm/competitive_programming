...
// Start: Sat 05 Apr 2025 09:14:57 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    for (int i{}; i != q; ++i) {
        ...
        i64 ans{};
        auto r = binary_search([n](auto x) { return x * x <= n; }, 0,
                               std::sqrt(n) + 2);
        auto rr = binary_search([r](auto x) { return x * x <= r; }, 0,
                                std::sqrt(r) + 2);
        for (auto [a, p] : v) {
            if (a > rr) {
                break;
            ...
            // a * b <= r;
            // auto it = --s.upper_bound({r / a, inf<int>});
            auto it = --std::ranges::upper_bound(v, std::pair{r / a, inf<int>});
            if (it->second == p) {
                --it;
            }
            auto [b, pp] = *it;
