...
// Start: Sat 05 Apr 2025 08:07:58 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<u64> pref(64); // pref[i] = sum of lsb of 1~2^i
    for (int i{1}; i != pref.size(); ++i) {
        pref[i] = pref[i - 1] * 2 + 1;
    }
    debug("pref", pref);
    auto prefix_sum_lsb = [&pref](auto x) {
        u64 res{};
        for (auto i = pref.size() - 1; i != -1; --i) {
            if (x >= 1ULL << i) {
                x -= 1ULL << i;
                res += pref[i];
            }
    ...
    for (int a{1}; a != 64; ++a) {
        auto t = n >> a;
        auto r = binary_search([&](auto x) { return x * x <= t; }, 0,
                               std::sqrt(t) + 1);
        ans += (r + 1) / 2;
    }
    std::cout << ans << '\n';
