...
// Start: Sat 12 Apr 2025 08:52:44 PM CST
// Author: ShelpAm
...
void solve_case()
{
    int n, m;
    std::cin >> n >> m;
    auto g = graph::Graph::read(n + 1, m, false, false, false);
    g.add_edge(0, 1, 1);

    std::set<int> s{1}; // reachable set
    Disjoint_set_union dsu(n + 1);

    for (int k{1}; k != n + 1; ++k) {
        for (auto [_, v] : g.edges_of(k)) {
            if (v <= k) {
                dsu.unite(k, v);
            }
            else {
        ...
        if (dsu.size(1) == k) {
            if (s.contains(k)) {
                ...
                // And find those v in s, remove them from s.
