...
// Start: Sat 12 Apr 2025 08:25:28 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    // Try to fill o as much as possible. And check if the string is fixed.
    auto x = t;
    for (int i{}; i != n; ++i) {
        if (x[i] == '?') {
            if ((i - 1 < 0 || x[i - 1] != 'o') &&
    ...
    // There are other possibilities where some brick can only be '.'
    // And when this happens, it applies for all ? bricks
    if (std::ranges::count(s, 'o') == k) {
        for (auto &e : s) {
            if (e == '?') {
                e = '.';
            }
    ...
    if (std::ranges::count(x, 'o') > k) { // There can be some not o (not fixed)
        std::cout << t << '\n';
    }
    else { // Only some places can vary in a little range.
        // For those range whose size is even, they can vary a little.
        // Otherwise, odds are fixed.

        // So reset in x whose size is even.
        int len{};
        for (int i{}; i != n + 1; ++i) {
            if (i == n || t[i] != '?') {
                // process i - len .. i - 1
