// Problem: F - Distance Component Size Query
// Contest: AtCoder Beginner Contest 356
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc356/tasks/abc356_f
// Memory Limit: 1024
// Time Limit: 2000
// Start: Sat 01 Jun 2024 08:02:10 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  int it{};
  dsu dsu(q * 2);
  map<i64, int> id_of_value;
  map<i64, int> proxy_of_value;
  for (int i{}; i != q; ++i) {
    int op;
    i64 x;
    cin >> op >> x;
    if (op == 1) {
      s.insert(op);
      if (!id_of_value.contains(x)) {
        id_of_value[x] = it++;
      }
      else {
        dsu.unite(it++, id_of_value[x]);
      }
    }
    else {
      cout << dsu.size(id_of_value[x]) << '\n';
    }
  }
}
