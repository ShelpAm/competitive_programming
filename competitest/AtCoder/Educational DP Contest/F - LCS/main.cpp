...
// Memory Limit: 1024
// Time Limit: 2000
// Start: Thu 30 May 2024 04:55:47 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  vector<vector<pair<int, int>>> prev(s.size() + 1,
                                      vector<pair<int, int>>(t.size() + 1));
  pair<int, int> a;
  auto& [p, q] = a;
  for (int i{1}; i <= s.size(); ++i) {
    for (int j{1}; j <= t.size(); ++j) {
      if (s[i - 1] == t[j - 1]) {
        if (check_max(f[i][j], f[i - 1][j - 1] + 1)) {
          prev[i][j] = {i - 1, j - 1};
        }
      }
      else {
        if (check_max(f[i][j], max(f[i][j - 1], f[i - 1][j]))) {
          prev[i][j] =
              f[i][j - 1] >= f[i - 1][j] ? pair{i, j - 1} : pair{i - 1, j};
        }
      }
      if (f[i][j] > f[p][q]) {
        p = i;
