...
/*Contest: Toyota Programming Contest 2024#8（AtCoder Beginner Contest 365）*/
/*Judge: AtCoder*/
/*URL: https://atcoder.jp/contests/abc365/tasks/abc365_d*/
/*Start: Sat 03 Aug 2024 08:07:13 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  auto beat{[](int const i, int const j) { return (i - j + 3) % 3 == 1; }};
  auto to_int{[](char const c) {
    if (c == 'R') {
      return 0;
    }
    if (c == 'P') {
  ...
  std::string s;
  std::cin >> s;
  std::vector<std::vector<int>> f(n + 1, std::vector<int>(3)); // R P S
  for (int i{1}; i != n + 1; ++i) {
    for (int j{}; j != 3; ++j) {
      for (int k{}; k != 3; ++k) {
        if (k != j &&
            (beat(j, to_int(s[i - 1])) || !beat(to_int(s[i - 1]), j))) {
          chmax(f[i][j], f[i - 1][k] + beat(j, to_int(s[i - 1])));
        }
      }
    }
  }
  std::cout << *std::max_element(f[n].begin(), f[n].end());
}
} // namespace
