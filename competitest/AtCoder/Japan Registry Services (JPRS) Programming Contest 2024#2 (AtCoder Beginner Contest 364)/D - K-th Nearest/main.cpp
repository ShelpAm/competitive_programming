...
/*Start: Sat 27 Jul 2024 09:28:26 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  id[-1e9] = 0;
  for (auto it{std::next(id.begin())}; it != id.end(); ++it) {
  ...
  for (int i{}; i != q; ++i) {
    ...
    auto d{binary_search(
        [&](auto const d) {
          return (std::prev(id.upper_bound(x + d))->second -
                  std::prev(id.lower_bound(x - d))->second) >= k;
        },
        1e9, -1)};
    std::cout << d << '\n';
  }
