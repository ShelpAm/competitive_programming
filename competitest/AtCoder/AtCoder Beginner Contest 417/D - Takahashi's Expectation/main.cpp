...
void solve_case()
{
    ...
    // f[i][j]: i次后，从j开始继续走的结局
    std::vector f(n + 1, std::vector<int>(1001));
    std::iota(f[n].begin(), f[n].end(), 0);
    for (int i{n - 1}; i != -1; --i) {
        auto [p, a, b] = t[i];
        for (int j{}; j != 1001; ++j) {
            f[i][j] = f[i + 1][j <= p ? j + a : std::max(0, j - b)];
        }
    }
    std::vector<int> pref_b(n + 1);
    for (int i{}; auto ti : t) {
        pref_b[i + 1] = pref_b[i] + ti[2];
        ++i;
    }
    debug("perb", pref_b);

    for (auto xi : x) {
        auto c = std::ranges::lower_bound(pref_b, xi - 1000) - pref_b.begin();
        debug("c", c);
        if (c <= n) {
            std::cout << f[c][std::max(0, xi - pref_b[c])] << '\n';
        }
        else {
            std::cout << xi - pref_b[n] << '\n';
        }
    }
}
} // namespace
