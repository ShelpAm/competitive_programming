...
// Problem: E - A Path in A Dictionary
// Contest: AtCoder Beginner Contest 417
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc417/tasks/abc417_e
// Start: Mon 11 Aug 2025 04:46:43 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto get_reachable = [&](int y) {
        ...
        while (!q.empty()) {
            auto s = q.front();
            q.pop();
            for (auto [_, t] : g.edges_of(s)) {
                if (!vis[t] && !reachable[t]) {
                    reachable[t] = 1;
                    q.push(t);
                }
            }
    ...
    std::cout << u + 1 << ' ';
    while (u != y) {
        auto reachable = get_reachable(y);
        int next{n};
