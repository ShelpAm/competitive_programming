...
// Memory Limit: 1024
// Time Limit: 2000
// Start: Sat 11 May 2024 08:31:57 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  int n;
  cin >> n;
  vector<u64> a(n), s(n + 1);
  for (int i{1}; i != n + 1; ++i) {
    cin >> a[i - 1];
    s[i] = (a[i - 1] + s[i - 1]) % mod<u64>;
  }
  u64 ans{};
  for (int j{2}; j != n + 1; ++j) {
    auto deci{[&](auto x) {
      ...
      return pow(10ULL, res, mod<u64>);
    }};
    ans += s[j - 1] * deci(a[j - 1]) % mod<u64> + (j - 1) * a[j - 1] % mod<u64>;
    ans %= mod<u64>;
  }
  cout << ans;
}
