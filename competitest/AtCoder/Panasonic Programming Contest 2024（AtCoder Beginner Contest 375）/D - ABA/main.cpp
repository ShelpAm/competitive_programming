...
/*Start: Sat 12 Oct 2024 08:34:16 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::unordered_map<char, std::vector<i64>> pref;
  constexpr auto charset{std::views::iota('A', 'Z' + 1)};
  for (auto const ch : charset) {
    pref[ch] = std::vector<i64>(s.size() + 1);
  }
  for (int i{}; i != s.size(); ++i) {
    for (auto const ch : charset) {
      pref[ch][i + 1] = pref[ch][i] + (ch == s[i]);
    }
  }

  i64 counter{};
  for (int i{1}; i != s.size(); ++i) {
    for (auto const &[_, p] : pref) {
      counter += (p[s.size()] - p[i]) * p[i - 1];
    }
  }
  std::cout << counter << '\n';
}
