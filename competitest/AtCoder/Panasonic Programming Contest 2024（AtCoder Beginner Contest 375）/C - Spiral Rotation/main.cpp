...
/*Start: Sat 12 Oct 2024 08:07:35 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  //       std::cout << std::format("({}, {}) <- ({}, {})", y, n + 1 - x, x, y)
  //                 << '\n';
  //     }
  //   }
  // }

  auto rotate{[&](auto x, auto y) { return std::make_pair(n - 1 - y, x); }};
  auto rotate_n{[&](auto x, auto y, int n) {
    n %= 4;
    for (int i{}; i != n; ++i) {
      std::tie(x, y) = rotate(x, y);
    }
  ...
  for (int i{}; i != n; ++i) {
    for (int j{}; j != n; ++j) {
      auto const d{std::min({i + 1, j + 1, n - i, n - j})};
      auto const [x, y]{rotate_n(i, j, d)};
      b[i][j] = a[x][y];
    }
  }
