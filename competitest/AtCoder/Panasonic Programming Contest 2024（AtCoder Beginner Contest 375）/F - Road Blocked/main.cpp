...
/*Start: Sat 12 Oct 2024 09:14:26 PM CST*/
/*Author: ShelpAm*/
...
auto floyd(adjacent_matrix_t const &f) -> adjacent_matrix_t
{
  auto const n{f.size()};
...
void solve_case()
{
  ...
  graph::adjacent_matrix_t f(n, std::vector<i64>(n, graph::infinity));
  for (int i{}; i != n; ++i) {
    f[i][i] = 0;
  }
  for (auto &[u, v, w] : a) {
  ...
  std::vector<std::array<i64, 3>> queries(q);
  for (auto &[op, x, y] : queries) {
    std::cin >> op;
    if (op == 1) {
      std::cin >> x;
      auto const &[u, v, _]{a[--x]};
      f[u][v] = f[v][u] = graph::infinity;
    }
    else if (op == 2) {
      std::cin >> x >> y;
      --x, --y;
  ...
  std::vector<i64> ans;
  for (auto const &[op, x, y] : queries | std::views::reverse) {
    if (op == 1) {
      auto [u, v, w]{a[x]};
      {
        chmin(f[u][v], w);
      ...
      ans.push_back(f[x][y] == graph::infinity ? -1 : f[x][y]);
    }
  }
