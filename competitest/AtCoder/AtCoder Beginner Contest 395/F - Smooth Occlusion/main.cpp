...
// Contest: AtCoder Beginner Contest 395
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc395/tasks/abc395_f
// Start: Sat 01 Mar 2025 08:54:43 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto check{[&](int h) {
        ...
        for (int i{}; i != n; ++i) {
            r.push_back({std::max(0, h - d[i]), u[i]});
        }

        int L{0}, R{max<int> / 2};
        for (int i{}; i != n; ++i) {
            L = std::max(L - x, r[i].first);
            R = std::min(R + x, r[i].second);
            if (L > R) {
                return false;
            }
        }
    ...
    auto const h{binary_search(check, 0, min_h + 1)};
    std::cout << s - (h * n) << '\n';
}
} // namespace
