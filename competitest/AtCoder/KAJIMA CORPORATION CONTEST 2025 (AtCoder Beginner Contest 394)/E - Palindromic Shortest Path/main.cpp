...
// Start: Sat 22 Feb 2025 08:14:17 PM CST
// Author: ShelpAm
...
auto dijkstra(Graph const &g, std::vector<int> const &sources,
              std::vector<int> const &s1) -> Dijkstra_result
{
    std::vector<std::int_fast64_t> dist(g.size(), infinity);
...
void solve_case()
{
    ...
    graph::Graph g(n * n);
    auto encode{[n](auto i, auto j) { return (i * n) + j; }};
    // auto decode{[n](auto x) { return std::pair{x / n, x % n}; }};
    for (int i{}; i != n; ++i) {
        for (int j{}; j != n; ++j) {
            for (int u{}; u != n; ++u) {
                for (int v{}; v != n; ++v) {
                    if (a[i][u] != '-' && a[i][u] == a[v][j]) {
                        debug("edge",
                              std::pair{std::pair{u, v}, std::pair{i, j}});
                        g.add_edge(encode(u, v), encode(i, j), 2);
                    }
                }
            }
        }
    ...
    std::vector<int> s1;
    for (int i{}; i != n; ++i) {
        s0.push_back(encode(i, i));
        for (int j{}; j != n; ++j) {
            if (i != j && a[i][j] != '-') {
                s1.push_back(encode(i, j));
            }
        }
    }
    auto const res{graph::dijkstra(g, s0, s1)};

    for (int i{}; i != n; ++i) {
        for (int j{}; j != n; ++j) {
            if (res.distance[encode(i, j)] != graph::infinity) {
                std::cout << res.distance[encode(i, j)] << ' ';
            }
            else {
                std::cout << -1 << ' ';
            }
