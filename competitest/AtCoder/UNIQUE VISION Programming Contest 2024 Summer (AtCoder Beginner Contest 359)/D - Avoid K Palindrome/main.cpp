...
// Contest: UNIQUE VISION Programming Contest 2024 Summer (AtCoder Beginner
// Contest 359) Judge: AtCoder URL:
// https://atcoder.jp/contests/abc359/tasks/abc359_d Memory Limit: 1024 Time
// Limit: 2000 Start: Sat 22 Jun 2024 08:00:12 PM CST Author: ShelpAm
...
void solve_case()
{
  ...
  auto check{[&](int i, int mask, int j) {
    if (i < k) {
      return true;
    }
    int p{i};
    ...
    t.insert(t.begin(), j & 1 ? 'B' : 'A');
    debug("i mask j", vector{i, mask, j});
    debug("t str", t);
    return !is_palindrome(t);
  }};
  vector<vector<u64>> f(n + 1, vector<u64>(1 << k));
  f[0][0] = 1;
  for (int i{1}; i != n + 1; ++i) {
    auto ch{s[i]};
    debug("i", i);
    if (ch != '?') {
      for (int mask{}; mask != 1 << k; ++mask) {
        if (check(i, mask, ch == 'A' ? 0 : 1)) {
          debug("ok mask", mask);
          f[i][mask] = f[i - 1][mask];
        }
      }
      ...
      for (int mask{}; mask != 1 << k; ++mask) {
        for (int j{}; j != 2; ++j) {
          debug("mask", bitset<4>(mask));
          debug("j", j);
          if (check(i, mask, j)) {
            debug("check", "ok");
            (f[i][((mask << 1) + j) & ((1 << k) - 1)] += f[i - 1][mask]) %=
                mod998244353;
          }
        }
      }
