...
// https://atcoder.jp/contests/abc359/tasks/abc359_c Memory Limit: 1024 Time
...
void solve_case()
{
  ...
  i64 sx, sy, tx, ty;
  cin >> sx >> sy >> tx >> ty;
  auto floor{[](i64 p, i64 q) {
    if (p >= 0) {
      return p / q;
    }
    return -((-p + q - 1) / q);
  }};
  auto f{[&](i64 x, i64 y) { return pair{floor(x - y, 2), y}; }};
  auto [u, v]{f(sx, sy)};
  auto [p, q]{f(tx, ty)};
  if (u > p) {
    swap(u, p);
  ...
  i64 ans{abs(u - p) + abs(v - q)};
  if (v > q) {
    ans -= min(abs(u - p), abs(v - q));
  }
  cout << ans;
}
} // namespace
