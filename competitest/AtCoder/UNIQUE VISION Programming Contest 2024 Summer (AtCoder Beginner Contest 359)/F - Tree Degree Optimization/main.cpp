...
// https://atcoder.jp/contests/abc359/tasks/abc359_f Memory Limit: 1024 Time
...
namespace {
struct cmp_t {
  bool operator()(auto l, auto r) { return l < r; }
};
void solve_case()
{
#define int u64
  ...
  auto cmp{[&](auto l, auto r) {
    return (2 * l.first + 1) * a[l.second] > (2 * r.first + 1) * a[r.second];
  }};
  ...
  u64 ans{};
  while (!q.empty()) {
    ans += q.top().first * q.top().first * a[q.top().second];
    q.pop();
  }
  cout << ans;
}
