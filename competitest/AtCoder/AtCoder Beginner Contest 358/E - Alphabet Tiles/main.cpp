// Problem: E - Alphabet Tiles
// Contest: AtCoder Beginner Contest 358
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc358/tasks/abc358_e
// Memory Limit: 1024
// Time Limit: 2000
// Start: Sat 15 Jun 2024 08:56:33 PM CST
// Author: ShelpAm
...
{
  using namespace std;
  constexpr auto p{mod<int>};
  constexpr auto n{1000};
  vector<u64> fac(2 * n + 1, 1), inv(2 * n + 1, 1), pinv(2 * n + 1, 1);
  for (size_t i{2}; i != fac.size(); ++i) {
    fac[i] = fac[i - 1] * i % p;
    inv[i] = (p - p / i) * inv[p % i] % p;
    pinv[i] = pinv[i - 1] * inv[i] % p;
  }
  auto comb{[&](size_t n, size_t m) -> size_t {
  ...
  cin >> c;
  c.insert(c.begin(), 0);
  vector<vector<u64>> f(26 + 1, vector<u64>(K + 1));
  f[0][0] = 1;
