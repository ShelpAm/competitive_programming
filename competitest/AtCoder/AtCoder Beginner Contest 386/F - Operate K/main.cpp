...
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc386/tasks/abc386_f
// Start: Sun 29 Dec 2024 02:05:01 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    if (std::abs(static_cast<int>(s.size()) - static_cast<int>(t.size())) > k) {
        std::cout << "No\n";
        return;
    }

    std::unordered_map<int, int> cur, prev;
    auto get_or_default{[](auto &map, auto key, auto defaulted) {
        if (map.contains(key)) {
            return map.at(key);
        }
        return map[key] = defaulted;
    }};
    for (int i{}; i != k + 1; ++i) {
        cur[i] = i;
    }
    for (int i{1}; i != s.size() + 1; ++i) {
        ...
        auto const beg{std::max(1, i - k)};
        auto const end{std::min<int>(i + k + 1, t.size() + 1)};
        for (int j{beg}; j != end; ++j) {
            cur[j] = s[i - 1] == t[j - 1]
                         ? get_or_default(prev, j - 1, inf<int>)
                         : std::min({get_or_default(prev, j, inf<int>),
                                     get_or_default(cur, j - 1, inf<int>),
                                     get_or_default(prev, j - 1, inf<int>)}) +
                               1;
        }
    }
    std::cout << (get_or_default(cur, t.size(), inf<int>) <= k ? "Yes\n"
                                                               : "No\n");
}
} // namespace
