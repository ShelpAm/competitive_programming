...
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc391/tasks/abc391_b
// Start: Mon 24 Mar 2025 06:53:59 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    // auto pairs = std::views::cartesian_product(std::views::iota(0, n - m +
    // 1),
    //                                            std::views::iota(0, n - m +
    //                                            1));
    std::vector<std::pair<int, int>> pairs;
    for (auto i : std::views::iota(0, n - m + 1)) {
        for (auto j : std::views::iota(0, n - m + 1)) {
            pairs.push_back({i, j});
        }
    }
    auto [x, y] = *std::ranges::find_if(pairs, [&](auto p) {
        for (int i{}; i != m; ++i) {
            for (int j{}; j != m; ++j) {
                if (a[i + std::get<0>(p)][j + std::get<1>(p)] != b[i][j]) {
                    return false;
                }
            }
        }
