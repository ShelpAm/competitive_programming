...
// Start: Mon 24 Mar 2025 08:36:21 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::set<std::array<int, 3>> exist{{0, 0, 0}};
    std::priority_queue<std::pair<i64, std::array<int, 3>>> q;
    q.push({a[0] * b[0] + a[0] * c[0] + b[0] * c[0], {0, 0, 0}});
    while (--k) {
        auto [v, idx] = q.top();
        q.pop();
        for (int i{}; i != 3; ++i) {
            ++idx[i];
            if (idx[i] != n && !exist.contains(idx)) {
                exist.insert(idx);
                q.push({a[idx[0]] * b[idx[1]] + a[idx[0]] * c[idx[2]] +
                            b[idx[1]] * c[idx[2]],
                        idx});
            }
            --idx[i];
