...
// Problem: E - Hierarchical Majority Vote
// Contest: AtCoder Beginner Contest 391
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc391/tasks/abc391_e
// Start: Mon 24 Mar 2025 07:44:59 PM CST
// Author: ShelpAm
...
void solve_case()
{
    int n;
    std::cin >> n;
    auto const t = pow(3LL, n, inf<long long>);
    std::string s;
    std::cin >> s;
    auto a = s | std::views::transform([](char ch) { return ch - '0'; });

    std::vector to(2, std::vector(n + 1, std::vector<int>()));
    for (auto i : {0, 1}) {
        for (int k{}; k != t; ++k) {
            to[i][n].push_back(a[k] ^ i);
        }
    }

    for (int j{n - 1}; j != -1; --j) {
        for (auto i : {0, 1}) {
            for (int k{}; k != to[i][j + 1].size() / 3; ++k) {
                std::vector<int> from;
                for (int o{}; o != 3; ++o) {
                    from.push_back(to[i][j + 1][(3 * k) + o]);
                }
                std::ranges::sort(from);
                to[i][j].push_back(from[0] + from[1]);
            }
        }
    }

    std::cout << std::max(to[0][0][0], to[1][0][0]) << '\n';
}
} // namespace
