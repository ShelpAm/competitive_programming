...
// Start: Mon 24 Mar 2025 07:12:22 PM CST
// Author: ShelpAm
...
void solve_case()
{
    int n, w;
    std::cin >> n >> w;
    std::vector<std::array<int, 3>> block(n);
    for (int i{}; auto &e : block) {
        std::cin >> (e | std::views::take(2));
        --e[0];
        --e[1];
        e[2] = i++;
    }
    ...
    std::vector<int> exittime(n, max<int>);
    std::vector<int> h(w);
    std::vector<std::vector<int>> row(n);
    std::ranges::sort(block, {},
                      [](auto p) { return std::array{p[1], p[0], p[2]}; });
    int base{};
    for (auto [x, y, i] : block) {
        // y == current time
        row[h[x]].push_back(i);
        ++h[x];
        if (row[base].size() == w) {
