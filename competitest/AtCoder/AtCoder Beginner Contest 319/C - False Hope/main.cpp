...
// Contest: AtCoder Beginner Contest 319
// Judge: AtCoder
// URL: https://atcoder.jp/contests/abc319/tasks/abc319_c
// Memory Limit: 1024
// Time Limit: 2000
// Start: Thu 16 May 2024 07:45:11 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  auto check{[](auto const& a) {
    int m{};
    for (int i{}; i != 3; ++i) {
      if (a[i][0] == a[i][1] && a[i][2] != a[i][0]) {
        ++m;
      }
  ...
  auto dfs{[&n, &k, &a, &b, &used, check](auto self, int u) {
    if (u == 9) {
      if (check(b)) {
        ++k;
      }
