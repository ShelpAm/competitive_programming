...
// Start: Fri 07 Mar 2025 08:09:39 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<i64> ans;
    for (int idx{}; auto &e : a) {
        std::vector<i64> ok;
        for (int i{}; i != 6; ++i) {
            if ((e[2] - e[1]) % e[0] == 0 && (e[2] - e[1]) / e[0] >= 0) {
                ok.push_back((e[2] - e[1]) / e[0]);
            }
            std::ranges::next_permutation(e);
        }
        std::ranges::sort(ok);
        if (idx == 0) {
            ans = ok;
            ...
            debug("ans, ok", std::pair{ans, ok});
            std::vector<i64> tmp;
            std::ranges::set_intersection(ans, ok, std::back_inserter(tmp));
            ans = std::move(tmp);
        }
        ++idx;
    }
