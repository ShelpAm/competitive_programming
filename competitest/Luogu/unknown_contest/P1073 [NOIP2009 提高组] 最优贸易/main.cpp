...
/*Start: Sat 03 Aug 2024 07:09:55 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  graph::Graph h(scc.scc.size());
  std::vector<int> scc_id(n);
  std::vector<int> b(scc.scc.size(), inf<int>);
  std::vector<int> c(scc.scc.size(), -inf<int>);
  for (int i{}; i != scc.scc.size(); ++i) {
    for (auto const u : scc.scc[i]) {
      scc_id[u] = i;
      chmin(b[i], a[u]);
      chmax(c[i], a[u]);
    }
  }
  std::vector<int> in(scc.scc.size());
  for (int i{}; i != scc.scc.size(); ++i) {
    for (auto const u : scc.scc[i]) {
      for (auto const &[_, v] : g.edges_of(u)) {
        if (i != scc_id[v]) {
          h.add_edge(i, scc_id[v], 1);
        }
      }
    }
  }
  ...
  int ans{};
  std::vector<int> max_reachable(scc.scc.size(), -1);
  std::vector<int> reachable_n(scc.scc.size());
  reachable_n[scc_id[n - 1]] = 1;
  std::function<void(int)> dfs{[&](int const u) {
    if (max_reachable[u] != -1) {
      return;
    }
    max_reachable[u] = c[u];
    for (auto const &[_, v] : h.edges_of(u)) {
      ...
      if (reachable_n[v]) {
        reachable_n[u] = 1;
        chmax(max_reachable[u], max_reachable[v]);
      }
    }
    if (reachable_n[u]) {
      chmax(ans, max_reachable[u] - b[u]);
    }
  }};
  dfs(scc_id[0]);
  std::cout << ans << '\n';
