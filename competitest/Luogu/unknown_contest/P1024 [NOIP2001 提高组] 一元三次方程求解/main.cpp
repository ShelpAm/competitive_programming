...
/*Judge: Luogu*/
/*URL: https://www.luogu.com.cn/problem/P1024?contestId=200260*/
...
void solve_case()
{
  ...
  auto f{[&](double x) { return x * (x * (x * a + b) + c) + d; }};

  std::multimap<double, double> ans_of_diff;
  for (double i{-100}; i <= 100; i += 0.01) {
    ans_of_diff.insert({std::abs(f(i)), i});
  }

  std::set<double> ans;
  for (int idx{}; auto [diff, e] : ans_of_diff) {
    ans.insert(e);
    if (++idx == 3) {
      break;
    }
