// Problem: P8805 [蓝桥杯 2022 国 B] 机房
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P8805?contestId=171923
// Memory Limit: 128
// Time Limit: 1000
// Start: Tue 14 May 2024 01:46:48 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  vector<vector<int>> pa(msb(n) + 1, vector<int>(n));
  vector<u64> sum(n);
  vector<int> depth(n);
  depth[0] = -1;
  auto dfs{[&](auto self, int u, int p) -> void {
    pa[0][u] = p;
    sum[u] = g.edges_of(u).size() + sum[p];
    depth[u] = depth[p] + 1;
  ...
  for (int i{}; i != m; ++i) {
    ...
    cin >> u >> v;
    --u, --v;
    auto const p{lca(u, v)};
    cout << sum[u] + sum[v] - 2 * sum[p] + g.edges_of(p).size() << '\n';
  }
}
