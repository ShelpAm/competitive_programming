// Problem: P8694 [蓝桥杯 2019 国 AC] 估计人数
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P8694?contestId=173065
// Memory Limit: 128
// Time Limit: 1000
// Start: Tue 21 May 2024 02:25:44 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  vector<vector<int>> ends(n, vector<int>(m));
  vector<pair<int, int>> const dirs{{0, 1}, {1, 0}};
  function<void(int, int)> walk{[&](int x, int y) {
    vis[x][y] = 1;
    for (auto [i, j] : dirs) {
      i += x, j += y;
      if (i != n && j != m && a[i][j] == '1' && !vis[i][j]) {
        walk(i, j);
        return;
      }
    }
  ...
  auto has_a_way{[&](auto self, int i, int j, int n, int m) {
    if (i == n && j == m && a[i][j] == '1') {
      return true;
    }
    if (i > n || j > m || a[i][j] == '0') {
    ...
    return self(self, i + 1, j, n, m) || self(self, i, j + 1, n, m);
  }};
  auto find{[&](int n, int m) {
    for (int j{m}; j >= 0; --j) {
      for (int i{}; i <= n; ++i) {
        if (ends[i][j] != 0 && has_a_way(has_a_way, i, j, n, m)) {
          --ends[i][j];
          return true;
        }
      }
