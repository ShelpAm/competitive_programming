...
/*Start: Sun 04 Aug 2024 12:22:11 AM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::cin >> s >> p;
  --s;
  auto const [h, scc_id]{graph::contract_edges(g)};
  std::vector<int> end_reachable(h.adjacent.size());
  for (int i{}; i != p; ++i) {
    int x;
    std::cin >> x;
    end_reachable[scc_id[--x]] = 1;
  }

  std::vector<i64> b(h.adjacent.size());
  for (int i{}; i != n; ++i) {
    b[scc_id[i]] += a[i];
  }
  std::vector<i64> f(h.adjacent.size(), -1);
