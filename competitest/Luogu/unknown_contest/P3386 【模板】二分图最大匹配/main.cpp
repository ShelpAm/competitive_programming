...
// Memory Limit: 512
// Time Limit: 1000
// Start: Mon 13 May 2024 10:40:42 AM CST
// Author: ShelpAm
...
int maximum_matching(graph g, int n, int m)
{
  assert(n >= 0 && m >= 0);
  assert(g.adjacent.size() == static_cast<std::size_t>(n));
  int res{};
  int idx{}; // dfs order, identifying if a vertex was visited in current round
  std::vector<int> dfn(n);
  std::vector<int> pa(n, -1), pb(m, -1); // matches from a and matches from b
...
auto solve_case()
{
  ...
  cout << graph::maximum_matching(g, n, m);
}
