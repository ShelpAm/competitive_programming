...
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P8730?contestId=171923
// Memory Limit: 128
// Time Limit: 1000
...
auto solve_case()
{
  ...
  int n, a, b, c, d;
  cin >> n >> a >> b >> c >> d;
  vector<int> const norm{0, 5, 6, 1, 4, 7, 3, 4, 8};
  function<u64(int, int, int)> to_origin{[&norm](int n, int x, int y) -> u64 {
    if (n == 1) {
      return norm[x * 3 + y];
    }
    if (y == 1) {
      if (x == 0) {
        return 5 * pow(3ULL, n - 1, inf<u64>) + to_origin(n - 1, 2 - x, y);
      }
    }
  }};
}
