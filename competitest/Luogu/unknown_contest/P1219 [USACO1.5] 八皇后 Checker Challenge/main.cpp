...
/*Judge: Luogu*/
/*URL: https://www.luogu.com.cn/problem/P1219?contestId=200260*/
...
void solve_case()
{
  ...
  auto ok{[&](int x, int y) {
    ...
    for (int i{1}; i != n; ++i) {
      if (x - i >= 0 && y - i >= 0 && a[x - i][y - i]) {
        return false;
      }
      if (x - i >= 0 && y + i < n && a[x - i][y + i]) {
        return false;
      }
    }
    return true;
  ...
  for (auto const &e : ans | std::views::take(3)) {
    for (auto const k : e) {
      std::cout << k + 1 << ' ';
    }
    std::cout << '\n';
