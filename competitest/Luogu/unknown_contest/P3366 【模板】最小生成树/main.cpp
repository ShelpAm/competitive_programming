...
// Memory Limit: 128
// Time Limit: 1000
// Start: Fri 19 Jul 2024 10:47:35 AM CST
// Author: ShelpAm
...
auto prim(Graph const &g) -> std::int_fast64_t
{
  ...
  // If memory is concerned, use this:
  std::unordered_set<int> visited(g.adjacent.size());
  ...
  while (!q.empty()) {          // The main loop
    ...
    /*}*/
    /*visited[u] = true;*/
    // Replacement for memory concerning:
    ...
    weight += w;
    if (visited.size() == g.adjacent.size()) {
      return weight;
    }
  ...
  // If all vertices were travelled through, the algorithm would return in the
  // loop above.
  return -1;
}
...
void solve_case()
{
  ...
  int n, m;
  cin >> n >> m;
  auto t{graph::prim(graph::read_graph(n, m, false, true, true))};
  if (t == -1) {
    cout << "orz";
  }
  else {
