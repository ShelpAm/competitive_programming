...
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P9420?contestId=171012
// Memory Limit: 256
// Time Limit: 1000
...
auto solve_case()
{
  ...
  i64 const up{23333333333333}, low{2333};
  vector<i64> primes;
  vector<i64> not_prime(sqrt(up) + 1);
  ...
  for (auto const p : primes) {
    ans[1] += upper_bound(primes.begin(), primes.end(), sqrt(up) / p) -
              lower_bound(primes.begin(), primes.end(), sqrt(low) / p);
    auto const qua{static_cast<double>(p) * p * p * p};
    if (low <= qua && qua <= up) {
      --ans[1];
    }
  }
