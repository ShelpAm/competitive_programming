// Problem: T423719 『STA - R5』Remove and Decrease Game
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/T423719?contestId=156888
// Memory Limit: 512
// Time Limit: 1000
// Start: Thu 02 May 2024 02:01:03 PM CST
// Author: ShelpAm
...
auto solve_case() {
  ...
  // auto const m{ranges::max(a)};
  map<vector<int>, int> f;
  f[{}] = f[{0}] = 0;
  function<int(vector<int>)> sg{[&f, &sg](vector<int> const &v) {
    if (f.contains(v)) {
      return f[v];
    }
    vector<bool> tmp(3);
  ...
  // int const bound{12};
  // for (int k{1}; k != 6; ++k) {
  //   for (int u{bound}; u != k - 1; --u) {
  //     vector<int> res{u};
  //     auto make{[&](auto self, int i) -> void {
  //       if (i == k) {
  //         if (res.back() != 0) {
  //           sg(res);
  //         }
  ...
  //   }
  // }
  // map<int, vector<vector<int>>> w;
  // for (auto const &[key, value] : f) {
  //   w[value].push_back(key);
  // }
  // for (auto const &[sg, data] : w) {
  //   cout << "SG: " << sg << '\n';
  //   map<int, vector<vector<int>>> b;
  //   for (auto const &v : data) {
  //     b[v.size()].push_back(v);
  //   }
  //   for (auto [size, vecs] : b) {
  //     cout << "SIZE: " << size << '\n';
  //     for (auto const &line : vecs) {
  //       int s{};
  ...
  if (n <= 2) {
    if (n == 1) {
      ...
      cout << (ranges::min(a) % 2 == 0 ? "Alice" : "Bob") << '\n';
    }
  }
  // cout << (/* sg(a) != 0 */ ? "Alice" : "Bob") << '\n';
}
