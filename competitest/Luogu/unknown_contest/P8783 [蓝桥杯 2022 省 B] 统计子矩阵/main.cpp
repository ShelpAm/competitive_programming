...
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P8783?contestId=171012
// Memory Limit: 128
// Time Limit: 1000
...
auto solve_case()
{
  ...
  auto calc{[&](int i, int j, int x, int y) {
    return s[i][j] - s[i][y - 1] - s[x - 1][j] + s[x - 1][y - 1];
  }};
  i64 ans{};
  for (int i{1}; i != n + 1; ++i) {
    for (int j{i}; j != n + 1; ++j) {
      i64 sum{};
      for (int l{1}, r{1}; r != m + 1; ++r) {
        sum += calc(j, r, i, r);
        while (l < r && sum > k) {
          sum -= calc(j, l, i, l);
          ++l;
        ...
        if (sum <= k) {
          // debug("i j l r", vector<int>{i, j, l, r});
          ans += r - l + 1;
        }
      }
    }
