...
/*Start: Sat 03 Aug 2024 12:43:06 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::cin >> weight;
  auto const k{graph::read(n, m, true, false)};
  graph::Tarjan_scc scc(k);
  ...
  std::vector<int> in(n);
  for (int u{}; u != n; ++u) {
    for (auto const _ : scc.scc[u]) {
      value[u] += weight[_];
      for (auto const &[_, v] : k.edges_of(_)) {
        if (scc_id[v] != u) {
          g.add_edge(u, scc_id[v], 1);
          ++in[scc_id[v]];
        }
      }
    }
