...
// Memory Limit: 128
// Time Limit: 1000
// Start: Fri 24 May 2024 05:23:47 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  q.push({0, 0});
  vector<int> vis(n);
  vector<double> dis(n, inf<double>);
  dis[0] = 0;
  double ans{};
  while (!q.empty()) {
    auto const [w, u]{q.top()};
    q.pop();
    if (vis[u]) {
      continue;
    }
    ...
    ans += w;
    for (int v{}; v != n; ++v) {
      if (auto const neo_w{sqrt(pow(a[u].first - a[v].first, 2) +
                                pow(a[u].second - a[v].second, 2))};
          !vis[v] && check_min(dis[v], neo_w)) {
        q.push({neo_w, v});
      }
    }
  }
