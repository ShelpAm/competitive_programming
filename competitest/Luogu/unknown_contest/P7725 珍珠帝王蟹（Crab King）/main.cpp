// Problem: P7725 珍珠帝王蟹（Crab King）
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P7725
// Memory Limit: 256
// Time Limit: 1000
// Start: Tue 14 May 2024 07:51:48 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  for (int i{}; i != n; ++i) {
    ...
    cin >> ch >> v;
    if (ch == '+') {
      ((v >= 0 ? pos : neg) += abs(v)) %= mod<i64>;
    }
    else if (ch == '*') {
      if (v > 0) {
        times = times * v % mod<i64>;
      }
      else if (v < 0) {
        minustimes.push_back(v);
      }
    }
  }
  auto add{
      [](i64 x, i64 y) { return ((x + y) % mod<i64> + mod<i64>) % mod<i64>; }};
  auto mul{
      [](i64 x, i64 y) { return ((x * y) % mod<i64> + mod<i64>) % mod<i64>; }};
  ranges::sort(minustimes);
  i64 res{};
  if (minustimes.empty()) {
    res = add(mul(pos, times), -neg);
  }
  else if (minustimes.size() % 2 == 1) {
    res = add(mul(-neg, minustimes.back()), pos);
    minustimes.pop_back();
    res = mul(res, times);
    for (auto const e : minustimes) {
      res = mul(res, e);
    }
  }
  else {
    res = add(mul(pos, minustimes.back()), -neg);
    minustimes.pop_back();
    res = mul(res, times);
    for (auto const e : minustimes) {
      res = mul(res, e);
