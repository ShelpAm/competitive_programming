// Problem: P8764 [蓝桥杯 2021 国 BC] 二进制问题
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P8764?contestId=173065
// Memory Limit: 128
// Time Limit: 1000
// Start: Tue 21 May 2024 01:39:57 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  using namespace std;
  using ll = i64;
  vector<vector<ll>> f(100, vector<ll>(100));
  for (int i = 0; i <= 64; i++) {
    f[i][0] = 1;
  }
  for (int i = 1; i <= 64; i++) {
  ...
  i64 n, k;
  cin >> n >> k;
  function<i64(i64, int)> solve{[&](i64 n, int k) -> i64 {
    if (n == 0) {
      return 0;
    }
    if (k == 0) {
    ...
    return f[msb(n)][k] + solve(n - (1LL << msb(n)), k - 1);
  }};
  cout << solve(n, k);
}
