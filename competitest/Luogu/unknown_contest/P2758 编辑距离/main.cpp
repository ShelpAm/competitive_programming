...
// Memory Limit: 125
// Time Limit: 1000
// Start: Wed 19 Jun 2024 11:14:05 PM CST
// Author: ShelpAm
...
void solve_case()
{
  ...
  string s, t;
  cin >> s >> t;
  vector<vector<int>> f(s.size() + 1, vector<int>(t.size() + 1, inf<int>));
  f[0][0] = 0;
  ...
  for (int i{1}; i != s.size() + 1; ++i) {
    for (int j{1}; j != t.size() + 1; ++j) {
      check_min(f[i][j], f[i - 1][j - 1] + (s[i - 1] != t[j - 1]));
      check_min(f[i][j], f[i - 1][j] + 1);
      check_min(f[i][j], f[i][j - 1] + 1);
    }
  }
