// Problem: U439498 笑起来最帅的小孩
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/U439498?contestId=157725
// Memory Limit: 512
// Time Limit: 800
// Start: Sat 20 Jul 2024 12:30:06 PM CST
// Author: ShelpAm
...
public:
  ...
  auto operator()(int const n, int const m) -> std::int_fast64_t
  {
    ...
    return _combination(n, m) * _combination._inverse.factorial(m);
  }
...
void solve_case()
{
  ...
  constexpr auto p{2007072007};
  auto const n{sum_of(num)};
  Combination comb(9, p);
  u64 ans{};
  auto pw{(pow(10ULL, n, p) - 1) * comb._inverse(10 - 1) % p};
  for (int i{}; i != 10; ++i) {
    ans += num[i] * i;
    ans %= p;
  }
  ans = ans * pw % p;
  ans = ans * pow(n, p - 2, p) % p;
  cout << ans << '\n';
}
} // namespace
