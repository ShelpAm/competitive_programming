...
// Memory Limit: 125
// Time Limit: 1000
// Start: Sun 12 May 2024 10:01:11 AM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  function<bool(int, int, int)> unite{[&](int d, int x, int y) {
    if (find(x) == find(y)) {
      return (dis[x] - dis[y] + 3) % 3 == d;
    }
    dis[pa[x]] = (3 + d + dis[y] - dis[x]) % 3;
    // assert(find(x) == pa[x] && find(y) == pa[y]);
    pa[pa[x]] = pa[y];
    return true;
  }};
  int fake{};
