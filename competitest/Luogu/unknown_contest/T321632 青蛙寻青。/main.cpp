// Problem: T321632 青蛙寻青。
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/T321632?contestId=116062
// Memory Limit: 512
// Time Limit: 2000
// Start: Wed 01 May 2024 02:11:39 PM CST
// Author: ShelpAm
...
auto solve_case() {
  ...
  bool failed{};
  int max_color{};
  auto serialize{[&failed, &max_color](auto &seq) {
    vector<int> colors{seq.front()};
    for (auto const e : seq) {
      check_max(max_color, e);
      if (e != colors.back()) {
        colors.push_back(e);
      }
    }
    vector<bool> presented(max_color + 1);
    for (auto e : colors) {
      if (presented[e]) {
        failed = true;
        return vector<int>{};
  ...
  auto c{b};
  auto dfs{[&](auto self, int i, int diffcnt) -> void {
    if (i == m) {
      auto const colorc{serialize(c)};
      if (!failed) {
        // debug("colorc", colorc);
        if (colorc == colora) {
          check_min(ans, diffcnt);
        }
      }
      failed = false; // reset the variable
      return;
    }
    for (int j{1}; j != max_color + 1; ++j) {
      c[i] = j;
      self(self, i + 1, diffcnt + (c[i] != b[i]));
      // c[i]=b[i];
    }
  }};
  dfs(dfs, 0, 0);
