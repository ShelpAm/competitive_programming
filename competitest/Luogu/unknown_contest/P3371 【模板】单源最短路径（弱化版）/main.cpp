...
// Memory Limit: 125
// Time Limit: 1000
// Start: Fri 10 May 2024 12:56:06 PM CST
// Author: ShelpAm
...
graph read_graph(size_t const num_of_vertices, size_t const num_of_edges,
...
struct bellman_ford_result {
  bool no_negative_circle;
  std::vector<std::int_fast64_t>
      distance; // When `no_negative_circle` is false, this is usable.
};
bellman_ford_result bellman_ford(graph const& graph, int const source)
{
  int const n{static_cast<int>(graph.adjacent.size())};
  std::vector<std::int_fast64_t> distance(graph.adjacent.size(), infinity);
  std::vector<int> cnt(graph.adjacent.size());
  ...
  while (!q.empty()) {
    ...
    for (auto const [w, v] : graph.edges_of(u)) {
      auto const alt{distance[u] + w};
      if (check_min(distance[v], alt)) {
        cnt[v] = cnt[u] + 1;
        if (cnt[v] == n) {
          return {false, {}};
        }
...
auto solve_case()
{
  ...
  cin >> n >> m >> s;
  auto dis{
      graph::bellman_ford(graph::read_graph(n, m, false, true, true), s - 1)
          .distance};
  for (auto const e : dis) {
