...
// Memory Limit: 512
// Time Limit: 1000
// Start: Tue 23 Apr 2024 01:13:19 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  auto gauss{[](auto& a) {
    ...
    for (int col{}; col != n; ++col) {
      // Find the max row, and swap it to the top(`row`)
      int mx{row};
      ...
      // Make current line begin with 1.
      for (int i{n}; i >= col; --i) {
        a[row][i] /= a[row][col];
      }
      // Make lines below with 0.
      for (int i{row + 1}; i != n; ++i) {
        for (int j{n}; j >= col; --j) {
    ...
    // Identify non-normal situations
    if (row != n) {
      for (int i{row}; i != n; ++i) {
        if (abs(a[i][n]) > eps) {
          // No solution
