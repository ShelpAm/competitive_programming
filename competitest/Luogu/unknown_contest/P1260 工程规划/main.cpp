...
// Memory Limit: 125
// Time Limit: 1000
// Start: Sun 12 May 2024 07:31:42 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  auto res{graph::bellman_ford(g, 0)};
  if (res.contains_negative_circle) {
    cout << "NO SOLUTION";
  }
  else {
    auto const lo{ranges::min(res.distance)};
    for (auto& e : res.distance) {
      e -= lo;
    }
    for (int i{1}; i != n + 1; ++i) {
      cout << res.distance[i] << ' ';
