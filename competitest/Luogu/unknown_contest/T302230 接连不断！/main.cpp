// Problem: T302230 接连不断！
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/T302230?contestId=116062
// Memory Limit: 512
// Time Limit: 1000
...
auto solve_case() {
  ...
  auto do_test{[&]() {
    ...
    for (int g{1}; g != nums.size(); ++g) {
      if (nums[g].empty()) {
        continue;
      }
      cout << "SAME (" << g << "): ";
      for (auto const i : nums[g]) {
        cout << i << ' ';
      }
      cout << '\n';
    }
  ...
  u64 wtf{1};
  sieve s(sqrt(n) + 1);
  for (auto [factor, cnt] : s.factorize(n)) {
    wtf *= factor;
  }
  cout << m / wtf + 1 << '\n';
}
