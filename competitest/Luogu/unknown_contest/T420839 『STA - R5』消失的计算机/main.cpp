// Problem: T420839 『STA - R5』消失的计算机
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/T420839?contestId=156888
// Memory Limit: 512
// Time Limit: 3000
...
auto solve_case() {
  ...
  cin >> n;
  vector<string> ans;
  auto inc{[&](int x) { ans.push_back("new " + to_string(x)); }};
  auto dec{[&](int x) { ans.push_back("dec " + to_string(x)); }};
  auto assign{[&](int x, int y) {
    ans.push_back("assign " + to_string(x) + ' ' + to_string(y));
  }};
  auto rep{[&](int times, auto func) {
