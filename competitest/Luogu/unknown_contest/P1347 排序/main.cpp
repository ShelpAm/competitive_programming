...
// Memory Limit: 125
// Time Limit: 1000
// Start: Mon 13 May 2024 10:02:54 AM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  for (int i{}; i != m; ++i) {
    string s;
    cin >> s;
    auto a{s[0]}, b{s[2]}, ch{s[1]};
    dsu.unite(a - 'A', b - 'A');
    if (ch == '<') {
      g.add_edge(a - 'A', b - 'A', 1);
    }
    else if (ch == '>') {
      g.add_edge(b - 'A', a - 'A', 1);
    }
    auto res{graph::toposort(g, true)};
    if (res.contains_circle) {
      cout << "Inconsistency found after " << i + 1 << " relations.\n";
      return;
    }
    if (dsu.size_[dsu.find(0)] == n && !res.contains_circle) {
      cout << "Sorted sequence determined after " << i + 1 << " relations: ";
      for (auto const e : res.order) {
        cout << static_cast<char>(e + 'A');
      }
      cout << ".";
      return;
    }
    // debug("size", dsu.size_[dsu.find(0)]);
    if (i == m - 1 && dsu.size_[dsu.find(0)] != n) {
