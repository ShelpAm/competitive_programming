...
// Memory Limit: 125
// Time Limit: 1000
// Start: Wed 19 Jun 2024 04:20:45 PM CST
// Author: ShelpAm
...
void solve_case()
{
  ...
  int n, m;
  cin >> n >> m;
  auto g{graph::read_graph(n, m, false, false, true)};
  vector<u64> d(n);
  for (int i{}; i != n; ++i) {
    d[i] ^= 1ULL << i;
  ...
  map<u64, int> res1, res2;
  auto search{[&](int l, int r, auto &min_ops) {
    auto len{r - l + 1};
    for (int mask{}; mask != 1 << len; ++mask) {
      u64 result_mask{};
      for (int k{}; k != len; ++k) {
        if (mask & 1 << k) {
          result_mask ^= d[l + k];
      ...
      if (!min_ops.contains(mask)) {
        min_ops.insert({result_mask, cnt});
      }
      else {
        check_min(min_ops[result_mask], cnt);
      }
    }
  }};
  search(0, (n - 1) / 2, res1);
  ...
  auto search1{[&](int l, int r) {
    ...
    for (int mask{}; mask != 1 << len; ++mask) {
      ...
      auto cnt{__builtin_popcount(mask)};
      if (res1.contains(full ^ result_mask)) {
        check_min(ans, cnt + res1[full ^ result_mask]);
      }
    }
  }};
  /*search1((n - 1) / 2 + 1, n - 1);*/
  search((n - 1) / 2 + 1, n - 1, res2);
  for (auto &[k, v] : res1) {
    if (res2.contains(full ^ k)) {
      check_min(ans, v + res2[full ^ k]);
    }
  }
  cout << ans;
}
