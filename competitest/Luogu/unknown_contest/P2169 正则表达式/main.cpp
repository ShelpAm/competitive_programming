...
/*Start: Sun 04 Aug 2024 12:07:35 AM CST*/
/*Author: ShelpAm*/
...
auto contract_edges(Graph const &g) -> std::pair<Graph, std::vector<int>>
{
...
void solve_case()
{
  int n, m;
  std::cin >> n >> m;
  auto const g{graph::read(n, m, true, true)};
  auto const [h, scc_id]{graph::contract_edges(g)};
  std::cout << graph::dijkstra(h, scc_id[0]).distance[scc_id[n - 1]] << '\n';
}
} // namespace
