// Problem: P10385 [蓝桥杯 2024 省 A] 艺术与篮球
// Contest: unknown_contest
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P10385?contestId=171012
// Memory Limit: 512
// Time Limit: 1000
...
{
  using namespace std;
  map<int, int> mp{{0, 13}, {1, 1}, {2, 2}, {3, 3}, {4, 5},
                   {5, 4},  {6, 4}, {7, 2}, {8, 2}, {9, 2}};
  vector<int> const proto{0, 31, 0, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
  int ans{};
  for (int i{2000}; i <= 2024; ++i) {
    auto days{proto};
    days[2] = i % 4 == 0 ? 29 : 28;
    for (int m{1}; m <= (i == 2024 ? 4 : 12); ++m) {
      for (int d{1}; d <= (i == 2024 && m == 4 ? 13 : days[m]); ++d) {
        int cnt{};
        for (auto const ch : to_string(10000 * i + 100 * m + d)) {
          cnt += mp[ch - '0'];
        }
        if (cnt > 50) {
          ++ans;
