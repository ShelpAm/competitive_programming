...
// Memory Limit: 125
// Time Limit: 1000
// Start: Mon 13 May 2024 05:37:50 PM CST
// Author: ShelpAm
...
struct tarjan_cuts {
  ...
  std::function<void(int, bool)> run{[&](int u, bool isroot = true) {
    low[u] = dfn[u] = ++idx;
    u64 cnt{};
    for (auto const [_, v] : g.edges_of(u)) {
      if (!dfn[v]) {
...
auto solve_case()
{
  ...
  int n, m;
  cin >> n >> m;
  auto const g{graph::read_graph(n, m, false, false, true)};
  graph::tarjan_cuts tarjan(g);
  for (int i{}; i != n; ++i) {
    if (!tarjan.dfn[i]) {
      tarjan.run(i, true);
    }
  }
  sort(tarjan.cuts.begin(), tarjan.cuts.end());
  cout << tarjan.cuts.size() << '\n';
  for (auto const e : tarjan.cuts) {
    cout << e + 1 << ' ';
  }
}
