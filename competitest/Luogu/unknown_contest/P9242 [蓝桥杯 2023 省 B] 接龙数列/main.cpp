...
// Judge: Luogu
// URL: https://www.luogu.com.cn/problem/P9242?contestId=171012
// Memory Limit: 256
// Time Limit: 1000
...
auto solve_case()
{
  ...
  vector<int> a(n);
  cin >> a;
  auto l{[](int x) { return to_string(x).front() - '0'; }};
  auto r{[](int x) { return to_string(x).back() - '0'; }};
  vector<int> len_ends_with(10);
  for (auto const e : a) {
    check_max(len_ends_with[r(e)], 1 + len_ends_with[l(e)]);
  }
  cout << n - ranges::max(len_ends_with);
}
