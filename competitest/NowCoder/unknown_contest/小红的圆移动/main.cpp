...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/91177/C*/
/*Start: Sun 29 Sep 2024 07:05:19 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  for (int i{}; i != n; ++i) {
    i64 x, y, r;
    std::cin >> x >> y >> r;
    auto const area{std::numbers::pi * r * r};
    auto const dist{std::max(0., r - std::sqrt(x * x + y * y))};
    cost[i] = area * dist;
  }
  std::ranges::sort(cost);

  std::cout << std::accumulate(cost.begin(), cost.begin() + n - k, 0.) << '\n';
}
} // namespace
