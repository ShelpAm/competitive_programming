...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/81114/E
// Memory Limit: 524288
// Time Limit: 2000
// Start: Tue 28 May 2024 05:39:38 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  string secret;
  getline(cin, secret);
  unordered_map<char, char> to, back;
  {
    string buf;
    unordered_set<char> set;
    for (auto ch : secret) {
      ch = toupper(ch);
      if ('A' <= ch && ch <= 'Z' && !set.count(ch)) {
        buf += ch;
        set.insert(ch);
      }
    }
    ...
    /*debug("buf.size()", buf.size());*/
    /*debug("buf", buf);*/
    assert(buf.size() == 26);
    for (int i{}; i != 26; ++i) {
      to.insert({'a' + i, buf[i]});
      back.insert({buf[i], 'a' + i});
    }
  }
  while (true) {
    string op, txt;
