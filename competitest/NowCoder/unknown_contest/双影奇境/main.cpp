...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/106899/D
// Start: Sun 06 Apr 2025 04:28:28 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    for (int i{}; i != n; ++i) {
        for (int j{}; j != m; ++j) {
            ...
            for (auto [dx, dy] : dirs) {
                ...
                dsu.unite(i * m + j, dx * m + dy);
            }
        }
    }

    debug("dsu._parent", dsu._parent);
    ...
    for (int i{}; i != q; ++i) {
        ...
        int x2, y2;
        int xe, ye;
        std::cin >> x1 >> y1 >> x2 >> y2 >> xe >> ye;
        --x1, --y1, --x2, --y2, --xe, --ye;

        auto can_only_faster = [&](auto x, auto y, auto x2, auto y2) {
            int ok{};
            for (auto [dx, dy] : dirs) {
                dx += x;
                dy += y;
                if (dx < 0 || dx >= n || dy < 0 || dy >= m ||
                    a[dx][dy] == '#' ||
                    std::pair{dx, dy} == std::pair{xe, ye}) {
                    continue;
                }
                ++ok;
            }
            return ok == 0 && std::abs(x - xe) + std::abs(y - ye) == 1 &&
                   std::abs(x2 - xe) + std::abs(y2 - ye) != 1;
        };

        if (!dsu.united(x1 * m + y1, xe * m + ye) ||
            !dsu.united(x2 * m + y2, xe * m + ye) ||
            (x1 + y1) % 2 != (x2 + y2) % 2 || can_only_faster(x1, y1, x2, y2) ||
            can_only_faster(x2, y2, x1, y1)) {
            std::cout << "No\n";
        }
        else {
            std::cout << "Yes\n";
