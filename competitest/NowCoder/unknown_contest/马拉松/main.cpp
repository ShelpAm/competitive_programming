...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/88269/I*/
/*Start: Wed 07 Aug 2024 01:33:07 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  std::vector<i64> size(n);
  std::vector<int> contains_y(n);
  contains_y[y] = 1;
  std::function<void(int, int)> dfs{[&](int const u, int const p) {
    size[u] = 1;
    for (auto const &[_, v] : g.edges_of(u)) {
      if (v != p) {
        dfs(v, u);
        size[u] += size[v];
        contains_y[u] |= contains_y[v];
      }
    }
  }};
  dfs(x, x);
  auto const y_dir_size{[&] {
    for (auto const &[_, v] : g.edges_of(x)) {
      if (contains_y[v]) {
        return size[v];
      }
    }
  ...
  auto const ans{(n - y_dir_size()) * size[y]};
  std::cout << ans << '\n';
}
} // namespace
