...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/100007/D
// Start: Fri 10 Jan 2025 08:23:43 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto run{[&](int u, int p, std::pair<int, int> lock) {
        debug("u p", std::pair{u, p});
        std::vector<std::array<int, 2>> f(n);
        // j represent the usage of the node. 1->connected with parent.
        auto dfs{[&](auto dfs, int u, int p) -> void {
            std::vector<int> d;
            int s{};
            for (auto const &[_, v] : g.edges_of(u)) {
                if (v != p && lock != std::pair{u, v}) {
                    dfs(dfs, v, u);
                    f[u][0] += std::ranges::max(f[v]);
                    d.push_back(f[v][0] - f[v][1]);
                    s += f[v][1];
                }
            }
            if (!d.empty()) {
                f[u][1] = 1 + std::ranges::max(d) + s;
            }
        }};
        dfs(dfs, u, p);
        return std::ranges::max(f[u]);
    }};

    if (n - 1 == m) {
        auto const left{n - (2 * run(0, 0, {}))};
        std::cout << (left == 0 ? "Yes" : std::to_string(left)) << '\n';
    }
    else {
        int s{};
        Disjoint_set_union dsu(n);
        for (auto const &[_, u, v] : graph::to_edges(g)) {
            if (!dsu.unite(u, v)) {
                s = u;
                break;
            }
        ...
        int ans{};
        for (auto const &[_, v] : g.edges_of(s)) {
            chmax(ans, run(s, v, {v, s}));
        }
        auto const left{n - (2 * ans)};
        std::cout << (left == 0 ? "Yes" : std::to_string(left)) << '\n';
    }
}
} // namespace
