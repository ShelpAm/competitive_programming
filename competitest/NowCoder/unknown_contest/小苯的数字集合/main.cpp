...
// Start: Fri 28 Mar 2025 05:13:17 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto op = std::vector<std::function<int(int, int)>>{
        [](auto x, auto y) { return x & y; },
        [](auto x, auto y) { return x | y; },
        [](auto x, auto y) { return x ^ y; },
        [](auto x, auto y) { return std::gcd(x, y); }};

    int ans{3};
    for (int i{}; i != 4; ++i) {
        for (int j{}; j != 4; ++j) {
            if (op[j](op[i](x, y), x) == 0 || op[j](op[i](x, y), y) == 0) {
                std::cout << 2 << '\n';
                return;
            }
        }
