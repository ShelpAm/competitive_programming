...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/87865/H*/
/*Start: Fri 02 Aug 2024 01:30:59 AM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::vector<std::vector<std::vector<int>>> f(
      n + 1, std::vector<std::vector<int>>(m + 1, std::vector<int>(n * m + 1)));
  f[1][1][0] = h;
  for (int i{}; i != n; ++i) {
    for (int j{}; j != m; ++j) {
      check_max(f[i + 1][j + 1][0],
                std::max(f[i + 1][j][0], f[i][j + 1][0]) - a[i][j]);
      for (int k{1}; k != n * m + 1; ++k) {
        check_max(f[i + 1][j + 1][k],
                  std::max(f[i + 1][j][k], f[i][j + 1][k]) - a[i][j]);
        check_max(f[i + 1][j + 1][k],
                  std::max(f[i + 1][j][k - 1], f[i][j + 1][k - 1]));
      }
    }
  }
  int ans{inf<int>};
