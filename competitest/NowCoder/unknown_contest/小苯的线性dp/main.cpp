...
// Start: Sat 29 Mar 2025 11:32:07 AM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    for (int k{n}; k != 2; --k) {
        auto l = n - k + 1;
        std::vector<i64> pref(n + 2);
        std::vector<i64> suff(n + 2);
        for (int i{1}; i != n + 1; ++i) {
            pref[i] = std::max(pref[i - 1], b[i] - b[std::max(0, i - l)]);
        }
        for (int i{n}; i != 0; --i) {
            suff[i] =
                std::max(suff[i + 1], b[std::min(n, i + l - 1)] - b[i - 1]);
        }
        i64 ans{};
        for (int i{1}; i != n + 1; ++i) {
            chmax(ans, std::max(pref[i - 1], suff[i + 1]) - a[i]);
        }
        std::cout << ans << ' ';
    }
    std::cout << std::max(b[n] - b[1] - a[1], b[n - 1] - a[n]) << ' ' << 0
              << '\n';
}
} // namespace
