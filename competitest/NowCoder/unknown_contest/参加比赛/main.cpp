...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/103786/E
// Start: Sat 08 Mar 2025 01:56:20 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    // f[i]: dest that player will reach if his value is score[i].
    std::vector<int> f(n);
    std::map<int, int> first_idx_of_score;
    for (int i{n - 1}; i != -1; --i) {
        if (first_idx_of_score.contains(a[i] + 1)) {
            f[i] = f[first_idx_of_score[a[i] + 1]];
        }
        else {
            f[i] = a[i] + 1;
        }
        first_idx_of_score[a[i]] = i;
    }
    ...
    for (int i{}; i != q; ++i) {
        int x;
        std::cin >> x;
        if (first_idx_of_score.contains(x)) {
            std::cout << f[first_idx_of_score[x]] - x << '\n';
        }
        else {
            std::cout << 0 << '\n';
        }
