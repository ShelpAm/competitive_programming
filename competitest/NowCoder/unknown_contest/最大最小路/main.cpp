...
// Start: Sun 29 Dec 2024 07:26:51 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto exclude{[&](std::unordered_set<int> const &s) {
        i64 ans{};
        ...
        for (int i{}; i != n; ++i) {
            ...
            if (!vis[i] && !s.contains(i)) {
                vis[i] = true;
                dfs(dfs, i);
            }
            ans += cnt * (cnt - 1) / 2;
        }
        return (static_cast<i64>(n) * (n - 1) / 2) - ans;
    }};
    ...
    std::cout << exclude(s1) + exclude(s2) - exclude(s3) << '\n';
}
} // namespace
