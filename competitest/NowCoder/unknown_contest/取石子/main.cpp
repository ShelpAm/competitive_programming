...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/97487/J*/
/*Start: Sat 07 Dec 2024 01:45:29 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
    ...
    std::deque<bool> visited(n);
    for (int i{(s - 1 + k) % n}, t{}; !visited[i]; (i += k) %= n, ++t) {
        path.push_back({a[i], t, i});
        visited[i] = true;
    }
    std::cout << std::ranges::min(path)[2] + 1 << '\n';
}
} // namespace
