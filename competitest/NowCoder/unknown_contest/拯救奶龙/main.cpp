...
// Start: Thu 13 Feb 2025 12:36:29 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::map<int, int> o;
    for (auto const [e, p] : std::views::zip(a, u) | std::views::reverse) {
        auto const &[v, c]{p};
        auto check_monotonicity{[&o](auto it) {
            while (it != o.begin() && std::prev(it)->second >= it->second) {
                o.erase(std::prev(it));
            }
        }};
        if (auto next{o.lower_bound(v)}; !o.contains(v)) {
            if (next == o.end() || c < next->second) {
                o[v] = c;
                check_monotonicity(std::prev(next));
            }
        }
        else if (chmin(o[v], c)) {
            check_monotonicity(o.find(v));
        }

        debug("o", o);
        auto it{o.lower_bound(e)};
        ans.push(it != o.end() ? it->second : -1);
    }
    while (!ans.empty()) {
        std::cout << ans.top() << ' ';
        ans.pop();
