...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/101964/B
// Start: Fri 28 Feb 2025 08:32:19 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto const t{std::ranges::max(o | std::views::values)};

    for (int x{std::min<int>(26, s.size())}; x != 0; --x) {
        auto check{[=]() mutable {
            auto const ch = std::ranges::find_if(o, [t](auto const p) {
                                return p.second == t;
                            })->first;
            auto it{o.begin()};
            auto inc{[&] { // find next available char that is not `t`
                while (it->first == ch || it->second == 0) {
                    ++it;
                    if (it == o.end()) {
                        it = o.begin();
                    }
            ...
            std::string a(s.size(), '0');
            for (auto const r : a | std::views::chunk(x)) {
                r.front() = ch;
                std::unordered_set<char> cur;
                for (auto &e : r | std::views::drop(1)) {
                    inc();
                    if (cur.contains(it->first)) { // Rounded
                        return std::string{};
                    }
                    e = it->first;
                    // --it->second;
