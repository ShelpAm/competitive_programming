...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/82401/E
// Memory Limit: 524288
// Time Limit: 2000
// Start: Wed 29 May 2024 06:24:10 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  cin >> t;
  t.insert(t.begin(), 0);
  vector<vector<u64>> c(2, vector<u64>(n + 1));
  vector<vector<u64>> s(2, vector<u64>(n + 1));
  vector<vector<u64>> q(2, vector<u64>(n + 1));
  for (int i{1}; i != n + 1; ++i) {
    auto const k{t[i] - '0'};
    ++c[k][i];
    for (auto& cnt : c) {
      (cnt[i] += cnt[i - 1]) %= mod<u64>;
    }
    s[k][i] += i;
    for (auto& sum : s) {
      (sum[i] += sum[i - 1]) %= mod<u64>;
    }
    for (auto& color : {0, 1}) {
      (q[color][i] = q[color][i - 1] + i * c[color ^ 1][i] - s[color ^ 1][i]) %=
          mod<u64>;
    }
  }
  debug("c", c);
  ...
  for (int i{}; i != m; ++i) {
    int l, r;
    cin >> l >> r;
    cout << (q[0][r] - q[0][l - 1] + q[1][r] - q[1][l - 1]) -
                (r - l + 1) *
                    (c[0][l - 1] + c[1][l - 1] - (s[0][l - 1] + s[1][l - 1]))
         << '\n';
  }
}
