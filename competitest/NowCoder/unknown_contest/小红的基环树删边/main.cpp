...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/82526/F
// Memory Limit: 524288
// Time Limit: 2000
// Start: Sun 26 May 2024 08:12:10 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  auto [dis, prev]{graph::dijkstra(g, 0)};
  set<pair<int, int>> path{};
  int t{n - 1};
  while (t != 0) {
    path.insert({prev[t], t});
    path.insert({t, prev[t]});
    t = prev[t];
  }
  int re{inf<int>};
  vector<int> vis(n);
  vector<int> di(n, inf<int>);
  graph::tarjan_bridges tarjan(g);
  tarjan.run(0, -1);
  sort(tarjan.bridges.begin(), tarjan.bridges.end());
  for (int i{}; i != n; ++i) {
    if (path.count(edges[i])) {
      if (binary_search(tarjan.bridges.begin(), tarjan.bridges.end(),
                        edges[i])) {
        cout << -1 << '\n';
      }
      else {
        if (re == inf<int>) {
          function<void(int, int, int)> dfs{[&](int u, int p, int d) {
            ...
            for (auto const [_, v] : g.edges_of(u)) {
              if (v != p &&
                  (pair<int, int>{u, v} != edges[i] &&
                   pair<int, int>{v, u} != edges[i]) &&
                  (check_min(di[v], d + 1) || !vis[v])) {
                vis[v] = true;
                dfs(v, u, d + 1);
              }
            }
