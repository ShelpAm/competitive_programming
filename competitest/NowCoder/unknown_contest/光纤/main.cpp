...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/100672/M
// Start: Sun 02 Mar 2025 02:50:30 PM CST
// Author: ShelpAm
...
void solve_case()
{
    double n, r, h, x0, y0, vx, vy;
    std::cin >> n >> r >> h >> x0 >> y0 >> vx >> vy;

    auto const d{std::sqrt((vx * vx) + (vy * vy))};

    auto get_time{[&](auto x0, auto y0, auto vx, auto vy) {
        auto const a{(vx * vx) + (vy * vy)};
        auto const b{2 * (x0 * vx + y0 * vy)};
        auto const c{(x0 * x0) + (y0 * y0) - (r * r)};
        auto const lambda{(-b + std::sqrt((b * b) - (4 * a * c))) / (2 * a)};
        return lambda;
    }};

    auto const t1{get_time(x0, y0, vx, vy)};
    x0 += vx * t1;
    y0 += vy * t1;
    vx *= -1;
    vy *= -1;
    auto const t2{get_time(x0, y0, vx, vy)};

    auto const L{h / (t1 + t2 * n)};
    auto const R{h / (t1 + t2 * (n - 1))};
