...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/103786/K
// Start: Sat 08 Mar 2025 02:25:02 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector f(n + 1, std::vector<i64>(4));
    for (int i{n - 1}; i != -1; --i) {
        for (int j{}; j != 4; ++j) {
            if (s[i] == 'K') {
                chmax(f[i][j], a + f[i + 1][j]); // a
                if (j - 1 >= 0) {
                    chmax(f[i][j], b + f[i + 1][j - 1]); // b
                }
            }
            else if (s[i] == 'J') {
                if (j + 1 <= 3) {
                    chmax(f[i][j], f[i + 1][j + 1]); // K
                }
                chmax(f[i][j], a + f[i + 1][j]); // a
                if (j >= 1) {
                    chmax(f[i][j], (2 * b) + f[i + 1][j]); // 2*b
                }
            }
        }
    }
