...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/103786/F
// Start: Sat 08 Mar 2025 02:52:31 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<std::vector<int>> f(msb(static_cast<u64>(n)) + 1,
                                    std::vector<int>(n));
    std::vector<int> d(n);
    std::vector<std::vector<int>> prefix(n, std::vector<int>(26));
    auto init{[&](auto init, int u, int p, int depth) -> void {
        prefix[u] = prefix[p];
        ++prefix[u][s[u] - 'a'];
    ...
    auto lca{[&](int u, int v) {
        ...
        // d is lower than v
        auto const abs{std::abs(d[u] - d[v])};
    ...
    for (int i{}; i != q; ++i) {
        ...
        auto const anc{lca(u, v)};
        std::vector<int> cnt(26);
        // prefix[u] + prefix[v] - prefix[a] - prefix[f[0][a]];
        auto add{[&cnt](std::vector<int> const &delta, int sign) {
            for (int i{}; i != 26; ++i) {
                cnt[i] += sign * delta[i];
            }
        }};
