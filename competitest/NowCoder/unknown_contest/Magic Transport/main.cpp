/*Problem: Magic Transport*/
/*Contest: unknown_contest*/
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/82345/H*/
/*Start: Fri 27 Sep 2024 06:29:18 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::multiset<int> s;
  i64 sum{};
  for (int idx{}, last{inf<int>}; auto const e : a) {
    m -= e;
    ++idx;
    if (sum + last <= m) {
      s.insert(last);
    ...
    else if (!s.empty() && sum + last - *s.rbegin() <= m) {
      sum += last - *s.rbegin();
      s.extract(*s.rbegin());
      s.insert(last);
    }
    std::cout << idx - (s.size() + 1) << ' ';
    m += e;
    last = e;
  }
  std::cout << '\n';
