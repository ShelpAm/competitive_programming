// Problem: Groundhog Chasing Death
// Contest: unknown_contest
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/80998/E
// Memory Limit: 524288
// Time Limit: 4000
// Start: Thu 02 May 2024 01:29:11 PM CST
// Author: ShelpAm
...
auto solve_case() {
  ...
  for (ll t : vec) {
    ...
    while (i <= b || j <= d) {
      if (i > b) {
        mp3[t] += (j + d) * (j + d + 1) / 2 * (b - a);
        break;
      }
      if (j > d) {
        mp3[t] += (i + b) * (i + b + 1) / 2 * (d - c);
        break;
      }
      if (mp1[t] * i > mp2[x] * j) {
        mp3[t] += j * (i - a + 1);
        j++;
      } else if (mp1[t] * i < mp2[t] * j) {
        mp3[t] += i * (j - c + 1);
        i++;
      } else {
        mp3[t] += i * (j - c + 1);
