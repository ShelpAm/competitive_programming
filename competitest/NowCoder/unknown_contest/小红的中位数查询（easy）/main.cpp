/*Problem: 小红的中位数查询（easy）*/
/*Contest: unknown_contest*/
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/91177/E*/
/*Start: Sun 29 Sep 2024 07:17:26 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  auto const d{queries[0].second - queries[0].first + 1};
  ...
  ans[{0, d - 1}] = s.find_by_order(d / 2)->first;
  for (int i{d}; i != n; ++i) {
    s.insert({a[i], i});
    s.erase({a[i - d], i - d});
    ans[{i - d + 1, i}] = s.find_by_order(d / 2)->first;
  }
  debug("ans", ans);

  for (auto [l, r] : queries) {
    --l, --r;
    assert(ans.contains({l, r}));
    std::cout << ans[{l, r}] << '\n';
  }
}
} // namespace
