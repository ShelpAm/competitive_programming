...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/84781/D
// Memory Limit: 524288
// Time Limit: 2000
// Start: Fri 07 Jun 2024 08:10:06 PM CST
// Author: ShelpAm
...
void solve_case() {
  ...
  auto dfs{[&](auto self, int u, int d, int p) -> void {
    ...
    for (auto [_, v] : g.edges_of(u)) {
      ...
      self(self, v, d + 1, u);
      check_max(mx_depth[u], mx_depth[v]);
      q.push(mx_depth[v]);
      if (q.size() == 3) {
        q.pop();
      }
    ...
    if (q.size() >= 1) {
      ...
      if (q.size() >= 1) {
        auto second{q.top() - d};
        q.pop();
        check_max(dd, best + 1 + second);
      }
      check_max(dd, best + 1 + d);
    }
  }};
  dfs(dfs, 0, 0, -1);
  /*for (auto e : mx_depth) {*/
