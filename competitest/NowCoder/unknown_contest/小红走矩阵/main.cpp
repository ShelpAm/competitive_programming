...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/86387/F*/
/*Start: Mon 29 Jul 2024 01:50:48 AM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  auto bfs{[&](std::vector<std::pair<int, int>> const &dirs) {
    std::vector<std::vector<std::vector<int>>> f(
        2, std::vector<std::vector<int>>(n, std::vector<int>(m, inf<int>)));
    f[0][0][0] = 0;
    std::queue<std::tuple<int, int, int>> q; // used x y
    q.push({0, 0, 0});
    while (!q.empty()) {
      auto const [k, x, y]{q.front()};
      q.pop();
      for (auto [dx, dy] : dirs) {
        dx += x;
        dy += y;
        ...
        if (a[dx][dy] == 'X') {
          if (check_min(f[1][dx][dy], f[0][x][y] + 1)) {
            q.push({1, dx, dy});
          }
        }
        else {
          for (auto const nk : {0, 1}) {
            if (check_min(f[nk][dx][dy], f[nk][x][y] + 1)) {
              q.push({nk, dx, dy});
            }
          }
        }
    ...
    return std::make_pair(f[0][n - 1][m - 1], f[1][n - 1][m - 1]);
  }};
  std::vector<std::pair<int, int>> dirs{{-1, 0}, {0, -1}, {1, 0}, {0, 1}};
  int ans{inf<int>};
  check_min(ans, bfs(dirs).first);
  for (int i{}; i != dirs.size(); ++i) {
    auto const tmp{dirs[i]};
    dirs.erase(dirs.begin() + i);
    auto const [p, q]{bfs(dirs)};
    check_min(ans, std::min(p, q));
    dirs.insert(dirs.begin() + i, tmp);
  }
  std::cout << (ans != inf<int> ? ans : -1);
}
} // namespace
