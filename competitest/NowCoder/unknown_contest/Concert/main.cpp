...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/82345/G*/
/*Start: Fri 27 Sep 2024 06:06:22 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::vector<std::vector<i64>> f(2, std::vector<i64>(n, -inf<i64>));
  int fl{};
  f[fl][0] = a[0];

  for (int i{}; i != k; ++i) {
    fl ^= 1;
    f[fl].assign(n, -inf<i64>);
    for (int u{}; u != n; ++u) {
      for (auto [_, v] : g.edges_of(u)) {
        chmax(f[fl][v], f[fl ^ 1][u] + a[v]);
      }
    }
  }

  std::cout << (f[fl][0] < -inf<int> ? -1 : f[fl][0]) << '\n';
}
} // namespace
