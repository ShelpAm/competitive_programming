...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/101921/E
// Start: Mon 17 Feb 2025 09:26:33 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<std::tuple<int, int>> prev(k + 1); // prev, count, i
    std::deque<bool> f(k + 1);                     // previous ok
    f[0] = true;
    for (int i{n}, s{}; i != 0; --i) {
        s += i; // s: value of an item
        for (int j{s}; j <= k; ++j) {
            if (f[j - s]) {
                f[j] = true;
                prev[j] = {j - s, i};
    ...
    for (auto const e : ans | std::views::drop(1)) {
        std::cout << e << ' ';
    }
    std::cout << '\n';
}
