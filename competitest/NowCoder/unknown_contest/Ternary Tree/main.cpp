/*Problem: Ternary Tree*/
/*Contest: unknown_contest*/
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/82345/B*/
/*Start: Fri 27 Sep 2024 01:24:55 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::vector<i64> cnt_of_height(n + 1);
  for (int i{}; i != cnt_of_height.size(); ++i) {
    cnt_of_height[i] = (pow(3LL, i, inf<i64>) - 1) / 2;
  }
  std::ranges::reverse(cnt_of_height);
  cnt_of_height.insert(cnt_of_height.begin(), 0);

  auto path_from_root{[](auto x) {
    std::vector<i64> path;
    while (x != 0) {
      path.push_back(x);
      x = (x + 1) / 3;
  ...
  debug("cntinheight", cnt_of_height);

  std::unordered_map<i64, i64> size;
  std::unordered_set<i64> removed;
  for (int i{}; i != m; ++i) {
    i64 x;
    std::cin >> x;
    auto path{path_from_root(x)};

    if (!size.contains(x)) {
      size[x] = cnt_of_height[path.size()];
    }
    auto const diff{
        std::ranges::all_of(path, [&](auto x) { return !removed.contains(x); })
            ? size[x]
            : 0};
