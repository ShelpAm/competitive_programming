...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/99287/E
// Start: Sat 28 Dec 2024 07:39:38 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::string s;
    std::cin >> s;
    auto const g{graph::Graph::read(n, n - 1, false, false)};

    std::string a(n, 0);

    std::vector<int> d(n);
    auto init{[&](auto dfs, int u, int p) -> void {
        d[u] = s[u] == 'B' ? 1 : -1;
        for (auto const &[_, v] : g.edges_of(u)) {
            if (v != p) {
                dfs(dfs, v, u);
    ...
    auto dfs{[&](auto dfs, int u, int p, int x, int y) -> void {
        x += std::abs(d[u] + 2) - std::abs(d[u]);
        y += std::abs(d[u] - 2) - std::abs(d[u]);

        a[u] = s[u] == 'R' && x < 0 || s[u] == 'B' && y < 0 ? '1' : '0';
