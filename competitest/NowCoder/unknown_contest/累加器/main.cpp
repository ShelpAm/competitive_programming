...
/*Start: Wed 31 Jul 2024 01:34:36 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  auto const diff{[](unsigned const i) {
    std::bitset<24> p{i}, q{i - 1};
    int cnt{};
    for (int i{}; i != p.size(); ++i) {
      cnt += p.test(i) != q.test(i);
    }
    return cnt;
  }};
  std::vector<int> pref(2e6 + 1);
