...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/88527/I*/
/*Start: Wed 14 Aug 2024 02:14:56 PM CST*/
/*Author: ShelpAm*/
...
void solve_case() noexcept
{
  ...
  std::vector<i64> dist(1e6 + 1);
  for (auto &e : a) {
    std::cin >> e;
    ++cnt[e];
  }
  for (i64 i{1}; i != dist.size(); ++i) {
    dist[i] += dist[i - 1] + cnt[i] * i;
    cnt[i] += cnt[i - 1];
  }
  i64 ans{inf<i64>};
  for (i64 i{1}; i != dist.size(); ++i) {
    chmin(ans, dist.back() - 2 * dist[i] + (2 * cnt[i] - cnt.back()) * i);
  }
  std::cout << ans + 1 << '\n';
}
} // namespace
