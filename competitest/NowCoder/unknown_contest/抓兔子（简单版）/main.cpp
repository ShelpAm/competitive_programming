...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/84781/C
// Memory Limit: 524288
// Time Limit: 2000
// Start: Fri 07 Jun 2024 06:31:13 PM CST
// Author: ShelpAm
...
void solve_case() {
  ...
  vector<int> f(1 << n, inf<int>);
  f[0] = 0;
  set<int> path;
  auto get_from{[&](int mid) {
    int from{};
    for (int j{}; j != n; ++j) {
      if (mid & 1 << j) {
        for (auto v : adj[j]) {
  ...
  auto dfs{[&](auto self, int mask) -> int {
    ...
    for (int i{}; i != n; ++i) {
      if (mask & 1 << i) {
        if (auto from{get_from(mask ^ 1 << i)}; !path.count(from)) {
          path.insert(from);
          check_min(f[mask], self(self, from) + 1);
          path.erase(from);
        }
      }
    }
