...
// Start: Thu 06 Feb 2025 04:41:10 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector k(31, std::vector<i64>(n + 1));
    std::vector o(31, std::vector<std::array<i64, 2>>(n + 1));
    std::vector prefix(31, std::vector<std::array<i64, 2>>(n + 1));
    for (int i{}; i != 31; ++i) {
        for (int j{}; j != n; ++j) {
            o[i][j + 1] = o[i][j];
            ++o[i][j + 1][a[j] >> i & 1];
            prefix[i][j + 1] = prefix[i][j];
            ++prefix[i][j + 1][b[j] >> i & 1];
            k[i][j + 1] = k[i][j];
            k[i][j + 1] += o[i][j + 1][(b[j] >> i & 1) ^ 1];
            k[i][j + 1] %= mod1e9p7;
        }
    }
    ...
    for (int i{}; i != q; ++i) {
        ...
        std::cin >> l >> r;
        i64 ans{};
        for (int bit{}; auto const &[t, c, d] : std::views::zip(k, o, prefix)) {
            ans += ((t[r] - t[l - 1]) % mod1e9p7 << bit) % mod1e9p7;
            (ans += mod1e9p7) %= mod1e9p7;
            ans -= ((d[r][0] - d[l - 1][0]) * c[l - 1][1] % mod1e9p7 << bit) %
                   mod1e9p7;
            (ans += mod1e9p7) %= mod1e9p7;
            ans -= ((d[r][1] - d[l - 1][1]) * c[l - 1][0] % mod1e9p7 << bit) %
                   mod1e9p7;
            (ans += mod1e9p7) %= mod1e9p7;
            ++bit;
        }
