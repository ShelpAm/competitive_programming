...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/78806/C
// Memory Limit: 524288
// Time Limit: 4000
// Start: Sat 27 Apr 2024 02:33:08 PM CST
// Author: ShelpAm
...
auto solve_case() {
  ...
  for (auto [pos, num] : in) {
    cnt += num;
    while (cnt >= k + i->second) {
      cnt -= i->second;
      ++i;
    }
    if (cnt >= k) {
      auto const nextpos{++in.find(pos)};
      if (nextpos != in.end()) {
        ans += (nextpos->first - pos) * i->first;
      }
    }
  }
  if (cnt >= k) {
    ans += (m - in.rbegin()->first + 1) * i->first;
  }
  cout << ans;
}
