...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/92745/G*/
/*Start: Sat 12 Oct 2024 02:16:51 PM CST*/
/*Author: ShelpAm*/
...
auto bellman_ford(Graph const &g, int source, std::vector<int> &visited)
    -> Bellman_ford_result
{
...
auto contract_edges(Graph const &g) -> Contract_edges_result
{
  ...
  return Contract_edges_result{.h = h, .scc_id = scc_id, .scc = scc};
}
} // namespace graph
void solve_case()
{
  ...
  for (int i{}; i != n; ++i) {
    for (int j{}; j != n; ++j) {
      for (auto [dx, dy] : dirs) {
        ...
        if (a[dx][dy] == 'o' && a[i][j] == 'o') {
          g.add_edge(i * n + j, dx * n + dy, 1);
        }
      }
    }
  }

  auto const [h, vertex_scc_id, scc]{graph::contract_edges(g)};
  std::vector<int> liberty(h.size());
  std::vector<std::deque<bool>> vis(n, std::deque<bool>(n));
  auto calc_scc_liberty{
      [&](auto self, std::size_t scc_id, int x, int y) -> void {
        for (auto [i, j] : dirs) {
          i += x, j += y;
          if (i < 0 || i >= n || j < 0 || j >= n || vis[i][j]) {
            continue;
          }
          if (a[i][j] == 'o' && vertex_scc_id[(i * n) + j] == scc_id) { // White
            // and in the same SCC
            vis[i][j] = true;
            self(self, scc_id, i, j);
          }
          else if (a[i][j] == '.') { // Space
            ++liberty[scc_id];
          }
  ...
  for (int i{}; i != n; ++i) {
    for (int j{}; j != n; ++j) {
      if (a[i][j] == 'o' && !vis[i][j]) {
        vis[i][j] = true;
        calc_scc_liberty(calc_scc_liberty, vertex_scc_id[(i * n) + j], i, j);
        if (liberty[vertex_scc_id[(i * n) + j]] == 0) {
          initial_death += scc[vertex_scc_id[(i * n) + j]].size();
        }
      }
    }
  }

  // Traverse the re-formed tree.
  std::deque<bool> tree_visited(n * n);    // Of SCC
  std::vector<int> subtree_liberty(n * n); // Of SCC
  std::vector<int> size(n * n);            // Of SCC
  std::vector<int> ans(n * n);             // Answer for original vertices.
  auto dfs_tree{[&](auto self, int u, int p, int r) -> void {
    debug("u", u);
    tree_visited[u] = true;
    size[u] = scc[u].size();
    for (auto const &[_, v] : h.edges_of(u)) {
      if (v != p) {
        self(self, v, u, r);
        subtree_liberty[u] += subtree_liberty[v];
        size[u] += size[v];
        if (subtree_liberty[v] == 0 && scc[u].size() == 1) {
          ans[scc[u].front()] += size[v];
        }
      }
    }
    if (subtree_liberty[r] - subtree_liberty[u] == 0 && scc[u].size() == 1) {
      ans[scc[u].front()] += size[r] - size[u];
    }
    debug("size", size[u]);
  }};
  for (int i{}; i != n; ++i) {
    for (int j{}; j != n; ++j) {
      auto const scc_id{vertex_scc_id[i * n + j]};
      if (a[i][j] == 'o' && !tree_visited[scc_id]) {
        tree_visited[scc_id] = true;
        dfs_tree(dfs_tree, scc_id, -1, scc_id);
      }
    }
  }
  ...
  for (int i{}; i != n; ++i) {
    for (int j{}; j != n; ++j) {
      if (a[i][j] == 'o') {
        std::cout << ans[i * n + j] + initial_death << ' ';
      }
    }
  }
  std::cout << '\n';
