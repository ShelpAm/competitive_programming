...
// Memory Limit: 524288
// Time Limit: 4000
// Start: Tue 11 Jun 2024 08:16:38 PM CST
// Author: ShelpAm
...
[[maybe_unused]] constexpr std::int_fast64_t mod1e9{10000000007};
template <typename T>
...
void solve_case()
{
  ...
  vector<vector<int>> height(n + 1, vector<int>(m));
  for (int i{}; i != m; ++i) {
    for (int j{n - 1}; j != -1; --j) {
      height[j][i] = a[j][i] == 1 ? 0 : height[j + 1][i] + 1;
    }
  }
  i64 ans{};
  for (int i{}; i != n; ++i) {
    ...
    for (auto h : height[i]) {
      ...
      while (!s.empty() && s.top().first > h) {
        area += (h - s.top().first) * s.top().second;
        w += s.top().second;
        s.pop();
      }
      area += h;
