...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/99287/D
// Start: Fri 27 Dec 2024 09:52:10 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    // std::vector a(2, 0);
    // std::map<int, std::set<std::vector<int>>> t;
    // for (int i{1}; i != 5; ++i) {
    //     std::ranges::iota(a, i);
    //     do {
    //         int s{};
    ...
    // }
    // debug("t keys", t | std::views::keys);
    // debug("t", t);

    auto const x{binary_search(
        [&](i64 x) { return m >= x * (x + 1) * (x + 2) / 6; }, 0, 2e6)};
