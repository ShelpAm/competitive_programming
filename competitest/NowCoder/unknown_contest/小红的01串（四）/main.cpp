...
// Start: Sun 15 Dec 2024 07:27:30 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<std::vector<int>> r(n, std::vector<int>(2)); // r[i][x]
    // means first char of inequality x, at right.
    for (int i{}; i != n; ++i) {
        for (auto const x : {0, 1}) {
            auto k = s.find(s[i] ^ x, i + 1);
            r[i][x] = k == std::string::npos ? i : k;
        }
    }

    std::vector<i64> f(n, inf<i64>);
    f[0] = 0;
    for (int i{}; i != n; ++i) {
        chmin(f[r[i][0]], f[i] + x);
        chmin(f[r[i][1]], f[i] + y);
