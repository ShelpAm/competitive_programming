...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/104421/E
// Start: Sat 15 Mar 2025 04:30:05 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    vec<int> r(n);
    std::cin >> r;
    auto g{graph::Graph::read(n + 2 * k, m, true, true)};
    int p;
    std::cin >> p;
    for (int i{}; i != p; ++i) {
        int u, v, w;
        std::cin >> u >> v >> w;
        g.add_edge(--u + n + k, --v + n, w); // W -> R
    }
    vec<int> h(n);
    std::cin >> h;
    ...
    for (int i{}; i != n; ++i) {
        g.add_edge(r[i] + n, i, h[i]);  // R
        g.add_edge(i, r[i] + n + k, 0); // W
    }

    auto [dist, prev]{graph::dijkstra(g, 0)};
    std::cout << (dist[n - 1] == graph::infinity ? -1 : dist[n - 1]) << '\n';
}
} // namespace
