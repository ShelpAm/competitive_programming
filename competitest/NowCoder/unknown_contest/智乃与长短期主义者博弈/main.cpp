...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/101918/D
// Start: Fri 14 Feb 2025 08:06:45 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::function<int(int, int)> f{[&a, &f, &g](int l, int r) {
        if (l > r) {
            return 0;
        }
        ...
        return g[l][r] = std::max(
                   a[l] + (a[l + 1] >= a[r] ? f(l + 2, r) : f(l + 1, r - 1)),
                   a[r] + (a[l] >= a[r - 1] ? f(l + 1, r - 1) : f(l, r - 2)));
    }};

    auto const second{a[0] >= a[n - 1] ? f(1, n - 1) : f(0, n - 2)};

    std::cout << sum_of(a) - second << ' ' << second << '\n';
}
} // namespace
