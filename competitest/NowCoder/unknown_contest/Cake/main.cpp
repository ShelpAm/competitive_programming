...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/81601/A*/
/*Start: Thu 01 Aug 2024 12:18:03 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  int n;
  std::cin >> n;
  auto const g{graph::read_graph(n, n - 1, false, true, true)};
  std::vector<long double> f(n);
  auto dfs{[&](auto self, int const u, int const p, int const n0,
               int const n1) -> void {
    bool const first_turn{(n0 + n1) % 2 == 0};
    f[u] = !first_turn;
    for (auto const &[w, v] : g.edges_of(u)) {
      if (v != p) {
        self(self, v, u, n0 + (w == 0), n1 + (w == 1));
    ...
    check_min(f[u], static_cast<double>(n1) / (n0 + n1));
    if (u != 0 && g.edges_of(u).size() == 1) {
      f[u] = static_cast<double>(n1) / (n0 + n1);
    }
  }};
  dfs(dfs, 0, 0, 0, 0);
  std::cout << f[0] << '\n';
