...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/80186/F
// Memory Limit: 1048576
// Time Limit: 10000
// Start: Wed 01 May 2024 01:47:05 PM CST
// Author: ShelpAm
...
auto solve_case() {
  ...
  vector<int> cntc(N), cntr(N), col(N), row(N);
  for (auto &[x, y] : p) {
    cin >> x >> y;
    ++exist[{x, y}];
    ++cntc[x], ++cntr[y];
  ...
  for (int i{}; i != N; ++i) {
    ...
    check_max(maxrow, row[i]);
  }

  vector<int> cols, rows;
  for (int i{}; i != N; ++i) {
    if (col[i] > maxcol - 9 && col[i] != 0) {
      cols.push_back(i);
    }
    if (row[i] > maxrow - 9 && row[i] != 0) {
      rows.push_back(i);
    }
  }

  using ll = long long;
  function<ll(ll, ll)> ch{[r, &p = col, &q = row, &mp = exist](ll x, ll y) {
    ll cnt = p[x] + q[y];
    for (ll i = 0; i < 3; i++) {
      for (ll j = 0; j < 3; j++) {
        if (mp.count({x + i * r, y + j * r})) {
          cnt -= mp[{x + i * r, y + j * r}];
        }
      }
    }
