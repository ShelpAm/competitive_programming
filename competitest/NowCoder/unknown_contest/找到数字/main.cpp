...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/82612/C
// Memory Limit: 524288
// Time Limit: 2000
// Start: Fri 17 May 2024 07:11:50 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  function<void(int, int, u64)> check{[&](int i, int v, u64 cur) -> void {
    auto const f{cur % pow10[i] + cur / 10};
    if (f >= p) {
      if (f == p && v != 0) {
        ++cnt;
      }
    ...
    auto const next{(10 + num[i] - v) % 10};
    check(i + 1, next, cur + pow10[i + 1] * next);
  }};
  for (int i{}; i != 10; ++i) {
    check(0, i, i);
  }
