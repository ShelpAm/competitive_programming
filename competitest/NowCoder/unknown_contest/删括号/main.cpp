...
// Memory Limit: 2
// Time Limit: 21303000
// Start: Mon 03 Jun 2024 06:29:17 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  auto const m{static_cast<int>(t.size())};
  s.insert(s.begin(), 0);
  t.insert(t.begin(), 0);
  ...
  for (int i{1}; i != n + 1; ++i) {
    for (int j{1}; j != m + 1; ++j) {
      if (s[i] == t[j]) {
        for (int k{}; k < n + 1; k += 2) {
          check_max(f[i][j][0], f[i - 1][j - 1][k]);
        }
      }
      for (int k{}; k + 2 < n + 1; k += 2) {
        check_max(f[i][j][k + 1], f[i - 1][j][k]);
        if (s[i - 1] == '(' && s[i] == ')') {
          check_max(f[i][j][k + 2], f[i - 1][j][k + 1]);
        }
      }
      /*debug("i j", pair{i, j});*/
      /*debug("f[i][j]", f[i][j]);*/
