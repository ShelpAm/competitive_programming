...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/102303/F
// Start: Sun 23 Feb 2025 09:18:19 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto const k{msb(n - 1) + 1};
    std::vector<int> a{1};
    int j{1};
    for (int i{}; i != k; ++i) {
        // First append (a.size() - 1) elements from the begining of a, then
        // increase j, and append j to a.
        auto const sz{a.size() - 1};
        a.insert(a.end(), a.begin(), a.begin() + sz);
        a.push_back(++j);
    }
    ...
    std::cout << max << '\n';
    for (auto const e : a | std::views::take(n)) {
        std::cout << e << ' ';
    }
}
} // namespace
