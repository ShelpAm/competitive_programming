/*Problem: Puzzle: Wagiri*/
/*Contest: unknown_contest*/
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/81601/D*/
/*Start: Thu 01 Aug 2024 01:56:30 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::stack<int> circle;
  auto is_circled{[&](auto self, int const u, int const p) -> void {
    std::cerr << u << ' ';
    for (auto const &[w, v] : g.edges_of(u)) {
      if (v != p && w) {
        if (vis[v] < 2) {
    ...
    if (!circle.empty() && circle.top() == u) {
      circle.pop();
    }
    if (!circle.empty() && circle.top() != -1) {
      vis[u] = 2;
    }
  }};
  for (int i{}; i != n; ++i) {
  ...
  for (int i{}; i != n; ++i) {
    ...
    for (auto const &[_, v] : g.edges_of(i)) {
      if (vis[i] == 2 && vis[v] == 2) {
        ans.push_back({i, v});
      }
    }
  ...
  if (std::any_of(vis.begin(), vis.end(), [](auto const e) { return !e; })) {
    std::cout << "NO\n";
    return;
  }
  for (auto &[k, v] : ans) {
