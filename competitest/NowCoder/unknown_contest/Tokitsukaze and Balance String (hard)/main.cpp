...
// Start: Thu 06 Feb 2025 03:47:11 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto const mid{std::ranges::count(s.begin() + 1, s.end() - 1, '?')};
    auto const head_tail{(s.front() == '?') + (s.back() == '?')};
    if (head_tail == 0) {
        if (s.front() == s.back()) {
            std::cout << (s.size() - 2) * pow(2ULL, mid, mod1e9p7) % mod1e9p7
                      << '\n';
        }
        else {
            std::cout << 2 * pow(2ULL, mid, mod1e9p7) % mod1e9p7 << '\n';
        }
    }
    else if (head_tail == 1) {
        std::cout << s.size() * pow(2ULL, mid, mod1e9p7) % mod1e9p7 << '\n';
    }
    else { // head_tail == 2
        std::cout << 2 * s.size() * pow(2ULL, mid, mod1e9p7) % mod1e9p7 << '\n';
    }
}
} // namespace
