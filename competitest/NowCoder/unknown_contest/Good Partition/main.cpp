/*Problem: Good Partition*/
/*Contest: unknown_contest*/
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/82345/C*/
/*Start: Fri 27 Sep 2024 02:05:34 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::set<int> fuck{a.begin(), a.end()};
  ...
  disjoint_set_union::Dsu d(2 * R + 1);
  for (auto const e : a) {
    for (auto const v : s) {
      auto const f{v - e};
      if (!fuck.contains(f)) { // Without this, there will be more restrictions,
        // like unprecedented f cannot be in the same set of e, **which results
        // in incorrect relationships between present numbers.**
        continue;
      }
      if (0 <= f && f <= R && e != f) {
        d.unite(e, f + R);
  ...
  std::cout << (std::ranges::any_of(std::views::iota(1, R + 1),
                                    [&](auto e) { return d.united(e, e + R); })
                    ? "NO\n"
                    : "YES\n");
}
} // namespace
