...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/85347/C
// Memory Limit: 524288
// Time Limit: 2000
// Start: Sat 29 Jun 2024 09:16:08 PM CST
// Author: ShelpAm
...
void solve_case()
{
  ...
  int n, x;
  cin >> n >> x;
  vector<vector<u64>> f(n + 1, vector<u64>(sqrt(x) + 1));
  vector<vector<u64>> s(n + 1, vector<u64>(sqrt(x) + 1));
  for (auto &e : f[1]) {
    e = 1;
  }
  iota(s[1].begin(), s[1].end(), 0);
  for (int i{2}; i != n + 1; ++i) {
    for (int j{1}; j * j <= x; ++j) {
      f[i][j] = s[i - 1][j - 1];
    }
    for (int j{1}; j * j <= x; ++j) {
      s[i][j] = (s[i][j - 1] + f[i][j]) % mod1e9p7;
    }
  }
  debug("f", f);
  debug("s", s);
