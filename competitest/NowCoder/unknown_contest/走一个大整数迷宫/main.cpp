...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/85347/D
// Memory Limit: 524288
// Time Limit: 2000
// Start: Sat 29 Jun 2024 09:35:29 PM CST
// Author: ShelpAm
...
void solve_case()
{
  ...
  vector<vector<int>> c(n, vector<int>(m, p));
  for (auto &e : a) {
    for (auto &f : e) {
      cin >> f;
    }
  ...
  for (int i{}; i != n; ++i) {
    for (int j{}; j != m; ++j) {
      for (int k{}; k != b[i][j]; ++k) {
        c[i][j] = (c[i][j] * c[i][j]) % (p - 1);
      }
      c[i][j] = (a[i][j] * b[i][j]) % (p - 1);
    }
  }
  vector<vector<vector<int>>> dist(
      n, vector<vector<int>>(m, vector<int>(p - 1, inf<int>)));
  vector<pair<int, int>> dirs{{1, 0}, {0, 1}, {-1, 0}, {0, -1}};
  queue<tuple<int, int, int>> q;
  q.push({0, 0, c[0][0]});
  dist[0][0][c[0][0]] = 0;
  while (!q.empty()) {
    assert(q.size() <= 1.5 * n * m * p);
    auto [x, y, r]{q.front()};
    q.pop();
    for (auto [i, j] : dirs) {
      i += x, j += y;
      if (i >= 0 && i < n && j >= 0 && j < m) {
        if (auto neo{(r + c[i][j]) % (p - 1)};
            check_min(dist[i][j][neo], dist[x][y][r] + 1)) {
          q.push({i, j, neo});
        }
      }
    }
