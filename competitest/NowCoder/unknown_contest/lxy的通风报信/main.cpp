...
/*Start: Wed 24 Jul 2024 04:28:19 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  auto bfs{[&](int const x, int const y) {
    std::queue<std::pair<int, int>> q;
    q.push({x, y});
    std::vector<std::vector<int>> dist(n, std::vector<int>(m, inf<int>));
    dist[x][y] = 0;
    while (!q.empty()) {
      ...
      for (auto [di, dj] : dirs) {
        di += i;
        dj += j;
        if (di >= 0 && di < n && dj >= 0 && dj < m && a[di][dj] != '#' &&
            check_min(dist[di][dj], dist[i][j] + 1)) {
          q.push({di, dj});
        }
      }
    }
    for (int i{}; i != n; ++i) {
      for (int j{}; j != m; ++j) {
        if (a[i][j] == '*' && dist[i][j] != inf<int>) {
          g.add_edge(id[{x, y}], id[{i, j}], dist[i][j]);
        }
      }
    }
  }};
  ...
  auto const result{graph::prim(g)};
  if (result == -1) {
    std::cout << "No\n";
  }
  else {
