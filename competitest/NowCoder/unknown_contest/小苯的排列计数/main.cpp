...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/102303/D
// Start: Sun 23 Feb 2025 08:39:20 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    if (std::ranges::any_of(std::views::iota(0, n),
                            [&](auto i) { return a[i] < a[i + 1]; }) ||
        std::ranges::any_of(std::views::iota(1, n + 1),
                            [&](auto i) { return a[i] > n - i + 1; })) {
        std::cout << 0 << '\n';
        return;
    }

    Z<mod998244353> ans{1};
    int skiped{};
    int avail{n};
    for (int i{}; i != n; ++i) {
        if (a[i] != a[i + 1]) {
