...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/91177/D*/
/*Start: Sun 29 Sep 2024 07:09:53 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  i64 ans{};
  auto dfs{[&](auto self, int u, int p, i64 prob) -> void {
    depth[u] = depth[p] + 1;
    debug("u, p, prob, depth", std::vector<i64>{u, p, prob, depth[u]});
    auto const sons{g.edges_of(u).size() - (u != 0)};
    for (auto const &[_, v] : g.edges_of(u)) {
      if (v != p) {
        self(self, v, u, prob * c.inverse(sons) % mod1e9p7);
      }
    }
    if (sons == 0) {
      (ans += prob * depth[u] % mod1e9p7) %= mod1e9p7;
    }
  }};
  dfs(dfs, 0, 0, 1);
  std::cout << ans << '\n';
