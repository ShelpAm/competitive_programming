// Problem: World Finals
// Contest: unknown_contest
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/81596/H
// Memory Limit: 2097152
// Time Limit: 4000
// Start: Tue 16 Jul 2024 12:04:42 PM CST
// Author: ShelpAm
...
{
  using namespace std;
  constexpr auto name{"lzr010506"};
  auto input{[] {
    int n;
    cin >> n;
    vector<tuple<int, int, string>> a(n);
    tuple<int, int, string> me;
    set<string> names;
    for (auto &[a, b, c] : a) {
      cin >> c >> a >> b;
      if (c == name) {
    ...
    return tuple{a, me, names};
  }};
  auto const [a1, e1, p1]{input()};
  auto const [a2, e2, p2]{input()};
  auto result{[](vector<tuple<int, int, string>> const &a,
                 tuple<int, int, string> const &me,
                 set<string> const &can_join_other) {
    return count_if(a.begin(), a.end(), [&](auto const &e) {
      return e < me && !can_join_other.count(get<2>(e));
    });
  }};
  cout << min(result(a1, e1, p2), result(a2, e2, p1)) + 1;
}
} // namespace
