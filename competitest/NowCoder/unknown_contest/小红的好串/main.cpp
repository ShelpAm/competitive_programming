...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/80742/D
// Memory Limit: 524288
// Time Limit: 2000
// Start: Sun 05 May 2024 08:38:55 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  for (int j{}; j != pref.size(); ++j) {
    for (int i{}; i != n; ++i) {
      pref[j][i + 1] = pref[j][i] + (s[i] != sv[j]);
    }
  }
  while (q--) {
    int l, r;
    ...
    auto const rem{(r - l + 1) % 3};
    auto const d{(r - l + 1) / 3};
    int ans{inf<int>};
    vector<int> extends{0, 0, 0};
    for (int i{3 - rem}; i != 3; ++i) {
      ++extends[i];
    }
    do {
      int tmp{};
      ...
      for (int i{}; i != 3; ++i) {
        if (i == 2) {
          assert(l - 1 + myoff + d + extends[i] == r);
        }
        tmp += pref[i][l - 1 + myoff + d + extends[i]] - pref[i][l - 1 + myoff];
        myoff += d + extends[i];
      }
      check_min(ans, tmp);
    } while (next_permutation(extends.begin(), extends.end()));
    cout << ans << '\n';
  }
}
