...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/84781/J
// Memory Limit: 524288
// Time Limit: 4000
// Start: Fri 07 Jun 2024 06:56:30 PM CST
// Author: ShelpAm
...
void solve_case() {
  ...
  cin >> n;
  vector<int> a(n), b(n);
  vector<i64> f(n + 1, inf<i64>);
  f[0] = 0;
  for (int i{}; i != n; ++i) {
    cin >> a[i] >> b[i];
  }
  for (int i{1}; i != n + 1; ++i) {
    auto m{min(5 * sqrt(1e5), static_cast<double>(i))};
    for (i64 d{1}; d <= m; ++d) {
      check_min(f[i], f[i - d] + a[i - d] * pow(d, 3, inf<i64>) + b[i - d]);
    }
  }
  cout << f[n];
}
