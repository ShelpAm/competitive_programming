...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/103786/L
// Start: Sat 08 Mar 2025 03:34:19 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector l(msb(static_cast<u64>(n)) + 1, std::vector<int>(n, -inf<int>));
    std::unordered_map<int, int> idx_of_last_value;
    for (int i{}; i != n; ++i) {
        if (idx_of_last_value.contains(a[i] - 1)) {
            l[0][i] = idx_of_last_value[a[i] - 1];
