...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/84781/K
// Memory Limit: 524288
// Time Limit: 2000
// Start: Fri 07 Jun 2024 07:15:21 PM CST
// Author: ShelpAm
...
void solve_case() {
  using namespace std;
  unordered_map<i64, i64> g;
  function<i64(i64)> f{[&](i64 x) -> i64 {
    if (x <= 2) {
      return 0;
    }
    if (g.count(x)) {
    ...
    auto s{1LL << msb(x - 1)};
    auto t{1LL << (msb(x - 1) - 1)};
    return g[x] = f(x - 1 - s) + t;
  }};
  i64 x;
  cin >> x;
  i64 lo{}, hi{static_cast<i64>(1e18)};
  while (lo < hi) {
    auto mid{lo + hi >> 1};
    if (f(mid) < x) {
      lo = mid + 1;
