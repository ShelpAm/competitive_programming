...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/81600/H*/
/*Start: Tue 30 Jul 2024 01:05:38 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::vector<u64> use(n);
  for (int i{}; i != n; ++i) {
    for (auto const e : g[i]) {
      use[i] |= 1ULL << e;
    }
  }
  std::vector<std::unordered_map<u64, int>> f(n);
  std::queue<std::pair<int, u64>> q;
  for (int i{}; i != n; ++i) {
    f[i][1ULL << i] = 1;
    q.push({i, 1ULL << i});
  }
  while (!q.empty()) {
    auto const [u, mask]{q.front()};
    q.pop();
    for (auto const v : g[u]) {
      if (!(mask & 1ULL << v) &&
          check_max(f[v][mask | use[u]], f[u][mask] + 1)) {
        q.push({v, mask | use[u]});
      }
    }
  }
