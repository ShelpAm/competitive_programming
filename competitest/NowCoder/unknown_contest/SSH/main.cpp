...
/*Start: Wed 31 Jul 2024 01:00:52 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::map<std::string, std::map<std::string, std::set<std::string>>> servers;
  for (int i{}; i != n; ++i) {
    std::string ip;
    int k;
    std::cin >> ip >> k;
    for (int i{}; i != k; ++i) {
      ...
      for (int j{}; j != t; ++j) {
        std::string pub;
        std::cin >> pub;
        servers[ip][user].insert(keymap[pub]);
      }
    }
  }
  for (int i{}; i != q; ++i) {
    ...
    std::cin >> user >> ip >> pri;
    std::cout << (servers[ip][user].count(pri) ? "Yes" : "No") << '\n';
  }
}
} // namespace
