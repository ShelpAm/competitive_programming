...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/83687/E
// Memory Limit: 524288
// Time Limit: 2000
// Start: Mon 03 Jun 2024 08:06:27 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  vector<int> f(n + 1, inf<int>);
  f[0] = 0;
  vector<int> v(n + 1, inf<int>);
  for (int i{1}; i != n + 1; ++i) {
    check_min(f[i], v[a[i]] + 1);
    check_min(v[a[i]], f[i - 1]);
  }
  /*debug("f", f);*/
  cout << (f[n] == inf<int> ? -1 : f[n]);
}
