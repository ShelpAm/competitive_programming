...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/84714/C
// Memory Limit: 524288
// Time Limit: 2000
// Start: Tue 04 Jun 2024 01:08:00 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  int n;
  cin >> n;
  constexpr auto p{mod1e9};
  vector<i64> fac(2 * n + 1, 1), inv(2 * n + 1, 1), pinv(2 * n + 1, 1);
  ...
  for (int i{1}; i != n + 1; ++i) {
    ...
    cin >> a >> b;
    for (int j{n}; j != 0; --j) {
      for (int k{a}; i * k <= min(b * i, j); ++k) {
        /*debug("i j k", vector{i, j, k});*/
        f[j] += f[j - k * i] * c(n - j + k * i, k * i) % p *
                (fac[k * i] * pow(pinv[i], k, p) % p * pinv[k] % p) % p;
        f[j] %= p;
      }
    }
  }
