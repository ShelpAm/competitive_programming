...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/85187/F
// Memory Limit: 524288
// Time Limit: 2000
// Start: Sun 23 Jun 2024 07:24:46 PM CST
// Author: ShelpAm
...
void solve_case()
{
  ...
  for (int i{}; i != n; ++i) {
    for (int j{}; j != m; ++j) {
      for (auto [k, _] : sieve.factorize(a[i][j])) {
        t[k].push_back({i, j});
      }
    }
  }
  vector<vector<int>> f(n, vector<int>(m, inf<int>));
  f[0][0] = 0;
  ...
  while (!q.empty()) {
    ...
    vis[i][j] = 1;
    if (j + 1 != m && check_min(f[i][j + 1], f[i][j] + 1)) {
      q.push({w + 1, i, j + 1});
    }
    if (i + 1 != n && check_min(f[i + 1][j], f[i][j] + 1)) {
      q.push({w + 1, i + 1, j});
    }
    for (auto [k, _] : sieve.factorize(a[i][j])) {
      for (auto [x, y] : t[k]) {
        if (check_min(f[x][y], w + 1)) {
          q.push({w + 1, x, y});
        }
      }
    }
