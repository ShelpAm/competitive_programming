...
/*Start: Fri 01 Nov 2024 09:24:06 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  int n, m, l;
  std::cin >> n >> m >> l;
  std::vector<std::vector<std::pair<int, std::bitset<100>>>> g(n);
  for (int i{}; i != n - 1; ++i) {
    int u, v;
    std::string s;
    std::cin >> u >> v >> s;
    g[--u].push_back({--v, std::bitset<100>{s}});
    g[v].push_back({u, std::bitset<100>{s}});
  }
  std::vector<int> a(m);
  std::cin >> a;
  for (auto &e : a) {
  ...
  std::cout << ans.to_string().substr(100 - l) << '\n';
}
} // namespace
