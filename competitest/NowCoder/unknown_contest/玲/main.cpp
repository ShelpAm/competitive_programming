...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/81600/A*/
/*Start: Tue 30 Jul 2024 12:01:38 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  ...
  std::vector<std::vector<int>> f(msb(static_cast<u64>(n)) + 1,
                                  std::vector<int>(n));
  std::vector<std::vector<int>> g(msb(static_cast<u64>(n)) + 1,
                                  std::vector<int>(n));
  for (int i{}; i != n; ++i) {
    std::string s;
    std::cin >> s;
    ...
    if (!pos.empty()) {
      f[0][i] = g[0][i] = m - pos.front() - 1 + (pos.size() > 1);
    }
  }
  for (int i{1}; i != f.size(); ++i) {
    for (int j{}; j + (1 << i) - 1 != n; ++j) {
      f[i][j] = std::min(f[i - 1][j], f[i - 1][j + (1 << (i - 1))]);
      g[i][j] = std::max(g[i - 1][j], g[i - 1][j + (1 << (i - 1))]);
    }
  }
  auto query_min{[&f](u64 const l, u64 const r) {
    auto k{msb(r - l + 1)};
    return std::min(f[k][l], f[k][r - (1 << k) + 1]);
  }};
  auto query_max{[&g](u64 const l, u64 const r) {
    auto k{msb(r - l + 1)};
  ...
  for (int i{}; i != q; ++i) {
    ...
    --l;
    --r;
    std::cout << std::max(r - l + query_min(l, r), (r != l) + query_max(l, r))
              << '\n';
  }
}
} // namespace
