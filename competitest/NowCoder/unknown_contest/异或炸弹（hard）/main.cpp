...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/83687/F
// Memory Limit: 1048576
// Time Limit: 4000
// Start: Mon 03 Jun 2024 08:30:06 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  int n, m;
  cin >> n >> m;
  vector<vector<int>> pref(2 * n + 2, vector<int>(2 * n + 2));
  auto f{[&](int x, int y) { return pair{x - y + n, x + y}; }};
  /*auto phi{*/
  /*    [&](int u, int v) { return pair{(u + v - n) / 2, (v - u + n) / 2}; }};*/
  for (int i{}; i != m; ++i) {
    int x, y, r;
    cin >> x >> y >> r;
    auto [u, v]{f(x - r, y)};
