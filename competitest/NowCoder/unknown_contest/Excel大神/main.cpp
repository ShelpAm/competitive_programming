...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/103786/B
// Start: Sat 08 Mar 2025 01:04:32 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    constexpr auto base{26LL};

    std::vector<i64> pow{1};
    while (pow.back() < 1e18) {
    ...
    for (int i{1}; i != pow.size(); ++i) {
        pref_pow[i] += pref_pow[i - 1];
    }
    debug("pref_pow", pref_pow);

    // First bit should be considered separately, others follow the rule of
    // modulus
    std::string s;
    for (int i = pref_pow.size() - 1; i != -1; --i) {
        if (n >= pref_pow[i]) {
            n -= pref_pow[i];
            s.assign(i + 1, 'a');
            break;
