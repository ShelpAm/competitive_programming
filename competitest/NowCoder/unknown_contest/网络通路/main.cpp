...
/*Judge: NowCoder*/
/*URL: https://ac.nowcoder.com/acm/contest/90074/F*/
/*Start: Fri 27 Sep 2024 08:57:26 PM CST*/
/*Author: ShelpAm*/
...
void solve_case()
{
  int n, m;
  std::cin >> n >> m;
  std::vector<std::vector<int>> g(n, std::vector<int>(n, inf<int>));
  for (int i{}; i != m; ++i) {
    int u, v, d;
    std::cin >> u >> v >> d;
    --u, --v;
  ...
  for (int i{}; i != 1 << n; ++i) {
    ...
    for (int j{}; j != n; ++j) {
      if (i & 1 << j) {
        for (int k{}; k != n; ++k) {
          if (i & 1 << k) {
            if (j == k || g[j][k] == inf<int>) {
              continue;
            }
            chmin(f[i], f[i ^ j] + dist);
          }
        }
      }
    }

    std::cout << std::bitset<4>{static_cast<unsigned long long>(i)}.to_string()
              << ' ' << f[i] << '\n';
  }
