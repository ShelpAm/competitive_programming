// Problem: Coffee Chicken
// Contest: unknown_contest
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/80186/B
// Memory Limit: 1048576
// Time Limit: 2000
// Start: Wed 01 May 2024 12:08:35 PM CST
// Author: ShelpAm
...
auto solve_case() {
  using namespace std;
  std::string const s1{"COFFEE"}, s2{"CHICKEN"};
  vector<u64> a{0UL, s1.size(), s2.size()};
  for (int i{3}; i != 501; ++i) {
  ...
  function<string(u64, u64)> find{[&](u64 n, u64 k) {
    if (n <= 2) {
      // debug("k", k);
      return s[n].substr(k - 1, 10);
    }
    if (a[n - 2] < k) {
      return find(n - 1, k - a[n - 2]);
    }
    if (a[n - 2] < k + 10 - 1) {
      return find(n - 2, k) + find(n - 1, 1);
    }
    return find(n - 2, k);
  }};
  int T;
  ...
  while (T--) {
    u64 n, k;
    cin >> n >> k;
    cout << find(min(n, a.size()), k).substr(0, 10) << '\n';
  }
}
