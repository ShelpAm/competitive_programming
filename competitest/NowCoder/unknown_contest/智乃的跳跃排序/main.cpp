...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/101918/E
// Start: Fri 14 Feb 2025 08:22:25 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    { // Sort by index
        ...
        std::unordered_map<int, std::vector<int>> views;
        for (int i{}; i != n; ++i) {
            pos[i % k].push_back(i);
            views[i % k].push_back(a[i]);
        }

        for (auto &[group, coll] : views) {
            std::ranges::sort(coll);
            for (auto const &[i, e] : std::views::zip(pos[group], coll)) {
                a[i] = e;
            }
        }
    }
    ...
    Dsu dsu(n);
    std::map<int, std::vector<int>> o; // indices (v) of value (k)
    for (int i{}; i != n; ++i) {
        o[a[i]].push_back(i);
    }

    for (auto const &[e, v] : o) {
        if (o.contains(e + k) || o.contains(e - k)) {
            for (auto const r : v | std::views::slide(2)) {
                dsu.unite(r[0], r[1]);
            }
            if (o.contains(e + k)) {
                dsu.unite(v[0], o[e + k][0]);
            }
            if (o.contains(e - k)) {
                dsu.unite(v[0], o[e - k][0]);
            }
    ...
    for (int i{}; i != n; ++i) {
        ...
        views[dsu.find(i)].push_back(a[i]);
    }

    for (auto &[pa, coll] : views) {
        std::ranges::sort(coll);
        for (auto const &[i, e] : std::views::zip(pos[pa], coll)) {
            a[i] = e;
        }
    }

    std::cout << (std::ranges::is_sorted(a) ? "Yes\n" : "No\n");
}
} // namespace
