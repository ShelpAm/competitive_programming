...
/*Start: Wed 14 Aug 2024 01:45:10 PM CST*/
/*Author: ShelpAm*/
...
int days_of(int const y, int const m)
{
  switch (m) {
  case 1:
  case 3:
...
void solve_case() noexcept
{
  year_month_day t{2024, 8, 1};
  year_month_day beg{2000, 1, 1};
  std::map<year_month_day, bool> f;
  f[t] = false;
  while (t != beg) {
    year_month_day p{};
    if (t.d != 1) {
      f[p = {t.y, t.m, t.d - 1}] |= !f[t];
    }
    else if (t.m != 1) {
      f[p = {t.y, t.m - 1, days_of(t.y, t.m - 1)}] |= !f[t];
    }
    else {
      f[p = {t.y - 1, 12, days_of(t.y - 1, 12)}] |= !f[t];
    }
