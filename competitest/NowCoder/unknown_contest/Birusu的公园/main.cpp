...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/82774/G
// Memory Limit: 524288
// Time Limit: 2000
// Start: Thu 30 May 2024 09:08:28 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  u64 ans{};
  for (int j{1}; j != 2 * n; ++j) {
    if (j >= n && h[j - n] - (j - n) == q.front()) {
      q.pop_front();
    }
    check_max(ans, h[j] + j - 1 + q.front());
    while (!q.empty() && q.back() < h[j] - j) {
      q.pop_back();
    }
    q.push_back(h[j] - j);
  }
