...
// Judge: AcWing
// URL: https://www.acwing.com/problem/content/3877/
// Memory Limit: 64
// Time Limit: 1000
// Start: Mon 06 May 2024 05:13:57 PM CST
// Author: ShelpAm
...
auto solve_case()
{
  ...
  i64 ans{inf<i64>};
  for (auto const e : a) {
    auto const b1{lower_bound(b.begin(), b.end(), e)},
        b2{upper_bound(b.begin(), b.end(), e)};
    auto const c1{lower_bound(c.begin(), c.end(), e)},
        c2{upper_bound(c.begin(), c.end(), e)};
    vector<i64> l1, l2;
    if (b1 != b.end()) {
      l1.push_back(*b1);
    }
    if (b2 != b.begin()) {
      l1.push_back(*prev(b2));
    }
    if (c1 != c.end()) {
      l2.push_back(*c1);
    }
    if (c2 != c.begin()) {
      l2.push_back(*prev(c2));
    }
    for (auto const e1 : l1) {
      for (auto const e2 : l2) {
        check_min(ans, abs(e - e1) + abs(e - e2) + abs(e1 - e2));
      }
    }
  }
  cout << ans;
