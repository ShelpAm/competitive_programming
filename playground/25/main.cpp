...
static inline auto solve_case()
{
  ...
  print(a);
  for (size_t i{n / 2 - 1}; i != -1UZ; --i) {
    auto parent{i}, lson{parent * 2 + 1}, rson{lson + 1};
    while ((lson = parent * 2 + 1) < n) {
      if ((rson = lson + 1) < n && a[rson] < a[lson]) {
        swap(a[parent], a[rson]);
        parent = rson;
      }
      else {
        swap(a[parent], a[lson]);
        parent = lson;
      }
    }
  }
