...
void solve_case()
{
    ...
    std::vector f(n + 1, std::vector<int>(32, inf<int>));
    // f[n] is a virtual node for conveniently representing empty node.
    f[n][0] = 0;
    f[n][1] = 1;
    for (int i{2}; i != f[n].size(); ++i) {
        f[n][i] = f[n][i - 1] + f[n][i - 2] + 1;
    }
    debug("f[n]", f[n]);
    auto dfs = [&](auto self, int u) -> void {
        ...
        for (int i{1}; i != f[u].size(); ++i) {
            for (auto l : {i - 1, i - 2}) {
                for (auto r : {i - 1, i - 2}) {
                    if ((l == i - 1 || r == i - 1) && l >= 0 && r >= 0) {
                        chmin(f[u][i], f[sons[u][0]][l] + f[sons[u][1]][r]);
                    }
                }
            }
        }
