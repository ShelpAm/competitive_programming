...
class segment_tree {
  ...
  public:
    segment_tree(u64 const size): tree_(4 * (size + 1) + 1) {}
    // The input array should start from the index 1.
    segment_tree(vi const& a) { build(1, a.size() - 1, 1, a); }
...
auto solve_case()
{
    ...
    for (u64 i = 0; i != n; ++i) {
        ...
        if (op == '!') {
            i64 d;
            cin >> d;
            st.add_to_segment(l, r, d);
        }
        else if (op == '?') {
            st.max_on_segment(l, r);
        }
        else {
            cout << "wtf op error\n";
        }
