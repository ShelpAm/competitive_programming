...
auto solve_case()
{
    ...
    vu prefix(2 * n + 1);
    for (u64 i = 1; i != prefix.size(); ++i) {
        prefix[i] = prefix[i - 1] + a[i];
    }

    vvu f(2 * n + 1, vu(2 * n + 1, 100));
    u64 s = 0;
    for (u64 i = 1, j = 0; i != 2 * n + 1; ++i) {
        s -= a[i - 1];
        while (j != 2 * n && s + a[j + 1] <= w) {
            s += a[j + 1];
            ++j;
        }
        // debug("{i, j}", puu{i, j});
    ...
    for (u64 base_len = 1; base_len != n; ++base_len) {
        for (u64 i = 1; i != n + 1; ++i) {
            u64 j = i + base_len - 1;
            for (u64 extended_l = i; extended_l != 0; --extended_l) {
                if (prefix[j] - prefix[extended_l - 1] > 2 * w) {
                    break;
                }
                u64 extended_r = [&]() {
                    u64 lo = j + 1, hi = 2 * n;
                    while (lo < hi) {
                        ...
                        if (prefix[mid] - prefix[extended_l - 1] <= 2 * w) {
                            lo = mid;
                        }
                        else {
                            hi = mid - 1;
                ...
                check_min(f[extended_l][extended_r], f[i][j] + 1);
            }
        }
    }

    u64 ans = inf<u64>;
    for (u64 len = n; len != 2 * n + 1; ++len) {
        for (u64 i = 1; i != n + 1; ++i) {
            if (i + n - 1 <= 2 * n) {
                check_min(ans, f[i][i + n - 1]);
            }
        }
    }
    debug("f", f);
