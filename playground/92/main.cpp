...
void solve_case()
{
  ...
  int n, m;
  cin >> n >> m;
  vector<bitset<1000>> on(m);
  vector<bitset<1000>> off(m);
  for (int i{}; i != m; ++i) {
    int k;
    cin >> k;
    for (int j{}; j != k; ++j) {
  ...
  for (int i{1}; i != pw.size(); ++i) {
    pw[i] = pw[i - 1] * 2 % mod998244353;
  }

  i64 ans{};
  bitset<1000> p, q;
  for (auto b{1UL}; b != 1UL << m; ++b) {
    auto const sign{popcount(b) % 2 == 0 ? -1 : 1};
    for (int i{}; i != m; ++i) {
      if (b & 1 << i) {
        p |= off[i];
        q |= on[i];
    ...
    if ((p & q).none()) {
      debug("b", bitset<3>{b});
      debug("no", pw[n - (p | q).count()]);
      ans += sign * pw[n - (p | q).count()] % mod998244353;
      ans %= mod998244353;
    }
    p.reset();
    q.reset();
  }
  ans = (pw[n] - ans) % mod998244353;
  cout << (ans + mod998244353) % mod998244353;
}
} // namespace
