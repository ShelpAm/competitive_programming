...
template<size_t n> std::enable_if<n % 2 == 1>::type check()
{
  cout << "Calling check() for odd\n";
}
template<size_t n> std::enable_if<n % 2 == 0>::type check()
{
  cout << "Calling check() for even\n";
}
template<size_t n> concept even = requires { requires n % 2 == 0; };
template<size_t n>
// requires(n % 2 == 0)
requires even<n>
void check()
{
  cout << "ANOTHER check() for evens\n";
}
