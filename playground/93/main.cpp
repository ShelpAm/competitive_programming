...
void solve_case()
{
  ...
  unordered_map<int, set<int>> heights;
  for (auto const &[w, h] : a) {
    ++cnt[{w, h}];
    check_max(height[w], h);
    heights[w].insert(h);
  }
  if (!height.empty()) {
    auto it{height.begin()};
    while (next(it) != height.end()) {
      while (next(it) != height.end() && it->second <= next(it)->second) {
        it = height.erase(it);
      }
      if (next(it) != height.end()) {
        ++it;
      }
    }
  }
  ...
  for (auto const &[w, h] : a) {
    ...
    auto it{height.lower_bound(w)};
    {
      ...
      if (jt != height.end()) {
        check_min(res, h - jt->second);
      }
    }
    if (it != height.begin()) {
      check_min(res, w - prev(it)->first);
    }
    if (res == inf<int>) { // No nodes in left and right, go to find in lower h
      check_min(res, h - *prev(heights[w].lower_bound(h)));
    }
    /*else {*/
    /*  assert(res != inf<int>);*/
    /*}*/
