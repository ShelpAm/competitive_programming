...
class segment_tree {
  ...
  public:
    segment_tree(u64 const size): segment_tree(vi(size + 1)) {}
    // The input array should start from the index 1.
    segment_tree(vi const& a): tree_(4 * (a.size() - 1) + 1) { build(1, a.size() - 1, 1, a); }

    void add_to_segment(u64 l, u64 r, i64 d) { add_to_segment_impl(l, r, d, 1); }
    i64 max_on_segment(u64 l, u64 r) { return max_on_segment_impl(l, r, 1); }
  private:
    void build(u64 l, u64 r, u64 p, vi const& a)
    {
        tree_[p].l = l;
        tree_[p].r = r;
        if (l == r) {
    ...
    {
        auto l = p * 2, r = p * 2 + 1;
        tree_[p].value = max(tree_[l].value, tree_[r].value);
    }
    void push_down(u64 p)
    {
        if (tree_[p].l == tree_[p].r) {
            return;
        }

        auto l = p * 2, r = p * 2 + 1;
        tree_[l].tag += tree_[p].tag;
        push_down(l);
        tree_[r].tag += tree_[p].tag;
        push_down(r);
        tree_[p].tag = 0;
    }
    void add_to_segment_impl(u64 l, u64 r, i64 d, u64 p)
    {
        if (l <= tree_[p].l && tree_[p].r <= r) {
            tree_[p].tag += d;
            return;
        }

        auto const lson = p * 2, rson = p * 2 + 1;
        if (tree_[p].l < l) {
            if (tree_[lson].r >= l) {
                add_to_segment_impl(l, r, d, lson);
            }
            add_to_segment_impl(l, r, d, rson);
        }
        else if (tree_[p].r > r) {
            if (tree_[rson].l <= r) {
                add_to_segment_impl(l, r, d, rson);
    ...
    i64 max_on_segment_impl(u64 l, u64 r, u64 p)
    {
        if (l <= tree_[p].l && tree_[p].r <= r) {
            return tree_[p].value + tree_[p].tag;
        }
        ...
        if (tree_[p].l < l) {
            if (tree_[lson].r >= l) {
                check_max(mx, max_on_segment_impl(l, r, lson));
            }
            check_max(mx, max_on_segment_impl(l, r, rson));
        }
        else if (tree_[p].r > r) {
            if (tree_[rson].l <= r) {
                check_max(mx, max_on_segment_impl(l, r, rson));
...
class segment_tree_1 {
  ...
  public:
    segment_tree_1(u64 const size): tree_(4 * (size + 1) + 1) {}
    // The input array should start from the index 1.
    segment_tree_1(vi const& a): tree_(4 * (a.size()) + 1) { build(1, a.size() - 1, 1, a); }

    void add_to_segment(u64 l, u64 r, i64 d) { add_to_segment_impl(l, r, d, 1); }
    i64 sum_on_segment(u64 l, u64 r) { return sum_on_segment_impl(l, r, 1); }
  private:
    void build(u64 l, u64 r, u64 p, vi const& a)
    {
        tree_[p].l = l;
    ...
    {
        auto l = p * 2, r = p * 2 + 1;
        tree_[p].value = tree_[l].value + tree_[r].value;
    }
    void push_down(u64 p)
    {
        if (tree_[p].l == tree_[p].r) {
    ...
    void add_to_segment_impl(u64 l, u64 r, i64 d, u64 p)
    {
        ...
        push_down(p);
        auto const lson = p * 2, rson = p * 2 + 1;
        if (tree_[p].l < l && tree_[lson].r >= l) {
            add_to_segment_impl(l, r, d, lson);
            add_to_segment_impl(l, r, d, rson);
        }
        else if (tree_[p].r > r && tree_[rson].l <= r) {
            add_to_segment_impl(l, r, d, rson);
            add_to_segment_impl(l, r, d, lson);
        }
        push_up(p);
    }
    i64 sum_on_segment_impl(u64 l, u64 r, u64 p)
    {
        if (l <= tree_[p].l && tree_[p].r <= r) {
            return tree_[p].value + tree_[p].tag * static_cast<i64>((tree_[p].r - tree_[p].l + 1));
        }
        ...
        i64 sum = 0;
        if (tree_[p].l < l && tree_[lson].r >= l) {
            sum += sum_on_segment_impl(l, r, lson);
            sum += sum_on_segment_impl(l, r, rson);
        }
        else if (tree_[p].r > r && tree_[rson].l <= r) {
            sum += sum_on_segment_impl(l, r, rson);
            sum += sum_on_segment_impl(l, r, lson);
...
auto solve_case()
{
    ...
    for (u64 i = 0; i != q; ++i) {
        ...
        if (op == 1) {
            i64 k;
            cin >> k;
            st.add_to_segment(x, y, k);
        }
        else if (op == 2) {
            cout << st.sum_on_segment(x, y) % m << '\n';
        }
    }
}
