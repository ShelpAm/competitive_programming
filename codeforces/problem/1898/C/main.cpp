...
auto solve_case()
{
    ...
    auto vertical = vvc(n - 1, vc(m, 'B')), horizontal = vvc(n, vc(m - 1, 'B'));
    auto rev = [](char const ch) { return ch == 'R' ? 'B' : 'R'; };
    horizontal[0][0] = horizontal[1][0] = 'R';
    for (u64 i{1}; i < m - 1; ++i) {
        horizontal.front()[i] = rev(horizontal.front()[i - 1]);
    }
    vertical[0].back() = rev(horizontal.front().back());
    for (u64 i{1}; i < n - 1; ++i) {
        vertical[i].back() = rev(vertical[i - 1].back());
    }
    horizontal[n - 1].back() = horizontal[n - 2].back()
        = vertical.back().back();
    vertical.back()[m - 2] = rev(vertical.back().back());
    cout << "YES\n";
    print(horizontal);
    print(vertical);
}
