...
static inline auto solve_case()
{
  ...
  for (size_t i{}; i != n; ++i) {
    set<size_t> next_check;
    vec1uz dying;
    for (auto const j: check) {
      auto it{alive.find(j)};
      auto const pre{prev(it)}, nex{next(it)};
      if (d[j] < (it != alive.begin() ? a[*pre] : 0) + (nex != alive.end() ? a[*nex] : 0)) {
        dying.emplace_back(j);
        if (j != *alive.rbegin()) {
          next_check.emplace(*nex);
        }
        if (j != *alive.begin()) {
          next_check.emplace(*pre);
        }
      }
    }
    cout << dying.size() << ' ';
    for (auto const j: dying) {
      alive.erase(j);
      next_check.erase(j);
    }
    check = next_check;
