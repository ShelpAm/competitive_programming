...
static inline auto solve_case()
{
  ...
  vec1uz sum_to_right(n + 1), sum_to_left(n + 2);
  for (size_t i{1}; i != n + 1; ++i) {
    sum_to_right[i] = sum_to_right[i - 1] + (go_right[i - 1] ? 1 : a[i] - a[i - 1]);
  }
  for (size_t i{n}; i != 0; --i) {
    sum_to_left[i] = sum_to_left[i + 1] + (go_right[i + 1] ? a[i + 1] - a[i] : 1);
  }

  size_t m;
  cin >> m;
  for (size_t mi{}; mi != m; ++mi) {
    size_t x, y;
    cin >> x >> y;
    if (x < y) {
      cout << sum_to_right[y] - sum_to_right[x] << '\n';
    }
    else {
      cout << sum_to_left[y] - sum_to_left[x] << '\n';
    }
  }
  // print(a);
  // print(go_right);
