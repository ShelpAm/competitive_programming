...
static inline auto solve_case()
{
  ...
  auto possible{[&]() {
    i64 cnt1{}, cnt2{};
    for (i64 i{}; i != n; ++i) {
      if (auto const diff{(a[i] - a[(i + 1) % n])}; diff < 0) {
        ++cnt1;
      }
      else if (diff > 0) {
        ++cnt2;
  ...
  auto non_reversing_cost{[&]() {
    i64 cnt{};
    i64 const mx{max(a)}, mn{min(a)};
    i64 begin{};
    while (mn != a[(begin + n) % n] || mx != a[(begin - 1 + n) % n]) {
      if (begin == -n) {
        return -1LL;
      }
      if (mx == a[(begin + n) % n] && a[(begin - 1 + n) % n] == mn) {
        ++cnt;
        break;
      }
      --begin;
  ...
  i64 ans{inf<i64>};
  check_min(ans, non_reversing_cost());
  std::ranges::reverse(a);
  check_min(ans, non_reversing_cost() + 1);
  return ans;
}
