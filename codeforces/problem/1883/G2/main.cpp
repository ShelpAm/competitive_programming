...
auto solve_case()
{
    ...
    debug("v", v);
    vi suffix(2 * n + 2);
    for (i64 i{2 * n - 2}; i != -1; --i) {
        suffix[i] = suffix[i + 1] + v[i].second;
    }
    debug("suffix", suffix);
    vi cnt_zero(2 * n + 2);
    cnt_zero[0] = suffix[0] == 0 ? 1 : 0;
    for (i64 i{1}; i != suffix.size(); ++i) {
        cnt_zero[i] = cnt_zero[i - 1] + (suffix[i] == 0 ? 1 : 0);
    }
    auto minus_one = count(suffix, -1);
    u64 ans{};
    i64 last_val{1};
    for (i64 i{}; i != v.size(); ++i) {
        auto interval = pii{last_val, v[i].first - 1};
        auto [l, r] = interval;
        if (l > r) {
            continue;
        }
        ...
        check_min(r, m);
        ans += (cnt_zero[i] + minus_one) * (r - l + 1);
        debug("delta", (cnt_zero[i] + minus_one) * (r - l + 1));
        if (r == m) {
            break;
        }
        last_val = r + 1;
