...
auto solve_case()
{
  ...
  // f[i] = 111111 -> where there are i+1 1s
  // vu f(n + 1);
  // f[0] = 1;
  // for (u64 i{1}; i != n + 1; ++i) {
  //   f[i] = 10 * f[i - 1] + 1;
  ...
  // high_accuracy_number num{};
  // for (u64 i{}; i != n; ++i) {
  //   num = num + high_accuracy_number{string(i + 1, '1')} * (s[n - 1 - i] -
  //   '0');
  // }
  // cout << num << '\n';
  ...
  // debug("prefix", prefix | drop(1));

  for (u64 i{n}; i != 0; --i) {
    prefix[i] += prefix[i + 1] / 10;
    prefix[i + 1] %= 10;
  }

  // debug("pushed_prefix", prefix);

  for (auto const i: prefix | drop(1) | take(n)) {
    cout << i;
  }
  cout << '\n';
}
