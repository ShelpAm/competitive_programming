...
auto solve_case()
{
  ...
  char trump;
  cin >> trump;
  std::vector<string> heap(2 * n);
  cin >> heap;

  unordered_map<char, vu> cards;
  for (auto const& card: heap) {
    cards[card[1]].push_back(card[0] - '0');
  }

  for (auto& [kind, stack]: cards) {
    sort(stack);
  }
  ...
  for (auto& [kind, stack]: cards) {
    ...
    while (stack.size() >= 2) {
      pairs.push_back(
          {std::to_string(stack[0]) + kind, std::to_string(stack[1]) + kind});
    ...
    if (!stack.empty()) {
      ...
      pairs.push_back({std::to_string(stack[0]) + kind,
                       std::to_string(cards[trump].front()) + trump});
      cards[trump].erase(cards[trump].begin());
    }
  }

  {
    auto& stack{cards[trump]};
    auto const kind{trump};
    while (stack.size() >= 2) {
      pairs.push_back(
          {std::to_string(stack[0]) + kind, std::to_string(stack[1]) + kind});
      stack.erase(stack.begin());
