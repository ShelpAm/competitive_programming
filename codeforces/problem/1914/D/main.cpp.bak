...
{
  constexpr i64 k{3};
  // n must be greater than or equal to cnt_line.
  i64 n;
  cin >> n;
  vvi v(k, vi(n));
  cin >> v;
  ...
  for (i64 i{}; i != k; ++i) {
    ...
    sort(idx, [&](auto const l, auto const r) { return v[i][l] > v[i][r]; });
    candidates.emplace_back(idx.begin(), idx.begin() + k);
  }

  i64 ans{};
  set<i64> selected;
  auto dfs{[&](auto self, i64 const i, i64 const sum) -> void {
    if (i == k) {
      check_max(ans, sum);
      return;
    }
