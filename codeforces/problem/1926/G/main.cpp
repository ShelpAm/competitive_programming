...
auto solve_case()
{
  ...
  vvu f(n, vu(2, 0));
  for (u64 u{n - 1}; u != static_cast<u64>(-1); --u) {
    if (type[u] != 'C') {
      f[u][type[u] == 'P' ? 1 : 0] = inf<u64>;
    }
    for (auto const v: adj[u]) {
      for (auto const t: {0, 1}) {
        f[u][t] += min(f[v][t], f[v][t ^ 1] + 1);
      }
    }
  }
  return min(f[0]);
  ...
  //
  // vb vis(n);
  // auto C{[&](auto self, u64 const u, u64 const prev) -> puu {
  //   vis[u] = true;
  //   u64 P{}, S{};
  //   for (auto const v: adj[u]) {
  //     if (v == prev) {
  ...
  //     }
  //     if (s[v] == 'C') {
  //       auto const [dp, ds]{self(self, v, u)};
  //       P += dp;
  //       S += ds;
  //     }
  //     else if (s[v] == 'P') {
