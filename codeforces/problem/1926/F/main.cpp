...
auto solve_case()
{
  std::vector<pii> const dirs{{-1, -1}, {-1, 1}, {1, -1}, {1, 1}};

  u64 const n{7};
  std::vector<string> v(n);
  cin >> v;

  auto is_bad_point{[&](pii const& point) {
    auto const& [i, j]{point};
    if (i == 6 || i == 0 || j == 6 || j == 0) {
      return false;
    }
    if (v[i][j] != 'B') {
  ...
  auto is_connecting_bad_points{[&](pii const& point) {
    auto const& [i, j]{point};
    if (v[i][j] != 'B') {
      return false;
    }
    for (auto const& [di, dj]: dirs) {
      if (is_bad_point({i + di, j + dj})) {
        return true;
      }
    }
    return false;
  ...
  auto solve{[&](u64 const mod) {
    std::vector<pii> black_points;
    std::set<pii> bad_points;
    for (u64 i{1}; i != n - 1; ++i) {
      for (u64 j{1}; j != n - 1; ++j) {
        if ((i + j) % 2 == mod && v[i][j] == 'B') {
          black_points.emplace_back(i, j);
        }
        if ((i + j) % 2 == mod && is_bad_point({i, j})) {
          bad_points.insert({i, j});
        }
      }
    }

    debug("mod", mod);
    debug("bad_points", bad_points);

    auto ok{[&](u64 const mod) {
      for (u64 i{1}; i != n - 1; ++i) {
        for (u64 j{1}; j != n - 1; ++j) {
          if ((i + j) % 2 == mod && is_bad_point({i, j})) {
            return false;
    ...
    u64 ans{inf<u64>};
    auto dfs{[&](auto self, u64 const cnt) {
      if (cnt > ans) {
        return;
      }
      if (ok(mod)) {
      ...
      for (auto const& [i, j]: black_points) {
        if (v[i][j] == 'B' && is_connecting_bad_points({i, j})) {
          v[i][j] = 'W';
          self(self, cnt + 1);
          v[i][j] = 'B';
        }
