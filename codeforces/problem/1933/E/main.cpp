...
auto solve_case()
{
    ...
    for (auto [l, u]: queries) {
        auto next = lower_bound(prefix, prefix[l - 1] + u) - prefix.begin();
        auto prev = next - 1;
        if (prev < l) {
            // debug("prev is less than l, prev:", prev);
            prev = l;
            next = prev + 1;
        }
        i64 d1 = prefix[prev] - prefix[l - 1];
        i64 d2 = next <= n ? prefix[next] - prefix[l - 1] : -0x3f3f3f3f;
        // debug("prev, next",
        //       pair{(2 * u - d1 + 1) * d1 / 2, (2 * u - d2 + 1) * d2 / 2});
        // debug("prev", prev);
        // debug("next", next);
        if ((2 * u - d1 + 1) * d1 / 2 >= (2 * u - d2 + 1) * d2 / 2) {
            cout << prev << ' ';
        }
        else {
            cout << next << ' ';
