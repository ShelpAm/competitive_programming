...
auto solve_case()
{
    ...
    vu lcnt(n + 1), rcnt(n + 1);
    auto up = [&](u64& mex, vu const& cnt) {
        while (cnt[mex] != 0) {
            ++mex;
        }
    };
    auto down = [&](u64& mex, vu const& cnt, u64 last_decreased) {
        if (cnt[last_decreased] == 0 && mex > last_decreased) {
            mex = last_decreased;
        }
        while (mex != 0 && cnt[mex - 1] == 0) {
            --mex;
        }
    };
    u64 lmex = 0, rmex = 0;
    ...
    i64 p = -1;
    for (i64 idx = 0; auto e: a | take(n - 1)) {
        ++lcnt[e];
        --rcnt[e];
        up(lmex, lcnt);
        down(rmex, rcnt, e);
        debug("lcnt", lcnt | take(10));
        debug("rcnt", rcnt | take(10));
        debug("lmex rmex", puu{lmex, rmex});
        if (lmex == rmex) {
            p = idx + 1;
        }
        ++idx;
