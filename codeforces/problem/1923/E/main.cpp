...
auto solve_case()
{
    ...
    std::vector<std::unordered_map<u64, u64>> cnt(n);
    u64 ans{};
    auto dfs{[&](auto self, u64 const u, u64 const p) -> void {
        for (auto const v: adj[u]) {
            if (v == p) {
                continue;
            }
            self(self, v, u);
            if (cnt[v].contains(colors[u])) {
                ans += cnt[v][colors[u]];
                cnt[v].extract(colors[u]);
            }
            if (cnt[u].size() < cnt[v].size()) {
                swap(cnt[u], cnt[v]);
            }
            for (auto const [color, count]: cnt[v]) {
                ans += cnt[u][color] * count;
                cnt[u][color] += count;
            }
        }
        cnt[u][colors[u]] = 1;
