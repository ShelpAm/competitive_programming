...
auto solve_case()
{
  ...
  vi prev_different(n + 1), next_different(n + 1);
  prev_different[0] = -1;
  for (i64 i{1}; i != n + 1; ++i) {
    prev_different[i] = a[i] == a[i - 1] ? prev_different[i - 1] : i - 1;
  }
  next_different[n] = n + 1;
  for (i64 i{n}; i != -1; --i) {
    next_different[i] = a[i] == a[i + 1] ? next_different[i + 1] : i + 1;
  }

  // debug("prev_diff", prev_different);
  // debug("next_diff", next_different);

  for (i64 i{1}; i != n + 1; ++i) {
    i64 seconds{inf<i64>};
    auto left_pos{--lower_bound(prefix, prefix[i - 1] - a[i]) - prefix.begin()};
    auto right_pos{upper_bound(prefix, a[i] + prefix[i]) - prefix.begin()};
    // debug("left_pos", left_pos); debug("right_pos", right_pos);

    if (left_pos > -1) {
      ++left_pos;
      if (next_different[left_pos] >= i && left_pos != i - 1) {
        left_pos = prev_different[left_pos];
      }
      if (left_pos > 0) {
        check_min(seconds, i - left_pos);
      }
    }
    if (right_pos < n + 1) {
      if (prev_different[right_pos] <= i && right_pos != i + 1) {
        right_pos = next_different[right_pos];
      }
      if (right_pos < n + 1) {
        check_min(seconds, right_pos - i);
      }
    }
    cout << (seconds == inf<i64> ? -1 : seconds) << ' ';
  }
  cout << '\n';
}
