...
auto solve_case() -> long double
{
    ...
    auto p = [&](auto p, u64 i, u64 j) -> long double {
        ...
        long double ans = 0;
        ans += static_cast<long double>(i) / (i + j);
        if (i >= 1 && j >= 2) {
            ans += static_cast<long double>(j) / (i + j) * (j - 1) / (i + j - 1) * i / (i + j - 2) * p(p, i - 1, j - 2);
        }
        if (j >= 3) {
            ans += static_cast<long double>(j) / (i + j) * (j - 1) / (i + j - 1) * (j - 2) / (i + j - 2) * p(p, i, j - 3);
        }
        return f[i][j] = ans;
    };
