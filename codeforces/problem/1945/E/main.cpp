...
auto solve_case()
{
    // It can be shown that if x is at critical positions, the element on l is non-greater than x.
    // Otherwise, x makes no contribution to the resulting `l` position.

    i64 n, x;
    cin >> n >> x;
    vi a(n);
    cin >> a;
    auto p = find(a, x) - a.begin();
