...
auto solve_case()
{
    ...
    std::priority_queue<puu, std::vector<puu>, decltype(cmp)> order{cmp};
    for (u64 i = 0; i != n; ++i) {
        order.push({v[i], i});
    }

    multiset<u64> s;
    u64 max_strenth_to_use = 0, min_number_to_use = inf<u64>;

    auto push_one = [&](u64 i) {
        while (!order.empty()) {
            auto [val, perm] = order.top();
            debug("perm", perm);
            if (perm >= i - 1) {
                break;
            }
    ...
    for (u64 i = 1; i <= n; ++i) {
        while (s.size() != i && push_one(i)) {
        }
        if (s.size() != i) {
            break;
        }
        auto alt = *s.begin() * s.size();
        if (alt > max_strenth_to_use) {
            max_strenth_to_use = alt;
            min_number_to_use = s.size();
        }
        s.extract(v[i - 1]);
    }
    assert(max_strenth_to_use != 0);
    cout << max_strenth_to_use << ' ' << min_number_to_use << '\n';
}
