...
auto solve_case()
{
  ...
  auto check{[&](u64 const max_cost) {
    // debug("checking", max_cost);
    vu f(n + 2); // f[i] denotes the cost of blocked elemnets when i is blocked.
    std::deque<u64> q;
    q.push_back(0);
    for (u64 r{1}, l{1}; r != n + 2; ++r) {
      while (prefix[r - 1] - prefix[l - 1] > max_cost) {
        if (q.front() == f[l - 1]) {
          q.pop_front();
        }
        if (++l == r) {
      ...
      f[r] = q.front() + a[r];
      while (!q.empty() && q.back() > f[r]) {
        q.pop_back();
      }
      q.push_back(f[r]);
      // debug("< q >", puu{q.front(), q.back()});
      // debug("f", f);
    }
    return f[n + 1] <= max_cost;
  }};

  i64 lo{}, hi{inf<i64>};
  while (lo < hi) {
    i64 const mid{(lo + hi) / 2};
    if (check(mid)) {
      hi = mid;
    }
    else {
