...
static inline auto solve_case()
{
  ...
  auto get_bit{[](size_t const num, size_t const i) { return (num >> i) & 1; }};

  bool first{true};
  for (auto i{n - 1}; i != -1UZ; --i) {
    if (get_bit(a, i) == 1 && get_bit(b, i) == 0) {
      if (first) {
        first = false;
        continue;
      }
      if (auto const x{1UZ << i}; r >= x) {
        a ^= x;
        b ^= x;
        r -= x;
      }
