...
static inline auto solve_case()
{
  i64 n;
  cin >> n;
  std::vector<pair<string, i64>> posts(n);
  unordered_map<string, i64> person;
  for (i64 i{}; i != n; ++i) {
    cin >> posts[i];
    person[posts[i].first] += posts[i].second;
  }

  i64 max_score{};
  for (auto const& [_, score]: person) {
    check_max(max_score, score);
  }

  set<string> candidates;
  for (auto const& [name, score]: person) {
    if (score == max_score) {
      candidates.emplace(name);
    }
  }
  ...
  for (i64 i{}; i != n; ++i) {
    person[posts[i].first] += posts[i].second;
    if (person[posts[i].first] >= max_score && candidates.contains(posts[i].first)) {
      return posts[i].first;
    }
  }
