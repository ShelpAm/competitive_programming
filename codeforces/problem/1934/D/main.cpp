...
auto solve_case()
{
    ...
    u64 n, m;
    cin >> n >> m;
    if (hi_bit(n) == hi_bit(m)) {
        cout << 1 << '\n';
        cout << n << ' ' << m << '\n';
    }
    else { // hibit n > hibit m
        if (std::popcount(n) == 1) {
            cout << -1 << '\n';
        }
        else {
            if (second_bit(n) >= hi_bit(m)) {
                auto const sec_bit = second_bit(n);
                vu a{n};
                a.push_back((1ULL << (sec_bit + 1)) - 1);
                if (a.back() != m) {
                    a.push_back(m);
                }
                cout << a.size() - 1 << '\n';
                // for (auto e: a) {
                //     cout << std::bitset<64>{e} << '\n';
                // }
                print(a);
            }
            else {
