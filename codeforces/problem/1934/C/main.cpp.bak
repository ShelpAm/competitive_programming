...
auto solve_case()
{
    ...
    auto answer = [&](u64 x, u64 y) { cout << "! " << x << " " << y << '\n'; };

    auto a = query(1, 1), b = query(n, 1), c = query(1, m), d = query(n, (m + 1) / 2);

    auto dis = [](pii a, pii b) { return abs(a.first - b.first) + abs(a.second - b.second); };

    pii A{(1 + a - b + n) / 2, (a + b + 3 - n) / 2};
    pii B{(2 * n - 1 - b + (m - 1) / 2 - d) / 2, (b - d + 1 + (m + 1) / 2) / 2};
    pii C{(c + a + 3 - m) / 2, (1 + a + m - c) / 2};
    pii D{(1 + c - m + n + (m + 1) / 2 - d) / 2, (m - 1 - c + n + (m + 1) / 2 - d) / 2};
    if (dis(A, {1, 1}) <= dis(C, {1, 1}) && dis(A, {n, 1}) <= dis(B, {n, 1})) {
        answer(A.first, A.second);
    }
    else if (dis(C, {1, 1}) <= dis(A, {1, 1}) && dis(C, {1, m}) <= dis(D, {1, m})) {
        answer(C.first, C.second);
    }
    else if (dis(B, {n, 1}) <= dis(A, {n, 1})
             && dis(B, {n, (m + 1) / 2}) <= dis(D, {n, (m + 1) / 2})) {
        answer(B.first, B.second);
    }
    else {
        answer(D.first, D.second);
    }
}
