...
static inline auto solve_case()
{
  long long h, w;
  cin >> h >> w;
  long long ia, ja, ib, jb;
  cin >> ia >> ja >> ib >> jb;
  if (ia > ib)
    return "Draw";
  if ((ib - ia) % 2 == 1) { // Alice chases
    if (std::abs(ja - jb) <= 1) {
      return "Alice";
    }
    if (ja > jb) {
      return ja - (ib - ia) / 2 <= 2 ? "Alice" : "Draw";
    }
    return ja + (ib - ia) / 2 >= w - 1 ? "Alice" : "Draw";
  }
  // Alice flees
  if (ja == jb) {
    return "Bob";
  ...
  if (ja > jb) {
    return jb + (ib - ia) / 2 >= w ? "Bob" : "Draw";
  }
  return jb - (ib - ia) / 2 <= 1 ? "Bob" : "Draw";
}
