...
static auto solve_case()
{
  ...
  vu f0(n, 1), f1(n), f2(n); // f1 is identical to the size of subtree
  vu size(n);
  auto dfs{[&](auto self, u64 u, u64 prev) -> void {
    ...
    u64 subtree_kinds{1};
    for (auto const v: adj[u]) {
      subtree_kinds = subtree_kinds * (f1[v] + f0[v]) % mod<u64>;
    }
    f1[u] = subtree_kinds % mod<u64>;

    for (auto const v: adj[u]) {
      f2[u] = (f2[u] + f1[v] + f2[v]) % mod<u64>;
    }
  }};
  ...
  // debug("f1", f1);
  // debug("f2", f2);
  return (f0[root] + f1[root] + f2[root]) % mod<u64>;
}
