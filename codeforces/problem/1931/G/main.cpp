...
static inline auto solve_case()
{
  ...
  auto comb{[&](i64 const n, i64 const m) {
    cout << n << ' ' << m << " GET " << fac[n] * pinv[m] % p * pinv[n - m] % p
         << '\n';
    return fac[n] * pinv[m] % p * pinv[n - m] % p;
  }};
  ...
  while (t--) {
    ...
    i64 ans{comb(c + a - 1, a - 1) * comb(d + b - 1, b - 1) % mod<i64>};
    if (a == b && a != 0) {
      ans = ans * 2 % mod<i64>;
    }
    cout << ans << '\n';
