...
static inline auto solve_case()
{
  ...
  if (n == 2) {
    assert(k == 2);
    std::ranges::reverse(a[0]);
    return a[0] == a[1];
  }
  vi standard{a[0].begin() + 1, a[0].end()};
  assert(standard.size() == n - 1);
  auto const x{a[0].front()};
  if (auto const x_pos{std::ranges::find(a[1], x) - a[1].begin()};
      x_pos != n - 1) {
    auto const next{a[1][x_pos + 1]};
    standard.insert(std::ranges::find(standard, next), x);
  }
  else if (x_pos != 1) {
    auto const prev{a[1][x_pos - 1]};
    standard.insert(next(std::ranges::find(standard, prev)), x);
  }
  else {
    assert(false);
  }
  assert(standard.size() == n);

  for (auto list: a) {
    auto copy{standard};
    copy.erase(std::ranges::find(copy, list.front()));
