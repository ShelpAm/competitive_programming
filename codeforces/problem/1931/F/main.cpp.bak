...
static constexpr auto& operator>>(auto& istream, auto&& t)
{
  ...
  if constexpr (std::ranges::range<T>) {
    ...
    // static_assert(false, "[operator>>] tuple: not implemented yet.");
  }
  else {
    istream >> t;
  }
...
static constexpr void print(auto const& t)
{
  ...
  if constexpr (impl::string_like<T>) {
    ...
    // static_assert(false, "[print] tuple: not implemented yet.\n");
  }
#endif
  else {
    std::cout << t << ' ';
...
static auto solve_case()
{
  ...
  impl::vec1<set<u64>> adj(n);
  for (auto const& a: v) {
    for (u64 i{1}; i < n - 1; ++i) {
      adj[a[i] - 1].emplace(a[i + 1] - 1);
    }
  }

  auto has_cycle{[n, &adj]() {
    vb perm_vis(n);

    for (u64 i{}; i != n; ++i) {
      vb tmp_vis(n);
      auto visit{[&](auto self, u64 const u) -> bool {
        if (perm_vis[u]) {
          return false;
        }
        if (tmp_vis[u]) {
