...
using order_set
    = tree<i64, null_type, less<>, rb_tree_tag, __gnu_pbds::tree_order_statistics_node_update>;
...
static inline auto solve_case()
{
  i64 n;
  cin >> n;
  std::vector<pii> intervals(n);
  cin >> intervals;

  sort(intervals, [](auto const& l, auto const& r) { return l.first < r.first; });

  // priority_queue<i64, vi, greater<>> rs;
  i64 cnt{};
  // set<i64> rss;
  order_set rs;
  for (auto const& [l, r]: intervals) {
    ...
    rs.insert(r);
    cnt += rs.size() - rs.order_of_key(r) - 1;
  }
  return cnt;
}
