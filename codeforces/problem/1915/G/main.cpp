...
auto dijkstra(impl::vec2<pii> const& adjacent, size_t const source, vi const& s)
{
  vvi dis(adjacent.size(), vi(1001, inf<i64>));
  vi prev(adjacent.size());
  dis[source][s[source]] = 0;

  priority_queue<std::tuple<i64, i64, i64>, impl::vec1<std::tuple<i64, i64, i64>>, greater<>> q;
  q.emplace(dis[source][s[source]], s[source], source);

  while (!q.empty()) { // The main loop
    auto const [_, si, u]{q.top()}; // Extract closest vertex. (Get and remove best vertex)
    q.pop();

    for (auto const& [w, v]: adjacent[u]) {
      if (auto const alt{dis[u][si] + w * si}, new_s{min(s[v], si)}; alt < dis[v][new_s]) {
        dis[v][new_s] = alt;
        prev[v] = u;
        q.emplace(alt, new_s, v);
      }
    }
  }
...
static inline auto solve_case()
{
  ...
  vi s(n);
  cin >> s;
  auto [dis, prev]{dijkstra(adj, 0, s)};
  // debug("dis", dis);
  return min(dis[n - 1]);
}
