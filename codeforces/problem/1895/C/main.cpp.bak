...
auto solve_case()
{
  ...
  u64 ans{};
  for (auto const& rev: {false, true}) {
    for (auto& longer: nums) {
      if (rev) {
        std::ranges ::reverse(longer);
      }
      for (u64 i{longer.size() % 2}; i <= longer.size() - rev; i += 2) {
        i64 sum{};
        for (auto const ch: longer | take((i + longer.size()) / 2)) {
          sum += ch - '0';
        }
        for (auto const ch: longer | drop((i + longer.size()) / 2)) {
          sum -= ch - '0';
        }
        if (sum > 0) {
          ans += cnt[i][sum];
