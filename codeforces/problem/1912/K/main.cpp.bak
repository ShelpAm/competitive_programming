...
static inline auto solve_case()
{
  ...
  // f[i][j] denotes the number of who ends with xx at i position
  // 0 -> oo;  1 -> oe;  2 -> eo;  3 -> ee
  vvi f(n + 1, vi(4)), g(n + 1, vi(4));
  g[2][0] = f[2][0] = a[1] % 2 == 1 && a[2] == 1 ? 1 : 0;
  g[2][1] = f[2][1] = a[1] % 2 == 1 && a[2] == 0 ? 1 : 0;
  g[2][2] = f[2][2] = a[1] % 2 == 0 && a[2] == 1 ? 1 : 0;
  g[2][3] = f[2][3] = a[1] % 2 == 0 && a[2] == 0 ? 1 : 0;
  for (i64 i{3}; i != n + 1; ++i) {
    if (a[i] % 2 == 1) {
      f[i][0] = g[i - 1][0] + g[i - 1][2] + (a[1] == 1);
      f[i][2] = g[i - 1][1] + g[i - 1][3] + (a[1] == 0);
    }
    if (a[i] % 2 == 0) {
      f[i][1] = g[i - 1][0] + g[i - 1][2] + (a[1] == 1);
      f[i][3] = g[i - 1][1] + g[i - 1][3] + (a[1] == 0);
    }
    for (i64 j{}; j != 4; ++j) {
      g[i][j] = g[i - 1][j] + f[i][j];
    }
  ...
  i64 ans{};
  for (i64 i{3}; i != n + 1; ++i) {
    ans += a[i] % 2 == 1 ? g[i - 1][1] + g[i - 1][2] : g[i - 1][0] + g[i - 1][3];
    ans %= mod<i64>;
  }
  return ans;
}
