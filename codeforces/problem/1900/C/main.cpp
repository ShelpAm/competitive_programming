...
auto solve_case()
{
  ...
  cin >> s;
  s.insert(s.begin(), '\0');
  std::vector<puu> sons(n + 1);
  cin >> (sons | drop(1));

  vb is_leaf(n + 1);
  for (u64 i{1}; i != n + 1; ++i) {
    auto const& [l, r]{sons[i]};
    if (l == 0 && r == 0) {
      is_leaf[i] = true;
    }
  }
  auto dfs{[&](auto self, u64 const u) -> u64 {
    auto const& [l, r]{sons[u]};
    if (u == 0) {
      return 0;
    }
    if (is_leaf[u]) {
    ...
    u64 ans{inf<u64>};
    if (l != 0) {
      check_min(ans, (s[u] != 'L' ? 1 : 0) + self(self, l));
    }
    if (r != 0) {
      check_min(ans, (s[u] != 'R' ? 1 : 0) + self(self, r));
    }
    return ans;
  }};
  return dfs(dfs, 1);
