...
using order_set = tree<pii, null_type, less<>, rb_tree_tag,
...
auto solve_case()
{
    ...
    for (auto e: a) {
        if (e > 0) {
            mp[e].push_back(mp[e].empty() ? 0 : mp[e].back() + 1);
            os.insert({e, mp[e].back()});
        }
        else if (e < 0) {
            sum_negatives += e;
        }
    ...
    for (auto [x, v]: queries) {
        ...
        if (u < 0) {
            ...
            if (v < 0) {
                ...
                mp[v].push_back(mp[v].empty() ? 1 : mp[v].back() + 1);
                os.insert({v, mp[v].back()});
            }
        }
        else {
            if (v < 0) {
                os.erase({u, mp[u].back()});
                mp[u].pop_back();
                sum_negatives += v;
            }
            else {
        ...
        auto cnt_of_blocked = os.order_of_key({-sum_negatives + 1, i64{}});
        cout << cnt_of_blocked + 1 << '\n';
    }
}
