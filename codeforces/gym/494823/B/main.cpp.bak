...
{
  arr1<size_t, 1001> squares;
  for (size_t i{}; i != squares.size(); ++i) {
    squares[i] = i * i;
  }
  arr1<size_t, 1000001> dp;
  dp.fill(0x3f3f3f3f);
  dp[0] = 0;
  for (auto const e: squares) {
    for (size_t i{e}; i < dp.size(); ++i) {
      check_min(dp[i], dp[i - e] + 1);
    }
  }
...
static inline void solve_all_cases(auto&& solve_case_f)
{
  ...
  for (size_t i{}; i != t; ++i) {
    if constexpr (std::is_void_v<decltype(solve_case_f())>) {
      ...
      print(solve_case_f() ? "First" : "Second");
    }
  }
}
int main()
