...
class segment_tree {
    using operation = decltype([](auto l, auto r) { return max(l, r); });
  public:
    segment_tree(u64 const size): segment_tree(vi(size + 1)) {}
    // The input array should start from the index 1.
    segment_tree(vi const& a): nodes_(4 * (a.size() - 1) + 1), n_(a.size() - 1)
    {
        build(1, n_, 1, a);
    }

    void modify_on_segment(u64 l, u64 r)
    {
        add_on_segment_impl(l, r, 1, n_, 1);
        update(1);
    }
    i64 max_on_segment(u64 l, u64 r) { return query_impl(l, r, 1, n_, 1); }
  private:
    void build(u64 l, u64 r, u64 x, vi const& a)
    {
        // if we don't change the structure of the tree, we won't save info of internal nodes,
        // since it can be calculated at runtime.
        if (l == r) {
            nodes_[x].value = a[l];
            return;
        }
    ...
    void update(u64 x) { nodes_[x].value = op_(nodes_[x * 2].value, nodes_[x * 2 + 1].value); }
    void spread_lazy_tag(u64 l, u64 r, u64 x)
    {
        if (l == r) {
            return;
        }

        nodes_[x * 2].tag += nodes_[x].tag;
        nodes_[x * 2].value = op_(nodes_[x * 2].value, nodes_[x * 2].value + nodes_[x].tag);
        nodes_[x * 2 + 1].tag += nodes_[x].tag;
        nodes_[x * 2 + 1].value
            = op_(nodes_[x * 2 + 1].value, nodes_[x * 2 + 1].value + nodes_[x].tag);
        nodes_[x].tag = 0;
    }
    void add_on_segment_impl(u64 a, u64 b, u64 l, u64 r, u64 x)
    {
        if (a <= l && r <= b) {
            nodes_[x].value = n_ :;
            nodes_[x].tag += d;
    ...
    i64 query_impl(u64 a, u64 b, u64 l, u64 r, u64 x)
    {
        if (a <= l && r <= b) {
            debug("a b l r x", vu{a, b, l, r, x});
            debug("nodes_[x].value", nodes_[x].value);
            debug("nodes_[x].tag", nodes_[x].tag);
        ...
        u64 const mid = (l + r) / 2;
        if (a <= mid) {
            res = op_(res, query_impl(a, b, l, mid, x * 2));
        }
        if (mid < b) {
            res = op_(res, query_impl(a, b, mid + 1, r, x * 2 + 1));
        }
        return res;
    }
    struct node {
